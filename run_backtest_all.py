...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="盘后全自动批处理（第134轮八步：采集+回测三件套+切片+对账+体检+影子）"
    )
    parser.add_argument(
        "--limit", type=int, default=0, help="回测只跑前N个品种（0=全部；快速抽样用5或10）"
    )
    parser.add_argument(
        "--skip", default="", help="跳过步骤：逗号分隔名称子串（如 pcr_vol,shadow,病理）"
    )
    ...
    print(
        f"品种: {'全部' if args.limit <= 0 else f'前{args.limit}个'} ｜ 步骤 {len(steps)}/8（跳过: {skip or '无'}）"
    )
    print(f"{'=' * 60}\n")
    ...
    # 报告文件存在性核查（研究的全部落盘物）
    from config import (
        BACKTEST_REPORT_FILE as f1,
    )
    from config import (
        INTRADAY_BT_REPORT_FILE as f2,
    )
    from config import (
    ...
    for label, fpath in checks:
        ...
        print(
            f"  {label} 报告: {'存在 ✅' if exists else '缺失 ❌'} ({os.path.basename(fpath)} {mt})"
        )
