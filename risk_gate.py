...
def record_streak(state, sym, ok, fail_threshold=None, recover_threshold=None):
    ...
    fail_threshold = (
        ...
        else getattr(config, "RISK_GATE_BLACKLIST_FAIL_ROUNDS", 3)
    )
    recover_threshold = (
        recover_threshold
        if recover_threshold is not None
        else getattr(config, "RISK_GATE_BLACKLIST_RECOVER_ROUNDS", 5)
    )
...
def evaluate(row):
    ...
    try:
        ...
        if bl_sym and is_blocked(bl_state, bl_sym):
            veto.append(_BLACKLIST_REASON % getattr(config, "RISK_GATE_BLACKLIST_FAIL_ROUNDS", 3))
        ...
        if (
            abs(score) >= config.SCORE_MID
            and direction != 0
            and direction * chg <= -config.RISK_GATE_DIVERGE_CHG
        ):
            veto.append(
                "强信号(%+.1f)与当日涨跌(%+.2f%%)严重背离，谨防追高/摸顶" % (score, chg * 100)
            )
