...
def test_minute_bars_cross_contract_stitch(tmp_db):
    # 换月后新旧主力按时间自然衔接、升序
    tmp_db.insert_minute_bars(
        [_bar("2026-09-01 09:05", contract="RB2610"), _bar("2026-10-01 09:05", contract="RB2701")]
    )
    rows = tmp_db.minute_bars_for_sym("RB", 5)
...
def _snap(i, equity=None):
    ...
    return {
        "ts": "2026-09-02 %02d:00:00" % (9 + i),
        "static_equity": equity + 50,
        "float_pnl": -50.0,
        "equity": equity,
...
def test_paper_order_status_counts(tmp_db):
    assert tmp_db.paper_order_status_counts() == {
        "pending": 0,
        "filled": 0,
        "blocked": 0,
        "rejected": 0,
...
def _sig_row(name="螺纹钢", score=4.0, ts="2026-09-12 10:00:00", hits=None):
    return {
        ...
        "parts": {"日线动量": 2.0},
        "flow": {},
        "hits": hits or [["2026-09-12 09:59:00", {"source": "x", "content": "央行降息，商品普涨"}]],
        "ts": ts,
    }
...
def test_signals_upsert_dedup_same_day(tmp_db):
    ...
    rows = tmp_db.conn.execute(
        """SELECT COUNT(*), MAX(ts), MAX(sent_json IS NOT NULL) FROM signals
           WHERE variety='螺纹钢' AND date(ts)='2026-09-12'"""
    ).fetchone()
...
def test_signals_upsert_different_day_new_row(tmp_db):
    ...
    rows = tmp_db.conn.execute("SELECT COUNT(*) FROM signals WHERE variety='螺纹钢'").fetchone()[0]
...
def test_signals_sent_json_filled(tmp_db):
    db = storage.MonitorDB(tmp_db.path)
    assert db.insert_future_signals(1, "2026-09-12 10:00:00", [_sig_row()]) == 1
    sent = tmp_db.conn.execute("SELECT sent_json FROM signals WHERE variety='螺纹钢'").fetchone()[0]
    import json
...
def test_prune_slims_raw_json_over_retention(tmp_db):
    ...
    try:
        ...
        db.insert_future_signals(
            1, datetime.now().strftime("%Y-%m-%d %H:%M:%S"), [_sig_row(score=6.0)]
        )
        db.prune()
        rows = tmp_db.conn.execute(
            "SELECT raw_json IS NULL, score FROM signals ORDER BY score"
        ).fetchall()
        ...
        sent = tmp_db.conn.execute(
            "SELECT COUNT(*) FROM signals WHERE sent_json IS NOT NULL"
        ).fetchone()[0]
        assert sent == 2
    finally:
...
# ---- 第145轮 #9：PIT 对齐（insert_fundamentals 用数据日 trade_date + fundamentals_asof 查询） ----
def test_insert_fundamentals_uses_as_of_trade_date(tmp_path):
    """insert_fundamentals 应用 pack["as_of"]（数据日）作为 trade_date，而非采集日。"""
    from storage import MonitorDB

    db = MonitorDB(str(tmp_path / "pit.db"))
    pack = {
        "score": 1.2,
        "as_of": "2026-09-08",
        "sub": {"库存仓单": {"current": 100}},
    }
    ...
    assert row["trade_date"] == "2026-09-08"  # 数据日，不是采集日
...
def test_fundamentals_asof_returns_correct_pit_data(tmp_path):
    """fundamentals_asof 返回 trade_date ≤ 目标的最近一基本面快照（PIT 约束）。"""
    from storage import MonitorDB

    db = MonitorDB(str(tmp_path / "pit2.db"))
    # 插入两行不同数据日
    pack_08 = {"score": 1.0, "as_of": "2026-09-08", "sub": {}}
    pack_11 = {"score": 2.0, "as_of": "2026-09-11", "sub": {}}
    db.insert_fundamentals("2026-09-08 16:00:00", [("螺纹", "RB", pack_08)])
    db.insert_fundamentals("2026-09-11 16:00:00", [("螺纹", "RB", pack_11)])
    # PIT：目标 9-10 → 只能拿 9-08 数据
    pit_10 = db.fundamentals_asof("2026-09-10")
    assert pit_10["RB"]["score"] == 1.0
    # PIT：目标 9-11 → 可拿 9-11 数据
    pit_11 = db.fundamentals_asof("2026-09-11")
    assert pit_11["RB"]["score"] == 2.0
    # PIT：目标在数据之前 → 空
