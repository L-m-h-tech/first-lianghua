...
def test_degrade_paths(with_key):
    ...
    assert (
        lr.review(ROWS, transport=lambda u, p, t: (200, "not json"))["degraded"]
        == "bad_response_body"
    )
...
def test_review_async_never_raises(with_key, tmp_path):
    ...
    assert (
        lr.review_async(ROWS, transport=lambda u, p, t: (200, "bad"), jsonl_path=jsonl) is not None
    )
...
def test_throttle_cap_and_dedup(with_key, tmp_path):
    ...
    with open(jsonl, "w", encoding="utf-8") as fp:
        rec = {
            "ts": today + " 10:00:00",
            "review": {"context": {"signals": [{"variety": "螺纹钢", "score": 7.2}]}},
        }
        fp.write(_json.dumps(rec, ensure_ascii=False) + chr(10))
