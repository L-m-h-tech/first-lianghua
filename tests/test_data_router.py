...
def test_availability_and_snapshot():
    ...
    snap = h.snapshot()
    assert set(
        ["name", "state", "total", "success", "fail", "trips", "availability", "cooldown_remaining"]
    ) <= set(snap)
...
def test_router_primary_success():
    c = Clock()
    r = DataRouter(
        [("a", lambda: "A"), ("b", lambda: "B")], fails_threshold=2, cooldown_sec=10, clock=c
    )
    res = r.request()
...
def test_router_failover_to_secondary():
    ...
    r = DataRouter([("a", bad), ("b", lambda: "B")], fails_threshold=2, cooldown_sec=10, clock=c)
    res = r.request()
