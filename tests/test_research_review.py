...
def test_equity_summary_bom_emptyrow_and_span(tmp_path):
    ...
    with open(str(p), "w", encoding="utf-8-sig", newline="") as f:
...
def test_sec_factor_health_alerts_and_sort():
    obj = {
        ...
        "daily": {
            "x": {"5": {"ic": 0.3}, "halflife": {"half_life": 20.0}},
    ...
    obj = {
        "horizons": {
            "30": {
                ...
                "r2": 0.1,
                "factors": [
                    {"factor": "p", "contrib": 0.001},
                    {"factor": "q", "contrib": -0.002},
                    {"factor": "r", "contrib": 0.003},
                ],
                "bhb_sectors": [
                    {"sector": "s1", "effect": -0.01},
                    {"sector": "s2", "effect": 0.02},
                ],
            }
        }
    }
...
def test_sec_journal_weak_threshold_and_green_ratio():
    obj = {
        ...
        "by_hold_band": [
            {"key": "短", "n": 50, "pf": 0.5, "net": -1},
            {"key": "小", "n": 2, "pf": 0.1, "net": -1},  # n 门槛挡掉
            {"key": "长", "n": 50, "pf": 2.0, "net": 1},
        ],
        "by_score_band": [],
        "excursion": {"loss_once_green": 3, "n_loss": 4},
    }
...
def test_sec_lab_and_validation():
    ...
    assert (
        abs(lab["methods"]["erc"]["sharpe"] - 0.5) < 1e-12 and lab["snapshot_eff_n"]["erc"] == 20.0
    )
    v = rr.sec_validation(
        {
            "dsr": {"dsr": 0.2, "verdict": "x", "n_trials": 18},
...
def test_collect_reads_synthetic_sidecars(tmp_path):
    (tmp_path / "trade_journal.json").write_text(
        json.dumps(
            {
                "n_trades": 10,
                "overall": {
