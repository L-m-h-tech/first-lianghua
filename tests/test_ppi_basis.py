...
def _nest(gap, rate):
    ...
    return (
        '<td><table width="100%%"><tr><td width="50%%" align="center">'
...
def _page(rows_html):
    trs = [
        '<tr align="center"><td>商品</td><td colspan="1">现货</td>'
        '<td colspan="2">最近合约</td><td colspan="3">主力合约</td></tr>'
    ]
    trs.extend(rows_html)
    filler = "生意社商品现货与期货价格对比 " * 400  # 撑过生产 8KB 数据页阈值
    return (
        '<html><head><meta charset="utf-8"><title>生意社：商品现货与期货价格对比表</title></head>'
...
def test_parse_challenge_and_empty_pages_none():
    ...
    # 周末空页（有导航表格但无产品行）
    assert (
        fd.parse_ppi_basis_html(
            "<html><table><tr><td>导航</td></tr><tr><td>首页</td></tr></table></html>"
        )
        is None
    )
...
def test_parse_missing_dominant_pair_skipped():
    ...
    row = (
        '<tr align="center"><td>螺纹钢</td><td>3138.00</td><td>2609</td><td>3000</td>'
...
def test_parse_multiline_and_commas():
    ...
    row = _row_html("碳酸锂", "143,480", "2701", "140,000", "2701", "141,000").replace(
        "碳酸锂", " 碳酸锂 "
    )
