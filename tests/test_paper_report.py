...
def row(
    sym="RB", name="螺纹钢", cat="黑色", score=5.0, price=3000.0, contract_code="", main_month=""
):
    return {
        "sym": sym,
        "name": name,
...
def test_paper_account_text_after_close():
    ...
    st.last_paper = st.paper.on_cycle("2026-09-02 10:05:00", [row(score=1.0, price=3020.0)])  # 平仓
    text = report.paper_account_text(st)
...
def test_account_text_shows_contract(tmp_db):
    ...
    st.last_paper = st.paper.on_cycle(
        "2026-09-02 10:00:00", [row(contract_code="RB2610", main_month="2610")]
    )
    text = report.paper_account_text(st)
    ...
    st2.last_paper = st2.paper.on_cycle(
        "2026-09-02 11:00:00", [row(score=5.0, contract_code="RB2610")]
    )
    st2.last_paper = st2.paper.on_cycle(
        "2026-09-02 11:05:00", [row(score=1.0, contract_code="RB2610")]
    )
...
def test_write_paper_account_cmp_rows_carries_new_fields(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(report.config, "BASE_DIR", str(tmp_path))
    monkeypatch.setattr(
        report.config, "PAPER_ACCOUNT_TXT", str(tmp_path / "reports" / "paper_account.txt")
    )
    (tmp_path / "reports").mkdir(exist_ok=True)
    st = _active_state()
...
def test_paper_gambler_style_and_config():
    ...
    for g in gamblers:
        assert abs(float(g.get("opt_premium_ratio", 0)) - 0.75) < 1e-9, (
            g["name"],
            "权利金占比应75%",
        )
...
def test_paper_accounts_margin_tier_affordability():
    ...
    cases = {
        ...
        "5000_保守": (5_000, 0.32),
        "3000_激进": (3_000, 0.54),
        "3000_赌徒": (3_000, 0.59),  # 第141轮：赌徒 per_symbol 上调 0.5
    }
