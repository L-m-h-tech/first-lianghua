...
def test_rolling_proxy_idx_aligned_and_nav_equal_length():
    ...
    proxy = pl.rolling_proxy(
        mat, methods=config.PC_METHODS, lookback=60, rebal=20, shrink=0.1, cap=0.5
    )
    base = proxy["equal"]
    ...
    for m in config.PC_METHODS:
        ...
        assert all(base["idx"][k] < base["idx"][k + 1] for k in range(len(base["idx"]) - 1))
...
def test_gross_net_daily_charge_only_at_segment_start():
    ...
    nf, cf = pl.gross_net_daily(
        [0.01, 0.01], [{"start": 0, "length": 2, "entry_turnover": None}], 1.0, 1e-3
    )
...
def test_dense_matrix_fill_missing_keeps_all():
    rm = {"A": {"d1": 0.01, "d2": 0.0, "d3": -0.01}, "B": {"d1": 0.02, "d3": 0.01}}  # B 缺 d2
