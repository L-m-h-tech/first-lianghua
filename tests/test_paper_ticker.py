...
def make_broker(fill="close", equity0=1_000_000.0, priority="futures_first", db=None, name="基准"):
...
def _full_state(prio="futures_first"):
    ...
    st = _State()
    meta = {"sym": "RB", "code": "RB0", "cat": "黑色", "oil_w": 0, "name": "螺纹钢", "ex": "SHFE"}
    ...
    klines = _MockObj(
        ...
        get=lambda code, cat: (
            {"close": 3000, "prev_close": 3000, "hv20": 0.2, "tech": {}, "ret5": 0.0, "ret20": 0.0},
            True,
        ),
    )
    st.klines = klines
    st.webdata = _MockObj(views_snapshot=lambda: {})
    st.news = _MockObj(score=lambda cat, variety=None: (0.0, 0), trend=lambda: 0.0)
    ...
    st.last_paper = None
    st._paper_stash = {
        "fut_rows": [row("RB", score=_FULL_SCORE)],
        "codes": ["RB0"],
        "strat_rows": [],
        "chain_map": {},
    }
...
def test_analyze_all_varieties_returns_complete_rows():
    ...
    from analyzer import analyze_all_varieties
...
def test_analyze_all_varieties_parity_with_direct():
    ...
    direct = analyze_variety(
        ...
        None,
        None,
        flow=flow_map.get("RB0"),
        fund_raw={},
    )
    assert fut[0]["score"] == direct["score"], "共享函数分数与直接调用不一致：%.4f != %.4f" % (
        fut[0]["score"],
        direct["score"],
    )
