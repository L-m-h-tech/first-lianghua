...
def _margin(**items):
    ...
    return {
        s: {"broker_margin": m, "multiplier": mu, "limit_basic": 0.07}
...
def test_mark_to_market_two_positions():
    pf = _pf(
        equity=100000,
        margin=_margin(RB=(0.10, 10), CU=(0.12, 5)),
        per_symbol=0.20,
...
def test_sizing_by_score():
    pf = _pf(sizing="score", score_weights={"强信号": 0.15, "轻仓": 0.05}, max_symbol_weight=1.0)
...
def test_max_concurrent_cap():
    pf = _pf(
        margin=_margin(AA=(0.1, 10), BB=(0.1, 10), CC=(0.1, 10)),
        max_concurrent=2,
        per_symbol=0.05,
        max_symbol_weight=1.0,
...
def test_sector_weight_cap_binds():
    pf = _pf(
        margin=_margin(RB=(0.1, 10), HC=(0.1, 10)),
        per_symbol=0.4,
...
def _three_sym_pf():
    pf = _pf(
        equity=200000,
        margin=_margin(AA=(0.02, 10), BB=(0.02, 10), CC=(0.02, 10)),
        per_symbol=0.8,
        max_symbol_weight=2.0,
        max_sector_weight=2.0,
        risk_liquidate=0.09,
...
def test_calibrator_changes_lots_only_when_passed():
    base = _pf(
        sizing="score", score_weights={"强信号": 0.30}, max_symbol_weight=2.0, max_sector_weight=2.0
    )
    lots0, _ = base.decide_lots("RB", 1, 1000, score=7.0)
    cal = _pf(
        sizing="score",
        score_weights={"强信号": 0.30},
        max_symbol_weight=2.0,
        max_sector_weight=2.0,
        calibrator=_FakeCalib(0.5),
    )
...
def test_trailing_risk_weights_properties_sum_gross_and_cap():
    ...
    feeds = {
        s: _synth_feed(s, 90, seed=k * 7 + 1)
        for k, s in enumerate(("RB", "CU", "MA", "SA", "I", "TA"))
    }
    for method in ("inv_vol", "erc", "gmv"):  # 第49轮 G5⑤ 补入第四种 gmv 最小方差
        w, meta = pf_mod.trailing_risk_weights(
            feeds, feeds["RB"].bars[80]["dt"], method, window=126, min_hist=30, cap=0.25, gross=1.0
        )
    ...
    w, _ = pf_mod.trailing_risk_weights(
        feeds, feeds["RB"].bars[80]["dt"], "erc", min_hist=30, gross=1.5
    )
    ...
    w, meta = pf_mod.trailing_risk_weights(
        feeds, feeds["RB"].bars[80]["dt"], "inv_vol", min_hist=30
    )
...
# ---- 第145轮 #11：滚动绩效（_roll_sharpe / _roll_maxdd） ----


def _DummyPortfolio():
    """构建最小真实 Portfolio 实例，塞入合成 curve 用于测滚动指标。"""
    ...
    pf.curve = []
    for i in range(120):
        r = 0.001 if i < 80 else -0.003
        eq *= 1 + r
        pf.curve.append(
            {
                "dt": _dt.datetime(2026, 1, 1) + _dt.timedelta(days=i),
                "equity": eq,
                "drawdown": 0.0,
                "risk": 0.05,
                "npos": 1,
...
def test_roll_sharpe_known_values():
    """固定序列验证滚动夏普数值。"""
    from portfolio import Portfolio

    # 全正收益 → 夏普应 > 0；样本不足窗口 → None；常数序列 → 0
    pos = [0.001] * 50
    neg = [-0.002] * 30
    flat = [0.0] * 10
    rets = pos + neg + flat
    s_full = Portfolio._roll_sharpe(rets, 50)
    s_short = Portfolio._roll_sharpe(rets, 200)
    s_const = Portfolio._roll_sharpe([0.005] * 30, 30)
    s_empty = Portfolio._roll_sharpe([], 20)
    assert s_full is not None and s_full < 0  # 后半段负多
    assert s_short is None
    assert s_const == 0.0  # 无波动
    assert s_empty is None


def test_roll_maxdd_known_values():
    """固定序列验证滚动回撤数值。"""
    from portfolio import Portfolio

    rets = [0.005] * 20 + [-0.01] * 10 + [-0.003] * 10
    m20 = Portfolio._roll_maxdd(rets, 20)
    m10 = Portfolio._roll_maxdd(rets, 10)
    m_full = Portfolio._roll_maxdd(rets, 40)
    assert m20 is not None and m20 > 0
    assert m10 is not None and m10 > 0
    assert m_full is not None
    # 全正序列无回撤
    assert Portfolio._roll_maxdd([0.001] * 30, 30) == 0.0


def test_performance_has_rolling_keys():
    """performance() 返回 dict 包含全部滚动指标键。"""
    pf = _DummyPortfolio()
    perf = pf.performance()
    for key in ("roll_sharpe_20", "roll_sharpe_60", "roll_maxdd_20", "roll_maxdd_60"):
        assert key in perf, f"missing key {key}"


def test_roll_sharpe_structure_in_report():
    """build_report 输出包含策略衰减告警段。"""
    ...
    pf = _DummyPortfolio()
    perf = pf.performance()
    report = build_report(pf, perf, _FakeArgs(), {}, [], (1, 100))
    assert "【策略衰减告警】" in report
    assert "滚动夏普（年化）" in report
    assert "滚动最大回撤" in report
