...
CU = {
    ...
    "bid_vol": 4.0,
    "ask_vol": 2.0,
    "open_interest": 211001.0,
    "volume": 26154.0,
    "prev_settle": 109110.0,
...
def test_collect_stores_valid_and_filters_invalid(tmp_db, monkeypatch, flat_calendar):
    ...
    res = obs.collect_once(
        tmp_db, now=_now(), force=True, fetcher=_fetcher({"RB0": RB, "CU0": quotes["CU0"]})
    )
...
def test_ask_lt_bid_filtered(tmp_db, monkeypatch, flat_calendar):
    ...
    res = obs.collect_once(tmp_db, now=_now(), force=True, fetcher=_fetcher({"RB0": bad}))
    assert res["stored"] == 0
...
def test_throttle_skips_within_interval(tmp_db, monkeypatch, flat_calendar):
    ...
    obs.collect_once(tmp_db, now=_now(), force=True, fetcher=_fetcher({"RB0": RB}))
    res = obs.collect_once(
        tmp_db, now=_now() + timedelta(seconds=60), fetcher=_fetcher({"RB0": RB})
    )
    assert res["skipped"] == "throttle" and res["stored"] == 0
    # 超过节流窗口 → 允许采集
    res2 = obs.collect_once(
        tmp_db, now=_now() + timedelta(seconds=301), fetcher=_fetcher({"RB0": RB})
    )
...
def test_disabled_and_no_pilots(tmp_db, monkeypatch, flat_calendar):
    ...
    assert (
        obs.collect_once(tmp_db, now=_now(), force=True, fetcher=_fetcher({"RB0": RB}))["skipped"]
        == "disabled"
    )
