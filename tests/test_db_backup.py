...
@pytest.mark.parametrize(
    ...
    [
        "random.db",
        "monitor_bad.db",
        "monitor_20260903-164500.db.json",
        "monitor_20261301-000000.db",
        "",
        "xmonitor_20260903-164500.db",
    ],
)
...
def test_list_only_recognizes_tool_files(tmp_path):
    for n in [
        "monitor_20260903-100000.db",
        "monitor_20260903-110000.db",
        "notes.txt",
        "other.db",
        "monitor_20260903-100000.db.json",
    ]:
...
def test_restore_rejects_broken_backup(tmp_path):
    src = str(tmp_path / "monitor.db")
    _make_db(src, 1)
    bad = str(tmp_path / "broken.db")
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "paper_x_badstamp.db",
        "_20260912-150100.db",
        "paper_x_20260912-150100.db.json",
        "",
        "random_20260912-150100.db",
    ],
)
...
def test_list_and_prune_paper_grouped(tmp_path):
    ...
    pdir.mkdir()
    for name in [
        "paper_a_20260910-090000.db",
        "paper_a_20260911-090000.db",
        "paper_b_20260911-090000.db",
        "stray.txt",
    ]:
