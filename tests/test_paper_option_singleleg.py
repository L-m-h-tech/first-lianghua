...
MARGIN = {
    "M": {"broker_margin": 0.07, "limit_basic": 0.06, "multiplier": 10},
...
def _chain_map(sym="M", strike=2900.0, ask=30.0, bid=29.0):
    return {
        (sym, 26, 11): {
            "calls": [{"strike": strike, "ask": ask, "bid": bid, "last": (ask + bid) / 2}],
            "puts": [],
        }
    }
...
def _ao_row(sym="M", all_pass=True, kind="call", K=2900.0, prem=30.0, days=60):
    return {
        ...
        "mm": 11,
        "month_label": "2611",
        "opt_code": "%s2611-C-%s" % (sym.lower(), int(K)),
        "prem": prem,
        "score": 6.0,
        "days": days,
        "all_pass": all_pass,
...
def _broker(
    ...
    return PaperBroker(
        ...
        opt_premium_ratio=opt_premium_ratio,
...
def test_option_only_tier_opens_single_leg_from_analyze_option():
    ...
    br = _broker()
    s = br.on_cycle_options(
        "2026-09-10 09:30:00", [], _chain_map(), _fut_rows(), opt_rows=[_ao_row()]
    )
    assert s["n_buy"] == 1
...
def test_option_only_skips_non_pass_single_leg():
    ...
    br = _broker()
    s = br.on_cycle_options(
        "2026-09-10 09:30:00", [], _chain_map(), _fut_rows(), opt_rows=[_ao_row(all_pass=False)]
    )
...
def test_option_only_skips_missing_chain_leg():
    ...
    s = br.on_cycle_options("2026-09-10 09:30:00", [], cm, _fut_rows(), opt_rows=[_ao_row()])
...
def test_option_only_skips_already_held_same_sym():
    ...
    br = _broker()
    s1 = br.on_cycle_options(
        "2026-09-10 09:30:00", [], _chain_map(), _fut_rows(), opt_rows=[_ao_row()]
    )
    assert s1["n_buy"] == 1
    s2 = br.on_cycle_options(
        "2026-09-10 09:31:00", [], _chain_map(), _fut_rows(), opt_rows=[_ao_row()]
    )
    assert s2["n_buy"] == 0
    assert len(br.opt_positions) == 1
