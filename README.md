...
14. **组合层风险（`portfolio_risk.py` + `tools/portfolio_risk_lab.py`，第47轮 G5①②③，研究侧只读）**：相关矩阵（平均绝对相关/板块块/最强最弱对）、组合 VaR 双口径（历史模拟法 VaR+ES 含真实肥尾、参数法 z·σ 与 √h 多日）、分散化收益，以及原油 SC 经各品种 OLS beta 的 ±5%/−10% 线性压力；对 equal/inv_vol/erc/gmv 同篮子对照。纯标准库、只读 G21 面板、不接 main、**不改 risk_gate 动作/综合分/既有 sizing**；G5 的熔断动作、第四种风险平价 sizing、组合历史净值回测留后续。
