...
### 近期轮次关键能力（第81-99轮，2026-09）

- **第99轮 研究侧可选全部**：G13续 LLM复核历史观察工具（145条历史/8条可评估/准确率37.5%诚实呈现）；B5扩展OpenVlab 10品种批量IV校验；B4确认100ppi数据页JS动态渲染→诚实降级；G25续因子体检管线就绪；代码审查保留。
...
| 参数 | 作用 |
| --- | --- |
| （无） | 常驻运行：新闻60s/原油10s；交易时段前30分钟每5分钟一轮、之后每10分钟一轮，非交易时段1分钟一轮 |
| `--once` | 只跑一轮分析后退出（测试用） |
...
D:\Python\python.exe portfolio.py --all --period 30 --risk-sizing erc          # ERC风险平价定目标名义(另可选 inv_vol/gmv最小方差)
...
3. **独立风控闸门**（`risk_gate.py`，与打分解耦的第二道防线，逐品种给 pass/warn/veto）：
   ...
   - **组合层单日浮亏熔断**（`circuit_breaker.py`，第48轮 G5④，与上面逐品种 veto 正交的**账户级**断路器）：盯"当日日初权益→当前权益"浮亏，warn≥`CIRCUIT_WARN_LOSS`(2%)只提示降杠杆、halt≥`CIRCUIT_HALT_LOSS`(3%)停开新仓（只平不开）、delever≥`CIRCUIT_DELEVER_LOSS`(5%)加"建议减仓约50%"文字（observe/paper_halt 下仅建议不自动砍仓；第三种模式 paper_delever 下按比例自动减仓，见下），保证金风险度≥`CIRCUIT_RISK_HALT`(95%)作为第二触发源抬到 halt；**当日粘性**（按当日峰值浮亏定档、触发后不因浮亏回落而解锁，防阈值附近抖动）、跨交易日自动重置并重计日初基准。**默认 `CIRCUIT_ACTION='observe'` 只计算/标注、PaperBroker 不挂断路器（self.breaker=None）、纸面成交逐字节等价旧版**；显式置 `paper_halt` 才在 on_cycle 阶段B过滤开新仓腿（open/reverse_open 剔除、close/reverse_close 保留，反手只平不反向开）；显式置 `paper_delever` 则在停开之外，**进入 delever 档时于阶段A挂单成交后、阶段B信号前自动减仓**——按 `CIRCUIT_DELEVER_RATIO`(50%) 逐品种向下取整（`reduce_lots_of`，不足1手不减、只减不清、只平不反向），账户内核 `Portfolio.close(reduce_lots=)` 做部分平仓并保留剩余持仓，当日各品种只减一次、跨交易日重置，锁板/无价顺延不登记下轮重试。断路器在阶段D快照后用最新权益更新、供**下一**轮使用（自动减仓因此严格晚一轮、无未来函数），真实账户永不自动操作、PAPER_ENABLED 默认 False。自测 `D:\Python\python.exe circuit_breaker.py`（15组）。
...
11. **研究侧一键复盘编排器（`tools/research_review.py`，第43轮 G30③，研究侧收盘后跑一次）**：不重跑任何工具，只把各研究工具**已落盘**的 sidecar（factor_health/factor_regime/attribution/trade_journal/portfolio_lab/backtest_validation 的 .json）+ portfolio_equity.csv + 主链 signal_tracking.txt 聚合成一份七段简报（数据源新鲜度总表→信号命中→G29因子→G28归因→G30①复盘→G3/G5组合风险→WP-F4防过拟合）与**规则化待办清单（WARN/INFO/OK 排序）**，秒级出 reports/research_review.txt+.json。任一产物缺失/损坏/陈旧（默认168h）安全降级并给出刷新命令；utf-8-sig 兼容 BOM、空目录也能成稿。**命名隔离**：主链已有实时轮动复盘 reports/daily_review.txt（main 产出、铁律不动），研究侧故命名 research_review。待办只提示、不自动改参。
...
14. **组合层风险（`portfolio_risk.py` + `tools/portfolio_risk_lab.py`，第47轮 G5①②③，研究侧只读）**：相关矩阵（平均绝对相关/板块块/最强最弱对）、组合 VaR 双口径（历史模拟法 VaR+ES 含真实肥尾、参数法 z·σ 与 √h 多日）、分散化收益，以及原油 SC 经各品种 OLS beta 的 ±5%/−10% 线性压力；对 equal/inv_vol/erc/gmv 同篮子对照。纯标准库、只读 G21 面板、不接 main、**不改 risk_gate 动作/综合分/既有 sizing**；④单日浮亏熔断第48轮落地（circuit_breaker）、⑤第四种 gmv sizing 接入与组合历史净值曲线第49轮落地（portfolio_nav.csv），**G5 五项至此全部完成**。第50轮 G5④ 补**熔断阈值历史校准**（`tools/circuit_review.py`，日频逐日代理只读面板）：真实61品种378日里默认3%halt/5%delever零触发、最差单日仅-2.15%（阈值为日内/杠杆账户设计、日频分散组合难触发），1%观察档仅次日有微弱续跌惯性、3日后均值回归，故维持默认阈值不改、只给校准结论。
...
**轮动节奏**：交易时段为 **9:00-11:30 / 13:30-15:00 / 21:00-次日02:30（不同品种按23:00、01:00、02:30分档收市）**；每个时段**开盘前30分钟每5分钟一轮**（对齐 5 分钟刻度，如 9:00、9:05…9:25），**之后每10分钟一轮**（对齐 10 分钟刻度，如 9:30、9:40、9:50…）；非交易时段每1分钟一轮。每份报告的块头都写明本轮时间、当前节奏与下一轮计划时间。**每一轮新报告都写在文件最前面**（最新在最上）。
