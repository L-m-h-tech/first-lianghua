...
3. **独立风控闸门**（`risk_gate.py`，与打分解耦的第二道防线，逐品种给 pass/warn/veto）：
   ...
   - **组合层单日浮亏熔断**（`circuit_breaker.py`，第48轮 G5④，与上面逐品种 veto 正交的**账户级**断路器）：盯"当日日初权益→当前权益"浮亏，warn≥`CIRCUIT_WARN_LOSS`(2%)只提示降杠杆、halt≥`CIRCUIT_HALT_LOSS`(3%)停开新仓（只平不开）、delever≥`CIRCUIT_DELEVER_LOSS`(5%)加"建议减仓约50%"文字（**仅建议、不自动砍仓**），保证金风险度≥`CIRCUIT_RISK_HALT`(95%)作为第二触发源抬到 halt；**当日粘性**（按当日峰值浮亏定档、触发后不因浮亏回落而解锁，防阈值附近抖动）、跨交易日自动重置并重计日初基准。**默认 `CIRCUIT_ACTION='observe'` 只计算/标注、PaperBroker 不挂断路器（self.breaker=None）、纸面成交逐字节等价旧版**；显式置 `paper_halt` 才在 on_cycle 阶段B过滤开新仓腿（open/reverse_open 剔除、close/reverse_close 保留，反手只平不反向开），断路器在阶段D快照后用最新权益更新、供下一轮使用（无未来函数），真实账户永不自动操作。自测 `D:\Python\python.exe circuit_breaker.py`（11组）。
