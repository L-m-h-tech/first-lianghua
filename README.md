...
## 七.6、纸面交易引擎 PaperBroker（第27轮 G1，补订单执行层塌陷；默认休眠）
...
- **总开关 PAPER_ENABLED 默认 False**：本轮只交付表+撮合，未接入 main、不动实时主链与综合分口径；第28轮再接 main、reports/paper_account.txt 与看板净值页签。纸面影子永不自动接实盘（实盘门槛见融合总纲 G20）。
