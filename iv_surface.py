...
def _strike_iv_map(legs, F, T, kind):
    ...
    for leg in legs:
        ...
        if iv is None or iv > config.IV_SURFACE_IV_CAP:  # 错价/陈旧价反推上天 -> 丢弃
            continue
        out[K] = {"iv": iv, "oi": float(leg.get("oi") or 0.0), "quality": quality, "spread": spr}
    return out
...
def _merge_strike(c, p):
    ...
    if c and p:
        if abs(c["iv"] - p["iv"]) <= config.IV_PARITY_DIFF:
            ...
            iv = sum(v * (w if w > 0 else 1.0) for v, w in ws) / sum(
                (w if w > 0 else 1.0) for _, w in ws
            )
...
def expiry_smile(chain, F, days):
        ...
        points.append(
            {
                ...
                "iv": iv_avg,
                "quality": quality,
                "call_iv": c["iv"] if c else None,
                "put_iv": p["iv"] if p else None,
                "call_delta": gc["delta"],
                "put_delta": gp["delta"],
    ...
    c25_iv = c25["call_iv"] if c25 and c25["call_iv"] is not None else c25["iv"] if c25 else None
    p25_iv = p25["put_iv"] if p25 and p25["put_iv"] is not None else p25["iv"] if p25 else None
    ...
    return {
        ...
        "yy": int(chain.get("yy") or 0),
        "mm": int(chain.get("mm") or 0),
        "days": int(days),
        "T": T,
        "points": points,
        "atm_strike": atm_pt["strike"],
        "atm_iv": atm_pt["iv"],
        "c25_iv": c25_iv,
        "p25_iv": p25_iv,
        "rr25": rr25,
...
def surface_text(exps, term_diff, term_shape, matrix_rows):
    ...
    if e0["rr25"] is not None:
        ...
        bits.append(
            f"{e0['label']} 25Δ风险反转{e0['rr25'] * 100:+.1f}vol({skew_dir})"
            f"、蝶式{e0['fly25'] * 100:+.1f}vol"
        )
    n_bad = sum(1 for e in exps for p in e["points"] if p["parity_warn"])
    if n_bad:
        bits.append(
            f"{n_bad}档call/put反推偏差>{config.IV_PARITY_DIFF * 100:.0f}vol(已取窄价差可信侧)"
        )
