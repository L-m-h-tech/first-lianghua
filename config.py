...
SHADOW_FOLLOW_HOUR = (
    17  # 第85轮：影子信号每日跟随的补跑时刻（17:00 后补当日收盘信号；首次周期即跑）
)
...
NO_NIGHT_SYMS = {"JD", "LH", "LG", "AP", "CJ", "PK", "SF", "SM", "UR", "SI", "LC", "PS", "EC"}
...
ALERT_WEBHOOK_URL = os.environ.get(
    "FUTURES_MONITOR_WEBHOOK", ""
)  # 飞书/钉钉/企业微信/Server酱/通用JSON
...
HTTP_THROTTLE_ENABLED = (
    True  # A5 请求级限流退避总开关（仅 429/503/Retry-After 时改变时序，正常路径零影响）
)
...
OPT_DELTA_BAND = (
    0.30,
    0.45,
)  # 建议买入合约的 |Delta| 区间（第102轮：tastytrade 买方最优区间，从(0.35,0.60)缩窄，delta>0.45 theta 损耗大）
...
# G17（第98轮）：期权精确保证金待真实账户数据（银河柜台参数，当前用静态表兜底）
# 成交量PCR：交易所日行情数据源缺口（上期所AJAX渲染无法纯requests获取）→诚实标注暂无
...
IV_SURFACE_MIN_DAYS = (
    30  # 剩余天数低于该值的月份不参与反推（最后30天时间价值小、报价误差被放大；按真实到期日过滤）
)
...
IV_OUTLIER_RATIO = (
    0.2,
    3.5,
)  # 微笑点IV相对ATM合理倍数带[0.2x,3.5x]，带外视为离群(深虚值小价格误差)，不参与矩阵/25Δ
...
MINUTE_ONCE_LMT = {
    ...
    30: 300,
    60: 300,
}  # --once同步小回填条数（新浪秒回，1m可全量）
MINUTE_WORKERS = (
    3  # 并发线程数（2026-09-11 第120轮：6→3，配合 SINA_REQ_GAP 全局限流避免瞬时并发触发456）
)
...
FUTURES_FEES_FILE = os.path.join(
    DATA_DIR, "futures_fees.csv"
)  # 用户券商手续费表转换出的64品种真实费率
...
PORTFOLIO_SIZING = (
    "equal_notional"  # 手数：equal_notional等名义/equal_risk等风险(ATR)/score按综合分档
)
...
PC_METHODS = (
    ...
    "erc",
    "gmv",
)  # 等权/逆波动/风险平价ERC/最小方差GMV；默认 equal=旧等名义口径
...
PRS_METHOD = (
    "erc"  # 接入方法：inv_vol 逆波动 / erc 风险平价（GMV 第40轮实证过集中、有效N仅11，不接入）
)
PRS_WINDOW = (
    PC_LOOKBACK  # 估协方差的历史bar数（日线=交易日；分钟=bar根数；只用当前bar之前=严格PIT）
)
...
# ---- G2（第60轮·最后一切片；第61轮默认翻 True）综合分 live part 是否改由 factor_parts 注册表装配 ----
# 第61轮起默认 True=analyzer 在内联算完后用 factor_parts 注册表重建 parts（9 part 全部逐位 parity、
# off/on 双哈希都=长期基线，等价证明见 factor_parts.selftest）；内联路径仍完整保留，置 False 即可即时回退。
PLUGIN_PARTS_ENABLED = True
...
REPORT_AUTO_OPEN = (
    True  # 首轮真实报告生成后是否自动用默认浏览器打开实时报告HTML（命令行 --no-launch 可临时关闭）
)
...
RISK_GATE_AUTO_DOWNGRADE = (
    False  # True=veto 时自动把信号降级为观望；False(默认)=只标注+告警，不改综合分/建议
)
RISK_GATE_MIN_VOLUME = 200  # 成交量(手)低于此视为流动性不足（price 缺失同样 veto）
RISK_GATE_DIVERGE_CHG = (
    0.02  # 强信号(|分|>=SCORE_MID)但当日涨跌幅反向超过 2% -> veto（防追高/摸顶）
)
...
CIRCUIT_HALT_LOSS = (
    0.03  # 单日浮亏≥3% -> halt（paper_halt/paper_delever 停开新仓、当日粘性、日切解除）
)
...
CIRCUIT_DELEVER_RATIO = (
    0.50  # delever 档减仓比例（paper_delever 下每品种当日按此向下取整减一次，不足1手不减）
)
...
FACTOR_EVAL_JSON = os.path.join(
    BASE_DIR, "reports", "factor_eval.json"
)  # P1-3 图表用结构化 sidecar
...
BACKTEST_OOS_RATIO = (
    0.0  # 样本外占比：0=关闭(旧口径)；如0.3=按时间排序后30%交易为OOS、与前70%IS并列对照
)
BACKTEST_VALIDATION_JSON = os.path.join(
    BASE_DIR, "reports", "backtest_validation.json"
)  # DSR/PBO sidecar(研究工具产出)
...
PAPER_EXIT_SCORE = (
    2.0  # 持仓后 |综合分|<该值（回到中性带）则平仓离场（默认=SCORE_NEUTRAL，迟滞防抖）
)
...
PAPER_ACCOUNT_DB_DIR = os.path.join(
    BASE_DIR, "data", "paper_accounts"
)  # 第102轮：多账户独立SQLite目录
PAPER_OPT_PREMIUM_MAX_RATIO = (
    0.03  # 第102轮：单笔买方权利金占权益上限3%（tastytrade共识：买方≤3-5%总资金）
)
PAPER_OPT_STOP_LOSS_RATIO = (
    0.50  # 第102轮：权利金止损线（亏损≥50%权利金即平仓，激进版可在实例层覆盖为0.40）
)

# ========== 第116轮：备用数据源（backup_sources.py 可选加载） ==========
# 主链（新浪/东财）封锁时的降级源；全部可选，依赖缺失/无账户零请求，不改变主链行为。
BACKUP_VENDOR_DIR = os.path.normpath(
...
PAPER_TICK_TRADING_ONLY = (
    True  # True=仅交易时段（含夜盘）ticker 生效；非交易 run_cycle 已每分钟且行情冻结
)
...
PAPER_UNIFIED_POOL_ANNOTATION = (
    "自2026-09-09(第104轮)起期权与期货统一资金池（权利金同扣、盈亏同计）"
)

# ========== 第105轮：纸面基准 & 对比看板重构（5档分组 + 下钻 + 二级详情页） ==========
PAPER_DETAIL_ENABLED = (
    True  # True=每轮生成 paper_detail_{name}.html 静态详情页（供对比页点账户名下钻）
)
...
# 保证金表已替换为交易所标准档（三立期货 wj.xcqihuo.cn API，山西三立期货；手续费每手+0.01元，
# 比例型同样每手+0.01，第147轮确认口径；fees 乘数=报价口径与 margins 一致，JD=10 等7品种修正），
# per_symbol 按新表最低品种（玉米C：现价~2265×0.07×10≈1587元/手）逐档核算，确保目标≥1手。
# risk_liquidate/risk_safe 无需上调（新表保证金率低，风险度远低于强平线）。
#
# 档位核算（第147轮：新三立表+20260911现价，最低一手保证金=玉米C 1587元）：
#   1万_激进：0.35×10000=3500 → 3500/1587≈2.21手 ✓（不改）
#   1万_基准：0.25×10000=2500 → 2500/1587≈1.58手 ✓（不改）
#   1万_保守：0.16×10000=1600 → 1600/1587≈1.01手 ✓（不改）
#   5000_激进/赌徒：0.40/0.45×5000=2000/2250 → 1.26/1.42手 ✓（不改）
#   5000_基准/保守：0.32×5000=1600 → 1600/1587≈1.01手 ✓（不改）
#   3000_激进/赌徒：0.54/0.59×3000=1620/1770 → 1.02/1.12手 ✓（不改）
#   3000_基准/保守：futures_max=0（纯期权），不需调期货 per_symbol。
#   10万全档：保证金口径下仍然可开多手（1.89~22.06手），per_symbol 不改。
#   1000全档：option_only（futures_max=0），期货永远不可开，走期权路径。
#   1000_赌徒：option_first（期货入口保留但预算 550<1587 永远开不了，行为=纯期权，设计预期）。
...
PAPER_ACCOUNTS = [
    ...
    # 基准/保守 per_symbol 降为轻仓水平（margin口径下刚好1-3手），避免满杠杆。
    {
        ...
        "risk_liquidate": 1.00,
        "risk_safe": 0.90,
        "opt_premium_ratio": 0.80,
        "stop_loss_ratio": 0.40,
    ...
    },
    {
        ...
        "risk_liquidate": 1.00,
        "risk_safe": 0.90,
        "opt_premium_ratio": 0.60,
        "stop_loss_ratio": 0.50,
        "priority": "futures_first",
        "target_basis": "margin",
        "max_daily_orders": 60,
        "max_active_per_sym": 6,
    },
    ...
    },
    {
        ...
        "risk_liquidate": 1.00,
        "risk_safe": 0.90,
        "opt_premium_ratio": 0.75,
        "stop_loss_ratio": 0.70,
...
TSMOM_LOOKBACKS = (
    ...
    126,
    252,
)  # 多窗口回看交易日（约1/3/6个月）；现网仅 ret5/ret20 短窗，补中长期趋势
...
TSMOM_FORECAST_HORIZONS = (
    5,
    20,
    60,
)  # 预测未来收益的持有交易日（约1周/1月/1季），用于看预测力随期限衰减
...
REGIME_VOL_LOOKBACK = (
    120  # 波动率 regime 用过去120日 ts_rank（只用过去、PIT；纯Py滚动秩，窗不宜过大）
)
...
from config_loader import apply_overrides as _apply_overrides
from config_loader import load_config_file as _load_config_file
...
CONFIG_OVERRIDE_REPORT = (
    _apply_overrides(globals(), _cfg_obj)
