...
- **paper_account.txt 三张明细表新增"合约"列**：当前持仓 / 在途挂单 / 最近成交均显示具体合约（如 RB2610），实时报告紧凑块与看板页签不受影响（本就只显示汇总计数）。
- **轮动节奏（用户拍板）**：`SESSION_INTERVAL` 1200→**600**（交易时段开盘后前 30 分钟仍每 5 分钟，30 分钟之后从每 20 分钟改为每 10 分钟）；主循环与纸面撮合同节奏刷新（utils.cycle_interval/next_cycle_time/rotation_desc 文案同步，报告块头显示"每10分钟轮动"）。
- **零评分/成交口径改动**：只加字段透传与节奏参数，综合分、撮合价、费率、滑点逻辑逐字节不变；合约字段缺失（历史行/测试行）安全回退空串，默认行为等价。
- 测试：test_paper_broker +2（合约透传开平仓/restore 保留）、test_paper_report +1（账户文本合约列）、test_utils_schedule 节奏断言 1200→600；**pytest 783→786 全绿**；端到端验证（临时库）委托/成交/持仓/平仓均带 RB2610。
...
- **[补丁]** 同花顺期货通路径随用户 C→E 盘迁移更新（config.THS_EXE → E:\同花顺期货通in\happ.exe，真实启动验证就绪）；总纲状态表一次性校正 9 项滞后行（G9/G10/G11/G6/G3/G12/G2/G8/G26 均标 ✅+优先级划线），README 增"近期轮次关键能力（第81-91轮）"小节。
...
- **行为边界**：纸面是 main 进程内组件（跟随启停），成交只发生在 main 运行时段；综合分/信号/建议零改动（G1 独立成段铁律，双哈希无需重跑）；回退=改回 False 即完全休眠（三表历史保留）。
- 看板"纸面账户(影子)"页签现可见；G1 三方对账前提（连续影子≥4周）自今日起开始积累。
...
- **[补丁] 成本节流**：每日上限（env FUTURES_MONITOR_LLM_MAX_PER_DAY，默认3）+ top品种同日去重——非交易时段每60秒轮询、强信号持续时修复前理论可达每小时数十次调用；tests 增 cap/dedup/跨日清零/零请求4断言，756→758 全绿。
- **[key 屏蔽核验]**：全库 167 个被跟踪文件扫描（key 前缀/后缀/中段）**零命中**；.env 未被跟踪；.env.example 的 G13 段改为明确占位说明"此处添加用户自己的llm key"。
...
- **真实验证（首跑）**：三信号已记录 2026-09-02（各12腿/60品种截面），0 个到期（影子从今日起算，约1个月后第一个信号到期）——诚实呈现"未到期"。当日快照：xsmom252 多腿 FU/LU/EC/AG/LC/BU…，tsmom252 多腿 FU/BU/CY/EC/LU/CF…。
- Runbook 新增 §3.4（命令/信号登记表/启动日守卫/排期）+ 任务模板 backup/futures_monitor_shadow_task.xml（每日18:15，**只生成不自动注册**）。
- selftest 5组（启动日守卫防回填/信号记录幂等/到期评估/合成强信号|IC|≈1/快照渲染）；pytest 748 全绿；研究侧零改动主链（隔离 grep 合规）。

## [0.82.0] — 2026-09-05 · 第82轮 长面板消费端+xsmom 8.5年翻转（双样本稳健✅） + tsmom252 blend OOS + 时段性异象正式关闭
- **G21续 长面板消费端扩展**：xsmom_eval 加 `--panel-db`（透传 load_adjusted_bars 的 db_path）；long_panel_builder 特征列现算（day_chg/ret5/ret20/ret63/ret126/ret252/hv20/hv60/tsmom63/126/252 全部 LIBRARY 表达式对近月复权收盘现算，selftest 5组）。长面板重建后 109,803 行、tsmom252 非空 93,675。
- **⚠️ 重大发现：xsmom 8.5年长窗复核翻转"动量证伪"**：`xsmom_eval --panel --panel-db research_panel_long.db --days 2150`——**全市场·多空基线 近4.1年 t+1.87 / 长8.5年 t+2.29（净均+1.15%/期、夏普0.87-0.93）✅双样本稳健；剔除能化·多空 t+2.88/+2.99、夏普1.14-1.43 ✅稳健**；主组合通过全部『确定不更差』判据。第32轮"动量证伪"确认为**口径/时段条件的**（近月拼接长序列 vs 主连复权4年样本）——与低波异象第80轮降级同款教训：任何证伪/成立结论都必须标注口径与样本窗。
- **G25/G29续① tsmom252 blend OOS**：长窗（2018起）正交合成含 tsmom252 后 IC 0.057→0.074（ICIR 0.27→0.36）、对齐净年化 4.60%→8.45%/夏普0.56→1.04；**但 tsmom252 单因子 OOS IC +0.0801/ICIR 0.396 为全因子（含blend）最强**——升级路径=单因子影子/分层多空（regime_cond_lab 已具备），而非经正交残差化。orthogonal_blend_oos 补 `--out/--json` 参数（对照变体独立落盘不覆盖基准）。
- **G25续② 时段性异象正式关闭（PIT 可识别性检验）**：低波族年度IC 符号翻转（2021/22 正 vs 2024-26 负）的**翻转前一年 IC 均接近0**，历史IC序列无法事前识别切换；年度样本仅8个——**时段性异象不可交易**，"前一年市场趋势方向"切换器假设登记为未来方向（需更长样本）。
- selftest：long_panel_builder 5组（特征列现算+落库回读闭环）；xsmom_eval selftest 15组全过；pytest 748 全绿；研究侧零改动主链（隔离 grep 合规）。
...
- **真实验证（64品种/长窗2018-01~2026-09/99期非重叠，--placebo-seeds 8）——⚠️诚实降级**：
  1) **复合低波族全样本截面IC 长窗≈0（+0.004）**，与 4 年面板口径的 -0.094 矛盾；且同期段（2022-05 起）在 term 口径也只有 -0.008——**"低波异象"对价格构建口径与时段双重敏感，属时段性现象而非稳健异象**；
  2) 长窗低波条件化有微弱真效应：真实低波 IC -0.051 略超 8 种子 placebo 下界（-0.044），但 ≈0.3%/年**无交易价值**；
  3) H 网格长窗全样本 IC 全部≈0（-0.006~+0.004）。
- **catalog 研究卡诚实降级**：expr_range_pct5/20、expr_hv20、composite_lowvol_family 四张卡 verdict 从"健康(反向)/随族观察"统一降级为"**时段性（口径/时段敏感，不构成稳健异象）**"并补长窗复检证据——第78-79轮的条件化结论在长样本上不成立。validate()=[]。
- selftest：term_history 9→10组；pytest 746 全绿；研究侧零改动主链（隔离 grep 合规）。

## [0.79.0] — 2026-09-05 · 第79轮 多种子placebo常设化 + 复合低波族4种子正式验证（边缘通过） + catalog 研究卡刷新
- **regime_cond_lab 加 `--placebo-seeds`（第79轮，多种子 placebo 常设化）**：robust_chain 的 placebo 从单种子升级为 K 种子（默认1兼容），报告 min/median/max 分布（IC与净年化）——第78轮复合因子的4种子人工验证流程入库，防"单种子恰好偏弱→高估结构强度"。
- **真实验证（复合低波族 --compose --placebo-seeds 4）**：真实低波 IC **-0.140**/净年化-13.64%，placebo 分布 IC min-0.118/median-0.097/max-0.057、年化 min-12.50%/median-4.64%/max+4.47%——**真实值略出分布上界=增益边缘通过**（比第78轮单种子-0.057的观感弱得多：单种子会高估结构强度）。维持"随族观察"不进综合分。
- **catalog 研究卡刷新**：expr_tsmom252 判定更新为"待复核（条件化不成立）"（补第78轮非重叠t+2.0与placebo未过证据）；新增 composite_lowvol_family 卡（hv20+range_pct5/20 等权秩平均，placebo 边缘通过、样本41期、随族观察）。validate()=[]。
- selftest：regime_cond_lab 7组（多种子断言：placebos 列表/种子数/min/median/max 齐全）；pytest 746 全绿；研究侧零改动主链。

## [0.78.0] — 2026-09-05 · 第78轮 tsmom252 复核（非重叠t仅+2.0、低波条件化未过placebo） + 低波异象族复合因子（placebo通过） + top-up 纳入调度
- **G25/G29续① tsmom252 截面正IC 复核（regime_cond_lab --expr --robust，按第76轮教训先 placebo）**：全样本截面 IC 跨H稳定（H5/10/20/40 = +0.034/+0.050/+0.070/+0.113）但**非重叠调仓口径 t 仅 +2.0**（expr_research 日频重叠口径 t+11.0 被慢信号自相关虚高）；**placebo 伪低波 +4.88%/IC+0.065 ≈ 全样本 +4.69%/+0.070，真实低波 +8.84%/+0.071 仅略高**——第39轮"长周期动量低波增强"的条件化部分大部分为分桶假象。catalog 卡维持"待复核"并补充降级表述（方向稳定≠可交易结构）。
- **G25续② 低波异象族复合截面因子（regime_cond_lab 新增 `--compose`）**：';'分隔多条 'EXPR[:名称]'，逐日对各成员 cs_uniform 秩等权平均（成对剔除缺失品种）。成员=hv20/range_pct5/range_pct20（负IC族共识）。**真实验证（64品种/996日/H20，修正后口径）**：复合全样本截面IC **-0.094** 强于任何单成员（0.058~0.068）且跨H（-0.028~-0.094）跨子期（-0.077/-0.097）稳定；**placebo 通过**——真实低波 IC **-0.140**（净年化 -13.74%，反向即 +13.7%/年）系统性远离 4 种子 placebo 分布（IC -0.042~-0.110，均值≈-0.071），与第76轮单因子 placebo 假象形成对照：族内复合降噪后 regime 条件化增益显现。维持"研究候选"不进综合分（须更长样本+G29 常驻）。
- **G22续③ top-up 纳入调度**：灾备恢复Runbook 新增 §3.3（top-up 命令/规则/排期建议）+ 任务模板 `backup/futures_monitor_term_topup_task.xml`（每日 18:00 盘后、需网络、**只生成不自动注册**，导入命令 schtasks /Create 或 taskschd.msc 图形导入——不擅自改系统任务）。
- selftest：regime_cond_lab 6→7组（compose 逐日等权秩手算：反向排序共识0/同向极值/缺失成对剔除）；pytest 746 全绿；研究侧零改动主链（隔离 grep 合规）。
