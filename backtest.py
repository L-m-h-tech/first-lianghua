...
def technical_score(ind):
    ...
    score = (
        math.tanh(ind.get("ret5", 0.0) * 160.0) * 2.5
...
def bootstrap_trade_stats(returns, n_boot=1000, seed=20260902, ci=(0.05, 0.95), min_trades=20):
    ...
    return {
        "n": n,
        "n_boot": int(n_boot),
        "cum_p5": quantile_inplace(cums, lo),
        "cum_median": quantile_inplace(cums, 0.5),
        "cum_p95": quantile_inplace(cums, hi),
        "dd_p5": quantile_inplace(dds, lo),
        "dd_median": quantile_inplace(dds, 0.5),
...
def _build_trade(
    ...
    return {
        ...
        "fee_mode": fee_mode,
        "blocked_exits": position.get("blocked_exits", 0),
        "exit": exit_reason,
    }
...
def simulate_prepared(
            ...
            if fill_mode == "close":
                ...
                if should_exit:
                    ...
                    if _locked_limit(prepared, i, exit_dir, limit_move):
                        ...
                        exit_reason = (
                            "反向" if opposite else ("到期" if held >= hold_days else "解锁离场")
                        )
                        trades.append(
                            _build_trade(
                                name,
    ...
    return {
        "name": name,
        "code": code,
        "bars": len(prepared["bars"]),
        "roll_count": prepared["roll_count"],
        "signals": signal_rows,
    ...
    if codes_arg:
        ...
        for x in codes_arg.split(","):
            ...
            if x in config.VARIETIES:
                ...
                items.append(
                    (x, meta["code"])
                )  # meta["code"]已是主连RB0，勿再补0（新浪日K对RB00返回null）
            else:
                tok = (
                    x if not x.isalpha() else x + "0"
                )  # 纯品种字母→主连补0；具体合约(如RB2601)原样
...
def fetch_and_run(item, args):
    ...
    try:
        ...
        if getattr(config, "BACKTEST_SLIP_CALIBRATE", False):
            try:
                ...
                cal = _st.MonitorDB().sym_spread_calibration(
                    days=int(getattr(config, "BACKTEST_SLIP_CALIBRATE_DAYS", 30)), min_samples=60
                )
                ...
                if (
                    _c
                    and _c.get("spread_bp_median") is not None
        ...
        if getattr(args, "walk_forward", False):
            grid = [
                (h, e) for h in config.BACKTEST_STABLE_HOLDS for e in config.BACKTEST_STABLE_ENTRIES
            ]
...
def _fmt_boot_ci(b):
    if not b:
        return (
            "样本不足（净交易<%d笔或已关闭），不做区间估计" % config.BACKTEST_BOOTSTRAP_MIN_TRADES
        )
    return (
        f"累计收益 {b['cum_p5'] * 100:+.1f}%~{b['cum_p95'] * 100:+.1f}%"
...
def _fmt_validation_ref(data):
    ...
    if isinstance(d, dict) and d.get("dsr") is not None:
        parts.append(
            f"组合日收益 DSR={d['dsr']:.2f}（观察SR {d.get('sr_obs', 0):.2f}，"
            f"多重试验阈值SR0 {d.get('sr0', 0):.2f}，{d.get('verdict', '')}）"
        )
    g = data.get("grid")
    if isinstance(g, dict) and g.get("n"):
        parts.append(
            f"分钟参数网格 {g['n']} 品种：PBO<0.2 共{g.get('pbo_good', 0)}、"
...
def _fmt_archive_info(info):
    ...
    return (
        f"回测留档：本次为 backtest_runs 表第 {info['seq']}/{info['total']} 条日线回测记录，"
...
def _fmt_metrics(m):
    ...
    return (
        f"n={m['n']} 胜率{m['win_rate'] * 100:.1f}% 均收{m['avg'] * 100:+.2f}% "
...
def _fmt_g3_extended(returns, hold_days):
    ...
    return (
        "  G3扩展绩效: 盈亏因子PF {pf}  盈亏比 {pr}  最大连胜 {ws}笔/连亏 {ls}笔  "
        "Omega {om}  Ulcer {ul}  Calmar {cal}"
    ).format(
        pf=_s(ts["profit_factor"]),
        pr=_s(ts["payoff_ratio"]),
        ws=ts["max_win_streak"],
        ls=ts["max_loss_streak"],
        om=_s(omega),
        ul=_s(ulcer, "{:.3f}"),
        cal=_s(calmar),
    )
        ...
        out.append(
            {
                ...
                "symbols": len(rows),
                "blocked_entry": v["blocked_entry"],
                "blocked_exit": v["blocked_exit"],
            }
        )
    return out
...
def build_report(results, errors, args):
    ...
    if args.no_cost:
        ...
        cost_txt = (
            f"真实券商手续费表{fee_date}（{real_fee_trades}/{total_trades}笔命中；按金额+按手数，开仓+平仓）"
            f"+滑点单边{args.slip_rate * 100:.3f}%；日线多日持仓不使用平今费率，未命中则回退单边{args.fee_rate * 100:.3f}%"
        )
    limit_txt = (
        "不过滤锁板" if args.no_limit_filter else f"疑似锁板阈值±{args.limit_move * 100:.0f}%"
    )
    fill_txt = (
        "信号根收盘价成交（旧口径，便于纵向比较）"
        if args.fill == "close"
    ...
    impact_txt = (
        "未计冲击成本"
        if args.impact_rate <= 0
        else f"另计单边冲击成本{args.impact_rate * 100:.3f}%（往返两次）"
    )
    L = [
        "=" * 96,
        f" 最小日线技术回测（生成于 {now}）",
    ...
    if ref_txt:
        L.append(
            "防过拟合交叉引用（tools/backtest_validation.py 最近结论，非本次计算）：" + ref_txt
        )
    ...
    if gross_metrics:
        L.append(
            f"  毛: {_fmt_metrics(gross_metrics)}｜成本拖累 均收{(net_metrics['avg'] - gross_metrics['avg']) * 100:+.2f}%/"
    ...
    if not getattr(args, "no_bootstrap", False) and args.bootstrap > 0 and net_metrics:
        ...
        boot_all = bootstrap_trade_stats(
            [t["ret"] for t in all_trades],
            args.bootstrap,
            args.seed,
            ci,
        ...
        for direction in ("多", "空"):
            vals = [t["ret"] for t in all_trades if t["direction"] == direction]
            b = bootstrap_trade_stats(
                vals, args.bootstrap, args.seed, ci, config.BACKTEST_BOOTSTRAP_MIN_TRADES
            )
    ...
    if getattr(args, "oos_ratio", 0.0) and 0.0 < args.oos_ratio < 1.0:
        ...
        L.append(
            f"  样本内外对照（按平仓时间排序，前{is_pct:.0f}%为IS样本内、后{args.oos_ratio * 100:.0f}%为OOS样本外）："
        )
        L.append(
            "    IS 全部："
            + _fmt_metrics(metrics_from_returns([t["ret"] for t in is_tr], args.hold))
        )
        L.append(
            "    OOS全部："
            + _fmt_metrics(metrics_from_returns([t["ret"] for t in oos_tr], args.hold))
        )
        for direction in ("多", "空"):
            iv = [t["ret"] for t in is_tr if t["direction"] == direction]
            ov = [t["ret"] for t in oos_tr if t["direction"] == direction]
            L.append(
                f"    IS/OOS {direction}头："
    ...
    if not getattr(args, "no_benchmark", False):
        ...
        if n_valid and net_metrics and pool_bh is not None:
            ...
            L.append(
                "  对照基准（同区间一直买入持有主连，主连已比例复权；策略仅信号时持仓、基准全程持仓）："
            )
            L.append(
                f"    等权{n_valid}品种篮子买入持有累计 {pool_bh * 100:+.1f}%；策略净累计 "
                f"{net_metrics['cumulative'] * 100:+.1f}%；超额(算术差) {ex * 100:+.1f}个百分点"
            )
            L.append(
                f"    逐品种跑赢买入持有基准 {beat}/{n_valid}，跑输/持平 {n_valid - beat}/{n_valid}"
            )
    ...
    if wf_symbols:
        ...
        L.append(
            "  滚动walk-forward纯样本外（每折只在前段IS的3×3网格选参、用于后段互不重叠OOS；选参不含本段未来）："
        )
        L.append(
            "    OOS拼接全部："
            + _fmt_metrics(metrics_from_returns([t["ret"] for t in all_oos], args.hold))
        )
        for direction in ("多", "空"):
            dv = [t["ret"] for t in all_oos if t["direction"] == direction]
            L.append(
                f"    OOS拼接{direction}头：" + _fmt_metrics(metrics_from_returns(dv, args.hold))
            )
        ...
        if is_avg is not None:
            L.append(
                f"    共{len(wf_symbols)}品种/{len(all_folds)}折（其中{fb}折IS样本不足回退默认参数）；"
                f"折级IS均收{is_avg * 100:+.2f}%→OOS均收{oos_avg * 100:+.2f}%；选参分布 {usage}"
            )
        else:
            L.append(
                f"    共{len(wf_symbols)}品种/{len(all_folds)}折；IS/OOS均有交易的折不足，无法比较衰减；选参分布 {usage}"
            )
    L.append("")
    ...
    if not args.no_stable:
        ...
        L.append(
            "  "
            + f"{'持有':<6}{'阈值':<8}{'样本':<8}{'胜率':<10}{'均收':<12}{'平均累计':<12}{'最大回撤':<12}{'夏普':<8}{'正均收品种'}"
        )
        for x in _aggregate_stability(results):
            L.append(
                "  "
                + f"{x['hold']:<6}{x['entry']:<8}{x['n']:<8}{x['win_rate'] * 100:.1f}%".ljust(16)
        ...
        L.append("")
    L.append(
        ("四" if not args.no_stable else "三")
        + "、分品种交易表现（扣费后，按交易次数排序，最多展示30个）"
    )
    ranked = sorted(
        [r for r in results if r.get("trade_metrics")], key=lambda x: -x["trade_metrics"]["n"]
    )[:30]
    for r in ranked:
        m = r["trade_metrics"]
        ...
        L.append(
            f"  {r['name']:<8} {r['code']:<8} K线{r['bars']}根/换月修正{r['roll_count']}处/"
    ...
    L.append("")
    L.append(
        "结论使用规则：胜率和盈亏比需同时看；样本<20只作观察。扣费后仍稳定、且参数网格多数为正，才说明技术规则具备初步稳健性。"
    )
    return "\n".join(L) + "\n"
...
def archive_run(args, results, errors, net_metrics, db_factory=None):
    ...
    try:
        ...
        params = {
            ...
            "no_real_fees": args.no_real_fees,
            "no_cost": args.no_cost,
            "oos_ratio": args.oos_ratio,
            "no_limit_filter": args.no_limit_filter,
        ...
        rid = db.insert_backtest_run(
            {
                ...
                "kind": "daily",
                "fill_mode": args.fill,
                "cost_mode": _cost_mode_text(args),
...
def main(argv=None):
    ...
    parser.add_argument(
        "--all", action="store_true", help="回测全部64个品种（默认也是全部，可用--limit限量）"
    )
    ...
    parser.add_argument(
        "--fee-rate",
        type=float,
        default=config.BACKTEST_FEE_RATE,
        help="真实费率表缺失时的兜底单边手续费率，默认万0.5",
    )
    parser.add_argument(
        "--slip-rate",
        type=float,
        default=config.BACKTEST_SLIP_RATE,
        help="单边滑点率（按价格比例），默认万1",
    )
    parser.add_argument(
        "--fees-file",
        default=config.FUTURES_FEES_FILE,
        help="真实券商手续费CSV，默认data/futures_fees.csv",
    )
    parser.add_argument(
        "--no-real-fees", action="store_true", help="不读真实手续费表，统一使用--fee-rate比例费率"
    )
    parser.add_argument(
        "--limit-move",
        type=float,
        default=config.BACKTEST_LIMIT_LOCK,
    ...
    parser.add_argument(
        "--impact-rate",
        type=float,
        default=config.BACKTEST_IMPACT_RATE,
        help="单边冲击成本率(价格比例)，默认0=不额外计，往返两次",
    )
    parser.add_argument(
        "--bootstrap",
        type=int,
        default=config.BACKTEST_BOOTSTRAP_N,
    ...
    parser.add_argument(
        "--seed",
        type=int,
        default=config.BACKTEST_BOOTSTRAP_SEED,
        help="bootstrap固定随机种子（可复现）",
    )
    parser.add_argument(
        "--oos-ratio",
        type=float,
        default=config.BACKTEST_OOS_RATIO,
        help="样本外占比(0=关闭)；如0.3=后30%%交易为OOS与前70%%IS并列对照",
    )
    parser.add_argument(
        "--no-archive", action="store_true", help="不向 storage.backtest_runs 落本次留档"
    )
    parser.add_argument(
        "--no-validation-ref",
        action="store_true",
    ...
    parser.add_argument(
        "--wf-train",
        type=int,
        default=config.BACKTEST_WF_TRAIN_BARS,
        help="walk-forward 每折IS训练窗交易日根数（默认%d）" % config.BACKTEST_WF_TRAIN_BARS,
    )
    parser.add_argument(
        "--wf-test",
        type=int,
        default=config.BACKTEST_WF_TEST_BARS,
        help="walk-forward 每折OOS测试窗交易日根数（默认%d，折间不重叠）"
        % config.BACKTEST_WF_TEST_BARS,
    )
