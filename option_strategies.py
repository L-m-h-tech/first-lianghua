...
def _calendar_legs_text(sym, ex, legs):
    ...
    for lg in legs:
        ...
        label = lg.get("label", "")
        code = contracts_mod.option_code_hint(sym, ex, yy, mm, lg["K"], lg["kind"]) if yy else ""
...
def _inst_check(inst, direction):
    ...
    tip = (
        "与策略同向"
        if ratio * direction > 0.2
        else ("观点中性" if abs(ratio) <= 0.2 else "轻度相反")
    )
...
def _naked_option_margin_points(F, leg):
    ...
    main = max(base * config.OPT_SELLER_MARGIN_RATE - otm, base * config.OPT_SELLER_MARGIN_MIN_RATE)
    return qty * (prem + main)


def _futures_margin_points(legs):
    return sum(
        abs(lg.get("qty", 1)) * lg["K"] * config.OPT_FUTURES_MARGIN_RATE
        for lg in legs
        if lg.get("kind") == "future"
    )
...
def recommend(name, fut_row):
    ...
    if prem_info.get("prem") is not None:
        prem_note += (
            f"；溢价榜: IV{prem_info['iv']:.1f}%/HV{prem_info['hv']:.1f}%"
    ...
    if score >= 2 or score <= -2:
        ...
        legs = [_leg(F, T, iv, kind, k_long, True), _leg(F, T, iv, kind, k_short, False)]
        ...
        cand = _mk_candidate(
            "牛市看涨价差" if kind == "call" else "熊市看跌价差",
            sgn,
            legs,
            F,
            sigma_move,
    ...
    if abs(score) >= config.OPT_SCORE_MIN:
        ...
        checks = [
            ...
            (
                "隐波状态",
                iv_ratio <= buy_cap and (iv_pct is None or iv_pct <= config.OPT_IV_PCT_BUY_MAX),
                f"{iv_src}IV/HV={iv_ratio:.2f}，裸买上限{buy_cap:.2f}/分位上限{config.OPT_IV_PCT_BUY_MAX:.0%}{prem_note}",
            ),
            (
                "幅度覆盖",
                need > 0 and expected >= 1.5 * need,
                f"预期波动≈{expected:.1f}点 vs 盈亏平衡需{need:.1f}点"
    ...
    if conflict and abs(score) >= 3:
        ...
        checks = [
            ...
            (
                "隐波状态",
                iv_ratio <= min(1.2, buy_cap)
                and (iv_pct is None or iv_pct <= config.OPT_IV_PCT_BUY_MAX),
    ...
    if abs(score) < 2:
        ...
        legs = [
            _leg(F, T, iv, "call", kcs, False),
            _leg(F, T, iv, "put", kps, False),
            _leg(F, T, iv, "call", kco, True),
            _leg(F, T, iv, "put", kpo, True),
        ]
        ...
        checks = [
            ...
            (
                "隐波状态",
                iv_ratio >= seller_floor
                and (iv_pct is None or iv_pct >= config.OPT_IV_PCT_SELL_FLOOR),
                f"{iv_src}IV/HV={iv_ratio:.2f}，卖方法下限{seller_floor:.2f}/分位下限{config.OPT_IV_PCT_SELL_FLOOR:.0%}{prem_note}",
            ),
            (
                "区间覆盖",
                (kcs - F) >= 1.3 * sigma_move and (F - kps) >= 1.3 * sigma_move,
        ...
        cand["max_profit"], cand["max_loss"], cand["be"] = (
            credit,
            max_loss,
            (kps + credit, kcs - credit),
        )
        cand["margin_points"] = max_loss
        cand["margin_note"] = (
            f"限定风险卖方，保守保证金≈{max_loss:.1f}点（最大亏损口径，实际以期货公司为准）"
        )
        cands.append(cand)
    ...
    if abs(score) < 2:
        ...
        legs = [
            _leg(F, T, iv, "call", k1, True),
            _leg(F, T, iv, "call", k2, False, 2),
    ...
    if 2 <= score < config.SCORE_MID or -config.SCORE_MID < score <= -2:
        ...
        if (k_short - k_long) * sgn <= 0:
            k_short = k_long + sgn * step
        legs = [_leg(F, T, iv, kind, k_long, True), _leg(F, T, iv, kind, k_short, False, 2)]
        width = abs(k_short - k_long)
        ...
        checks.insert(1, ("机构观点配合", iok, inode))
        cand = _mk_candidate(
            ("看涨" if sgn > 0 else "看跌") + "比率价差1:2", sgn, legs, F, sigma_move, checks, 3
        )
        cand["max_profit"], cand["max_loss"], cand["be"] = max_profit, None, far_be
        cand["margin_points"] = margin
        cand["margin_note"] = (
            f"1:2结构含1张裸露短腿，保守保证金≈{margin:.1f}点；"
    ...
    if 2 <= score < config.SCORE_MID:
        ...
        cand["margin_note"] = (
            f"需持有/买入1张期货，期货保证金保守≈{cand['margin_points']:.1f}点；Call由标的备兑"
        )
        cands.append(cand)
    ...
    if score >= config.OPT_SCORE_MIN:
        ...
        checks = [
            ...
            (
                "隐波状态",
                iv_ratio <= 1.80 and (iv_pct is None or iv_pct <= config.OPT_IV_PCT_SPREAD_MAX),
        ...
        cand["margin_points"] = _futures_margin_points(legs)
        cand["margin_note"] = (
            f"需持有/买入1张期货，期货保证金保守≈{cand['margin_points']:.1f}点；买Put无期权保证金"
        )
        cands.append(cand)
    ...
    if len(exps) >= 2:
        ...
        if (
            1 <= gap <= config.IV_CALENDAR_MAX_MONTH_GAP
            ...
            liq_ok = (
                ne["n_points"] >= config.IV_SURFACE_MIN_STRIKES
            ...
            checks = [
                ...
                (
                    "近月剩余",
                    ne["days"] >= config.IV_CALENDAR_NEAR_MIN_DAYS,
                    f"近月剩余{ne['days']}天/远月{fa['days']}天（近月≥{config.IV_CALENDAR_NEAR_MIN_DAYS}天）",
                ),
                (
                    "静态盈亏空间",
                    space_ok,
                    f"假设IV不变、近月到期标的收K时最优盈利≈{est_max_profit:.1f}点"
                    if est_max_profit is not None
                    else "近月到期标的收K时最优盈利无法估算（要求为正，否则时间价值损耗后无利可图）",
                ),
                (
                    "曲面流动性",
                    liq_ok,
            ...
            if long_cal:
                ...
                if debit > 0:
                    cand["margin_note"] = (
                        "借方日历，最大亏损=净支出%.1f点；近月到期后只剩远月多头，"
                        "建议支出≤账户5%%" % debit
                    )
                else:
                    cand["margin_note"] = (
                        "近月权利金反超远月、净%.1f点开仓(信用日历)；静态下两端保本，"
                ...
                cand["margin_points"] = margin
                cand["margin_note"] = (
                    f"贷方日历，近月到期后远月Call裸露，保守保证金≈{margin:.1f}点；"
            ...
            if est_max_profit is not None:
                cand["margin_note"] += (
                    "；近月到期标的收近平值时最大盈利≈%.1f点(估算,依赖IV路径)" % est_max_profit
                )
            cands.append(cand)
    ...
    chosen = next(
        (c for c in cands if all(c2[1] for c2 in _build_checks(c, name, fut_row, month_label))),
        None,
    )
    if chosen is None:
        chosen = cands[0]
    ...
    if ok_all:
        ...
        pos_note = (
            f"具体腿: {legs_text}；净{'收入' if chosen['net'] >= 0 else '支出'}{abs(chosen['net']):.1f}点；"
            f"最大盈利{mp} / 最大亏损{ml}；"
    ...
    return {
        ...
        "legs_text": legs_text,
        "net": chosen["net"],
        "max_profit": chosen.get("max_profit"),
        "max_loss": chosen.get("max_loss"),
        "be": chosen.get("be"),
        "delta": chosen["delta"],
        "gamma": chosen.get("gamma", 0.0),
        "vega": chosen["vega"],
        "theta_day": chosen["theta_day"],
...
def _build_checks(cand, name, fut_row, month_label):
    ...
    if cand["net"] < 0 and cand.get("margin_points", 0.0) <= 0:
        ...
        msg = cand.get(
            "margin_note", "卖方结构需缴纳保证金（按交易所/期货公司标准），大幅跳空可能接近最大亏损"
        )
