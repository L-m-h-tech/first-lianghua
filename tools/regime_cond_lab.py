...
import factor_regime as frg  # noqa: E402
import orthogonal_blend_oos as ob  # noqa: E402
...
def resolve_factor(bysym_rows, factor=None, expr=None):
    ...
    if expr:
        ...
        for sym, rows in bysym_rows.items():
            ...
            fac_map[sym] = {
                r["date"]: v for r, v in zip(rows_sorted, fac, strict=False) if fx._isnum(v)
            }
        return name, fac_map
    if not factor:
...
def compose_factor(maps):
    ...
    for d in sorted(all_dates):
        zs = [ob.cs_uniform({s: inner[d] for s, inner in m.items() if d in inner}) for m in maps]
        common = None
        for z in zs:
            ks = set(z)
...
def forward_maps(bysym_rows, horizon):
    ...
    for sym, rows in bysym_rows.items():
        ...
        out[sym] = {
            r["date"]: fwd[t]
            for t, r in enumerate(rows_sorted)
...
def placebo_regime_map(reg_map, dates, seed=20260905):
    ...
    out = {}
    for d, vec in zip(dates, shuffled, strict=False):
...
def robust_chain(
    ...
    for hh in grid:
        _d, summ, ics, n_books = evaluate_views(
            bysym_rows, fac_map, reg_map, hh, n_q, min_names, cost
        )
        out["h_grid"].append(
            {
                "h": hh,
                ...
                "all_ic": ics["all"]["mean_ic"],
                "low_ic": ics["low"]["mean_ic"],
    ...
    for y in sorted({d[:4] for d in dates}):
        a = em.cs_summary(
            [(d, ic, n) for d, v, ic, n in cs_ics_y if v == "all" and d.startswith(y)]
        )
        l = em.cs_summary(
            [(d, ic, n) for d, v, ic, n in cs_ics_y if v == "low" and d.startswith(y)]
        )
        out["yearly"].append(
            {
                "year": y,
    ...
    for label, sub in (("前半", dates[:half]), ("后半", dates[half:])):
        ...
        out["sub_periods"].append(
            {
                "label": label,
                "date_min": sub[0] if sub else None,
                "date_max": sub[-1] if sub else None,
                "low_annual": s_low["net"].get("annual_ret"),
                "all_annual": s_all["net"].get("annual_ret"),
                "low_ic": i_low["mean_ic"],
                "all_ic": i_all["mean_ic"],
    ...
    out["placebo"] = {
        "low_annual": first["low_annual"],
        "low_ic": first["low_ic"],
        "all_annual": first["all_annual"],
        "seeds": len(out["placebos"]),
        "low_ic_min": ic_min,
        "low_ic_median": ic_med,
        "low_ic_max": ic_max,
        "low_annual_min": an_min,
        "low_annual_median": an_med,
...
def render_robust(rb):
    ...
    for r in rb["sub_periods"]:
        L.append(
            "    %s（%s ~ %s）：低波净年化 %s / IC %s（全样本 %s / %s，低波有效天数 %d）"
            % (
                r["label"],
                r["date_min"],
                r["date_max"],
    ...
    if p:
        if p.get("seeds", 1) > 1:
            L.append(
                "  placebo（标签日期重排×%d种子，低波视图）：真实低波应远离分布才非分桶假象"
                % p["seeds"]
            )
            L.append(
                "    IC：min %s / median %s / max %s；净年化：min %s / median %s / max %s（全样本 IC 见上表）"
                % (
                    ...
                    ("%+.2f%%" % (100.0 * p["low_annual_min"]))
                    if p["low_annual_min"] is not None
                    else "--",
                    ("%+.2f%%" % (100.0 * p["low_annual_median"]))
                    if p["low_annual_median"] is not None
                    else "--",
                    ("%+.2f%%" % (100.0 * p["low_annual_max"]))
                    if p["low_annual_max"] is not None
                    else "--",
                )
            )
        else:
            L.append(
                "  placebo（标签日期重排，低波视图）：净年化 %s / IC %s（全样本 %s）"
                "——若与真实低波口径接近则为分桶假象、远离则 regime 效应成立（建议 --placebo-seeds 4 取分布）"
                % (
                    ("%+.2f%%" % (100.0 * p["low_annual"]))
                    if p["low_annual"] is not None
                    else "--",
                    ("%+.3f" % p["low_ic"]) if p["low_ic"] is not None else "--",
                    ("%+.2f%%" % (100.0 * p["all_annual"]))
                    if p["all_annual"] is not None
                    else "--",
                )
            )
    return L
...
def run(
    ...
    ics = {v: summarize_ics(cs_ics, v) for v in VIEWS}
    rb = (
        robust_chain(bysym, fac_map, reg_map, h, n_q, min_names, cost, placebo_seeds=placebo_seeds)
        if robust
        else None
    )
    result = {
    ...
    for v in VIEWS:
        ...
        result["views"][v] = {
            ...
            "avg_turnover": ls["avg_turnover_one_sided"],
            "total_cost": ls["total_cost"],
            "avg_spread": ls["avg_spread"],
            "cs_ic": ics[v],
        }
    if rb:
        result["robust"] = rb
    ...
    try:
        ...
        experiment_ledger.safe_record(
            "regime_cond_lab",
            {"factor": name, "h": h, "n_q": n_q, "min_names": min_names},
            {
...
def render_report(result, robust=None):
    ...
    L.append(
        "因子=%s（%s）；H=%d 对齐非重叠再平衡；%d层多顶空底；单边成本万%.1f；截面最少%d品种"
        % (
            result["factor"],
            "复合截面" if result.get("compose") else ("表达式" if result["expr"] else "面板列"),
            result["h"],
            result["n_q"],
            10000.0 * result["cost_oneway"],
            result["min_names"],
        )
    )
    L.append(
        "价格源=%s：%s；品种=%d 交易日=%d（%s ~ %s）；regime=vol(hv60过去120日ts_rank三分位,PIT)"
        % (
            result.get("source", "panel"),
            result.get("db"),
            result["n_symbols"],
            result["n_dates"],
            result.get("date_min"),
    ...
    L.append("-" * 104)
    L.append(
        "  %-16s %8s %10s %9s %10s %10s %9s %10s %11s"
        % (
            "口径",
            "调仓期",
            "净年化",
    ...
    for v in VIEWS:
        ...
        t_txt = (
            "%+.1f/%d" % (ic.get("t_stat") or 0.0, ic.get("n_days") or 0)
            if ic.get("mean_ic") is not None
            else "--"
        )
        L.append(
            "  %-16s %8d %10s %9s %10s %10s %9s %10s %11s"
            % (
                VIEW_LABEL[v],
                s["n_periods"],
                ("%+.2f%%" % (100.0 * net["annual_ret"]))
                if net["annual_ret"] is not None
                else "--",
                ("%+.2f" % net["sharpe"]) if net["sharpe"] is not None else "--",
                ("%+.2f%%" % (100.0 * net["max_drawdown"]))
                if net["max_drawdown"] is not None
                else "--",
                ("%+.2f%%" % (100.0 * s["gross_annual"]))
                if s["gross_annual"] is not None
                else "--",
...
def selftest():
    ...
    assert (
        rb7["placebo"]["low_ic_min"] is not None
        and rb7["placebo"]["low_ic_median"] is not None
    ...
    assert (
        abs(comp["A"]["d"]) < 1e-12 and abs(comp["B"]["d"]) < 1e-12 and abs(comp["C"]["d"]) < 1e-12
    )
    ...
    print(
        "regime_cond_lab selftest ALL PASS（因子装配两路径/regime标签/build_books结构无未来/"
...
def main(argv=None):
    ...
    ap.add_argument(
        "--expr", default=DEFAULT_EXPR, help="'EXPR[:名称]'（白名单DSL）；传空串则须给 --factor"
    )
    ...
    ap.add_argument(
        "--robust", action="store_true", help="第76轮：稳健链（H网格/子期分段/placebo标签重排）"
    )
    ap.add_argument(
        "--source",
        default="panel",
    ...
    ap.add_argument(
        "--term-db", default=None, help="term 源的缓存路径（默认 cache/term_history.db）"
    )
    ap.add_argument(
        "--codes", default="", help="term 源品种过滤（逗号分隔中文名/主连，缺省全品种）"
    )
    ap.add_argument(
        "--placebo-seeds",
        type=int,
    ...
    run(
        db_path=args.db,
        factor=args.factor,
        expr=(args.expr.strip() or None),
        h=args.h,
        n_q=args.quantiles,
        min_names=args.min_names,
        cost=args.cost_oneway,
        robust=args.robust,
