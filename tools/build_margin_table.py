    ...
    for cname, meta in config.VARIETIES.items():
        ...
        if sym in MULT_KNOWN_DIFF:
            ...
            if abs(fee_mult.get(sym, -1) - fee_m) > 1e-9 or abs(q_m - mult) > 1e-9:
                warn.append(
                    f"{sym}: 白名单口径异常 内置{mult}/费率{fee_mult.get(sym)} 预期{q_m}/{fee_m}"
                )
            notes = [why]
        else:
            if sym in fee_mult and abs(fee_mult[sym] - mult) > 1e-9:
                warn.append(
                    f"{sym}: 内置乘数{mult} != 费率表{fee_mult[sym]}（非白名单，需人工核对）"
                )
            notes = []
        ...
        rows.append(
            {
                ...
                "name": cname,
                "exchange": meta["ex"],
                "broker_margin": f"{info['margin']:.4f}",
                "exchange_margin": "",
                "limit_basic": f"{info['limit_basic']:.4f}"
                if info["limit_basic"] is not None
                else "",
                "multiplier": int(mult),
                "as_of": date,
                "source": f"{source_name}交易保证金比例(投机档)",
                "note": "；".join(notes),
            }
        )
