...
def turnover_series(vols, ois):
    ...
    out = []
    for v, p in zip(vols, ois, strict=False):
...
def symbol_stat(dates, close, vol, oi, win=Z_WIN, min_n=Z_MIN, chg_win=CHG_WIN):
    ...
    return {
        ...
        "n_valid": len(valid_to),
        "ret%d" % chg_win: ret,
        "oi_chg%d" % chg_win: doi,
...
def concentration_series(by_date):
    ...
    for d in dates:
        ...
        share.append(sh)
        nact.append(len(ps))
        hhi.append(sum((p / tot) ** 2 for p in ps))
...
def render(meta, sym_rows, quad_dist, conc_rows):
    ...
    add = L.append
    add(
        "G24续 投机/套保压力代理 spec_pressure_lab（纯离线只读 research_panel+term_history，只监控不下单、不进综合分）"
    )
    add("=" * 100)
    add(
        "窗口 %s ~ %s；投机度尾窗%d日(最少%d)、量仓四象限近%d日；覆盖 %d 品种。"
        % (
            meta.get("panel_d0"),
            meta.get("panel_d1"),
            meta["z_win"],
    ...
    add(
        "诚实边界：无多空/套保-投机分类持仓(G22缺口)，下列为成交活跃度/量仓行为**代理**，非真HP/SP。"
    )
    add("")
    add("【一】投机度（成交量/持仓量）总览")
    add(
        "  全市场投机度z中位数=%s；异常活跃(|z|≥%.1f) %d 只、异常清淡 %d 只。"
    ...
    for r in hot[:12]:
        add(
            ...
            % (
                r["sym"],
                (r.get("sector") or "未知")[:8],
                _fmt(r.get("turnover"), 2),
                _fmt(r.get("turn_z"), 2, signed=True),
                _fmt(r.get("turn_pctile"), 2, pct=True),
                _fmt(r.get("turn_mean"), 2),
                r.get("quadrant") or "—",
            )
        )
    ...
    add("")
    add(
        "【三】近月持仓集中度 / 换月压力（ckline 各合约持仓，覆盖 %d 品种）" % meta.get("conc_n", 0)
    )
    rolling = [r for r in conc_rows if r.get("rolling")]
    add(
        "  主力占比处自身尾窗极低分位(<%.0f%%，换月/持仓向后迁移) %d 只：%s"
        % (
            CONC_LOW_PCT * 100,
            len(rolling),
            "、".join("%s(%.0f%%)" % (r["sym"], 100 * r["main_share"]) for r in rolling[:14])
            or "无",
        )
    )
    ...
    for r in sorted(conc_rows, key=lambda x: x.get("main_share") or 1)[:12]:
        add(
            ...
            % (
                r["sym"],
                _fmt(100 * r["main_share"], 1) + "%",
                r.get("n_active"),
                _fmt(r.get("hhi"), 3),
                _fmt(r.get("share_pctile"), 2, pct=True),
    ...
    add(
        "  2) 量仓四象限是总量行为读法，无法区分多空双方身份，真·多空/套保压力须 G22 分类持仓（会员/COT）；"
    )
    add(
        "  3) 不同品种换手中枢差异大（如黑色普遍高于农产品），跨品种比较以各自尾窗 z 为准而非原始倍数；"
    )
    add(
        "  4) 近月集中度受合约挂牌/到期节奏影响，换月信号应结合主力切换日历确认，避免误读为趋势性减仓。"
    )
    return "\n".join(L)
...
def _load_panel(panel_db):
    ...
    for r in rows:
        s = r["sym"]
        b = out.setdefault(
            s, {"dates": [], "c": [], "v": [], "oi": [], "sector": r.get("sector") or "未知"}
        )
        b["dates"].append(r["date"])
        b["c"].append(r.get("c"))
        b["v"].append(r.get("v"))
        b["oi"].append(r.get("oi"))
        if r.get("sector"):
            b["sector"] = r["sector"]
    for b in out.values():
...
def _load_concentration(term_db, conc_days=CONC_DAYS):
    ...
    try:
        ...
        sql = (
            "SELECT sym,d,p FROM ckline WHERE d >= "
...
def run(panel_db=PANEL_DB, term_db=TERM_DB, txt_path=LAB_TXT, json_path=LAB_JSON, verbose=True):
    ...
    meta = {
        "panel_d0": min(pd0) if pd0 else None,
        "panel_d1": max(pd1) if pd1 else None,
        "z_win": Z_WIN,
        "z_min": Z_MIN,
        "chg_win": CHG_WIN,
        "hot_z": HOT_Z,
        "n_sym": len(sym_rows),
        "med_z": statistics.median(all_z) if all_z else None,
        "n_hot": n_hot,
        "n_cold": n_cold,
        "conc_n": len(conc_rows),
    }
    ...
    try:
        ...
        el.safe_record(
            ...
            },
            {
                ...
                "quadrant": quad_dist,
                "conc_n": len(conc_rows),
                "rolling": roll_names[:14],
                "hot_top": hot_names,
            },
            inputs=[panel_db, term_db],
