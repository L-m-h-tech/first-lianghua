...
import tradable_mask as tmask  # noqa: E402
import xsmom_eval as xs  # noqa: E402
...
def _term_maps(term_series, smooth, mom_k):
    ...
    for i, d in enumerate(dates):
        out[d] = {
            ...
            "carry_nn": cn_s[i],
            "carry_mom": cf_chg[i],
            "slope": term_series[i]["slope"],
            "curv": term_series[i]["curv"],
            "doi": doi[i],
        }
    return out


def carry_points_from_adjusted(
    name, sector, bars, term_series, horizons, smooth=5, mom_k=20, vol_lb=63
):
    ...
    nav_map = {r["date"]: v for r, v in zip(term_series, nav_term, strict=False)}
    ...
    for t in range(vol_lb, len(closes)):
        ...
        p["near_amount"] = (
            (tr.get("near_s") * p["near_vol"]) if (tr.get("near_s") and p.get("near_vol")) else None
        )
        ok = True
        for H in horizons:
            p["fwd%d" % H] = fwd[H][t]
            ...
            if p["fwd%d" % H] is None or p["fwdn%d" % H] is None:
                ok = False
        if ok and all(math.isfinite(p[fk]) for fk, _l, _d in CARRY_FACTORS if p[fk] is not None):
            pts.append(p)
    return pts


def build_carry_points(
    name, sector, raw_main_bars, term_series, horizons, smooth=5, mom_k=20, vol_lb=63, days=2500
):
    """网络旧路径：主连 raw[-days:] 比例复权 -> carry_points_from_adjusted（历史逐值一致）。"""
    bars, _roll = backtest.ratio_adjusted_bars(raw_main_bars[-days:])
    return carry_points_from_adjusted(
        name, sector, bars, term_series, horizons, smooth, mom_k, vol_lb
    )
...
def _fetch_one_carry(
    ...
    try:
        ...
        if prefer_panel:
            ...
            pts = carry_points_from_adjusted(
                name, sector, bars, term_series, horizons, smooth, mom_k, vol_lb
            )
        else:
            raw = futures_data.fetch_daily_kline(main_code)
            pts = build_carry_points(
                name, sector, raw, term_series, horizons, smooth, mom_k, vol_lb, days
            )
        if not pts:
...
def _ls_summary(dates, by_date, factor, main_h, vol_lb, n_q, min_names, cost_round):
    ...
    pers = xs.cross_section_periods(
        dates, by_date, factor, main_h, vol_lb, n_q, min_names, "equal", main_h
    )
    if not pers:
        return None
...
def compare_mask(
    ...
    raw_sum = _ls_summary(
        raw_main_dates, raw_by2, factor, main_h, vol_lb, n_q, min_names, cost_round
    )
    ...
    md_sum = _ls_summary(md_main_dates, md_by2, factor, main_h, vol_lb, n_q, min_names, cost_round)
    out.update(
        {
            "mask": md_sum,
...
def render_mask_compare(cmp):
    ...
    for label, key in (("原始(无掩码)", "raw"), ("掩码后(剔不可交易)", "mask")):
        ...
        pf, bp = s["pf"], s["bands"]
        L.append(
            "  %-14s %10d %10.2f %10.3f %8.0f %8.0f %8.3f"
            % (
                label,
                s["n_periods"],
                pf["net_t"],
...
def _load_regime_map():
    ...
    try:
        ...
        for row in con.execute(
            "SELECT sym,date,c,ret126,hv60 FROM research_panel ORDER BY sym,date"
        ):
            bysym[row[0]].append(dict(zip(cols, row, strict=False)))
        con.close()
...
def render_regime_compare(cmp):
    ...
    if cts:
        parts = [
            "%s:留%d/剔%d" % (w, cts[w]["kept"], cts[w]["dropped"])
            for w in ("low", "high")
            if w in cts
        ]
        L.append("  点数统计：" + "；".join(parts) + "（mid/无覆盖点不入条件化视图）")
    L.append(
        "  诚实结论：条件化只改变子截面样本；若低波/高波净t与全样本同向同量级，则 regime 条件化不增益。"
    )
    near = cmp.get("near")
    if near:
        ...
        for label, key in (("全样本", "all"), ("仅低波", "low"), ("仅高波", "high")):
            ...
            pf, bp = s["pf"], s["bands"]
            L.append(
                "  近月·%-10s %10d %10.2f %10.3f %8.0f %8.0f %8.3f"
                % (
                    label,
                    s["n_periods"],
                    pf["net_t"],
...
def _pf_line(pf):
    if pf is None:
        return "无样本"
    return (
        "n=%d 毛均%+.3f%% 净均%+.3f%% 净t=%+.2f 胜率%.0f%% 净累计%+.1f%% 年化%+.1f%% 夏普%.2f 回撤%.1f%%"
        % (
            pf["n"],
            pf["gross_mean"] * 100,
            pf["net_mean"] * 100,
...
def build_report(
    ...
    L.append("=" * 108)
    L.append(
        "样本：%d 个品种、%d 个(品种×交易日)点；主窗=全局最近 %d 个调仓日(交易日)，长样本 %d 交易日；"
        % (n_sym, len(points), main_days_n, days)
    )
    L.append(
        "      因子来自真实逐合约重建的历史期限结构(近/次/远结算价)，carry平滑%d日、basis-mom/DOI窗%d日；"
        % (smooth, mom_k)
    )
    L.append(
        "      目标=主连比例复权未来%d日收益；每调仓日跨品种按因子排序分%d档、多最高档/空最低档(市场中性)。"
        % (main_h, n_q)
    )
    L.append(
        "成本：单方向一次往返 %.4f%%、多空两腿 %.4f%%（银河真实费+滑点口径，与 XSMOM 一致）。"
    ...
    pers = xs.cross_section_periods(
        main_dates, main_by, factor, main_h, vol_lb, n_q, min_names, "equal", main_h
    )
    pf = xs.perf_stats(pers, main_h, cost_round, "ls")
    ...
    if bp:
        ...
        L.append(
            "  分档单调性 %.0f%%、档位列序RankIC=%+.3f、Q%d-Q1价差%+.3f%%"
    ...
    pers_n = xs.cross_section_periods(
        main_dates, near_by, factor, main_h, vol_lb, n_q, min_names, "equal", main_h
    )
    ...
    grid = {}
    for H in horizons:
        pp = xs.cross_section_periods(
            main_dates, main_by, factor, H, vol_lb, n_q, min_names, "equal", H
        )
    ...
    for fk, lab, _d in CARRY_FACTORS:
        pp = xs.cross_section_periods(
            main_dates, main_by, fk, main_h, vol_lb, n_q, cond_min, "equal", main_h
        )
        ppn = xs.cross_section_periods(
            main_dates, near_by, fk, main_h, vol_lb, n_q, cond_min, "equal", main_h
        )
        ...
        if pfp and bpp:
            ...
            L.append(
                "  %-14s  %+6.2f %+6.2f  %+7.3f  %6.2f  %5.0f  %+6.3f"
                % (
                    fk,
                    pfp["net_t"],
                    tn,
    ...
    for sec, v in sorted(sec_int.items(), key=lambda kv: -kv[1]["mean"]):
        L.append(
            "  板块内 %-6s n=%-3d 均收%+.3f%% 胜率%.0f%%"
            % (sec, v["n"], v["mean"] * 100, v["win"] * 100)
        )
    exposure, loso = xs.sector_breakdown(pers)
    if exposure:
        worst = max(exposure.items(), key=lambda kv: abs(kv[1]["net"]))
        L.append(
            "  多空腿净敞口最大板块：%s（占比%.0f%%）" % (worst[0], abs(worst[1]["net"]) * 100)
        )
    if loso:
        L.append(
            "  留一板块后多空毛均收："
    ...
    scan = xs.conditional_scan(
        windows, factor, vol_lb, main_h, n_q, cond_min, cost_round, candidates
    )
    ...
    scan_near = xs.conditional_scan(
        near_windows, factor, vol_lb, main_h, n_q, cond_min, cost_round, near_candidates
    )
    n_robust_near = 0
    ...
    ok_gate, why_gate = xs.gate_verdict(
        pf, oos_pf, bp, leg_long, leg_short, exposure, tmin, mono_gate, max_drive
    )
    L.append(
    ...
    L.append(
        "  结果：%s"
        % (
            "通过✔（可进入下一轮'影子对照'讨论，仍不自动进综合分）"
            if ok_gate
            else "不通过✘（维持研究归档，不进分、不挂影子）"
        )
    )
    for w in why_gate:
        L.append("  └ %s" % w)
    L.append(
        "  双样本稳健候选数：%d / %d（主连口径）；近月含roll口径稳健 %d / %d（=0 则与 TSMOM/XSMOM 同样按负结果诚实归档）"
    ...
    L.append("\n七、机制判读（两口径对照，务必一起读）")
    L.append(
        "  · 主连复权口径(不含roll)净t=%+.2f：衡量 carry 对未来【价格方向】的预测；近月连续含roll口径"
    ...
    if near_t_l >= tmin and near_t_s > 0 and main_t < 0:
        reading = (
            "边缘候选（区别于动量的双样本全负）：carry 在含展期的学术口径下长样本显著为正(t=%+.2f)、"
    ...
    try:
        ...
        for p in pers or []:
            ...
            amt_by_sym = {
                q["sym"]: (q.get("amount") or 0.0)
                for q in points
                if q.get("date") == p.get("date") and q.get("sym") in syms_all
            }
            namt_by_sym = {
                q["sym"]: (q.get("near_amount") or 0.0)
                for q in points
                if q.get("date") == p.get("date") and q.get("sym") in syms_all
            }
            ...
            cap_infos.append(
                {
                    "date": p.get("date"),
                    "long_syms": p.get("long_syms") or [],
                    "short_syms": p.get("short_syms") or [],
        ...
        if cap_infos:
            ...
            if amts:
                ...
                L.append(
                    "  每调仓日多空腿【主连代理】成交额合计：中位%.0f万元；按参与率1%%估算单期容量=%.0f万元"
            ...
            if namts:
                ...
                L.append(
                    "  每调仓日多空腿【真实逐合约】成交额合计：中位%.0f万元；按参与率1%%估算单期容量=%.0f万元"
                    "（近月结算×近月成交量；精确逐笔容量仍待 G14 一档盘口）"
                    % (med_namt / 1e4, med_namt * 0.01 / 1e4)
                )
            # 换手代理：多空腿成员的相对持仓集中度（腿内等权=1/n，n 越少越集中）
            n_lens = [
                len(c["long_syms"]) + len(c["short_syms"])
                for c in cap_infos
                if c["long_syms"] and c["short_syms"]
            ]
            if n_lens:
                L.append(
                    "  多空腿总成员数：平均%.0f个/期（腿数越少换手越敏感；日频多空持仓换手须 G14 盘口精确）"
    ...
    sidecar = {
        ...
        "factor": factor,
        "main_perf": pf,
        "main_perf_near": pf_near,
        "grid": {str(H): v for H, v in grid.items()},
        "family": {
            fk: {
                "perf": v["perf"],
                "perf_near": v.get("perf_near"),
                "mono": v["bands"]["mono"] if v["bands"] else None,
            }
            for fk, v in family.items()
        },
        "sector_internal": sec_int,
        "conditional": {
            cname: {
                "windows": {
                    wn: (
                        None
                        if pp is None
                        else {k: pp[k] for k in ("n", "net_mean", "net_t", "win", "sharpe")}
                    )
        ...
        "verdict": {
            ...
            "reasons": why_gate,
            "n_robust": n_robust,
            "n_robust_near": n_robust_near,
            "main_t": main_t,
            "near_t_short": near_t_s,
            "near_t_long": near_t_l,
            "reading": reading,
        },
        "errors": errors,
        "generated": _now(),
...
def run(argv=None):
    ...
    ap.add_argument(
        "--panel",
        action="store_true",
        help="G21续：主连读已复权面板（期限仍走term_history；面板约1023日，长2500样本请用缺省网络）",
    )
    ap.add_argument(
        "--panel-db",
        default=None,
    ...
    ap.add_argument(
        "--mask",
        action="store_true",
        help="G22续：读 research_panel.db 算可交易性掩码（疑似锁板/距交割月1号≤15天）并剔除不可交易点后重做截面多空对照",
    )
    ap.add_argument(
        "--mask-compare",
        action="store_true",
        help="G22续（第66轮）：掩码前后截面多空绩效对照表（同一输入各跑无掩码/有掩码）",
    )
    ap.add_argument(
        "--regime-compare",
        action="store_true",
        help="G23续（第75轮）：波动率 regime 条件化对照（全样本/仅低波/仅高波子截面多空）",
    )
        ...
        try:
            ...
            if os.path.exists(db_p):
                ...
                mask_notes = "；G22续掩码：原%d点→剔锁板%d+临近交割%d→剩%d点" % (
                    fm_main["original"],
                    fm_main["removed_locked"],
                    fm_main["removed_near"],
                    fm_main["filtered"],
                )
            else:
    ...
    print(text)
    print(
        "品种时点 %d、覆盖品种 %d；主裁决 ok=%s；双样本稳健候选 %d%s；报告 -> %s；JSON -> %s"
        % (
            len(main_points),
            sidecar["n_symbols"],
            verdict["ok"],
            sidecar["verdict"]["n_robust"],
            mask_notes,
            args.out,
            args.json,
        )
    ...
    for si in range(n_sym):
        ...
        for t in range(63, len(closes)):
            p = {
                ...
                "date": dates[t],
                "carry": carry_path[t],
                "carry_nn": carry_path[t],
                "carry_mom": carry_path[t] - carry_path[t - 20],
                "slope": carry_path[t],
                "curv": 0.0,
                "doi": rng.gauss(0, 0.01),
                "vol63": 0.01,
                "v": 1000 + si * 10,
                "oi": 5000 + si * 20,
...
def selftest():
    ...
    raw = [
        {
            "d": "2025-%02d-%02d" % (t // 28 % 12 + 1, t % 28 + 1),
            "o": 100 + t,
            "h": 101 + t,
            "l": 99 + t,
            "c": 100 + t,
    ...
    assert pts and all(
        "carry" in p and "fwd20" in p and "fwdn20" in p and p["carry"] is not None for p in pts
    )
    ...
    print(
        "carry_eval selftest ALL PASS（期限映射/年月范围/收益对齐/合成carry多空单调/"
