...
def init_session(force=False):
    ...
    try:
        ...
        r = http.post(
            AJAX + "/session.php?v=" + VER, source=SOURCE, headers=_HEADERS, timeout=TIMEOUT
        )
...
def fetch_endpoint(ep):
    ...
    try:
        r = http.post(
            AJAX + "/" + ep + "?v=" + VER, source=SOURCE, headers=_HEADERS, timeout=TIMEOUT
        )
        if r.status_code != 200:
        ...
        out.append(
            {
                ...
                "total_vol": _f(it.get("total_vol")),
                "wr_unit": _f(it.get("wr_unit")),
...
def parse_hg(data):
    ...
    for it in data or []:
        ...
        out.append(
            {
                "sym": sym,
                "name": str(it.get("variety") or ""),
                "code": str(it.get("code") or ""),
                "current_price": _f(it.get("current_price")),
                "support": _f(it.get("support")),
                "resistance": _f(it.get("resistance")),
            }
        )
    return out
...
def parse_broker_trend(data):
    ...
    for it in data or []:
        ...
        if not nm:
            continue
        out.append({"name": nm, "grade": str(it.get("grade") or ""), "money": _f(it.get("money"))})
    return out
...
def run(verbose=True):
    ...
    for ep, desc, table, _ in ENDPOINTS:
        ...
        with open(fname, "w", encoding="utf-8") as f:
            json.dump(
                {"ts": results["ts"], "endpoint": ep, "desc": desc, "n": len(rows), "rows": rows},
                f,
                ensure_ascii=False,
                indent=1,
            )
...
def render_txt(results):
    ...
    lines += [
        "",
        "说明：研究侧采集，不进综合分；来源 jiaoyikecha.com（席位持仓/龙虎牛熊/仓单/基本面），",
...
def selftest():
    ...
    # 端点解析（合成响应）
    wr = parse_daily_wr(
        [{"name": "螺纹钢", "symbol": "RB", "total_vol": 134546, "wr_unit": 0.1, "wr_pct": 2.5}]
    )
