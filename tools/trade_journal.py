    ...
    with open(path, encoding="utf-8-sig", newline="") as f:
        for r in csv.DictReader(f):
            t = {
                ...
                "entry_score": _f(r.get("entry_score"), None)
                if r.get("entry_score") not in (None, "")
                else None,
                "margin_rate": _f(r.get("margin_rate"), None)
                if r.get("margin_rate") not in (None, "")
                else None,
            }
            ...
            out.append(t)
    out.sort(
        key=lambda t: ((t["exit_dt"] or _dt.datetime.min), (t["entry_dt"] or _dt.datetime.min))
    )
    return out
...
def load_equity(path=DEFAULT_EQUITY):
    ...
    with open(path, encoding="utf-8-sig", newline="") as f:
        for r in csv.DictReader(f):
            out.append(
                {
                    "dt": parse_dt(r.get("dt")),
                    "equity": _f(r.get("equity")),
                    "static": _f(r.get("static")),
                    "float": _f(r.get("float")),
                    "risk": _f(r.get("risk")),
        ...
        rows.append(
            {
                ...
                "net": net,
                "avg_net": st["avg_pnl"],
                "payoff": st["payoff_ratio"],
                "pf": st["profit_factor"],
                "fees": fees,
                "gross": gross,
                "fee_over_gross": (fees / abs(gross)) if abs(gross) > 1e-9 else None,
                "avg_hold": (sum(holds) / len(holds)) if holds else 0.0,
                "max_ws": st["max_win_streak"],
                "max_ls": st["max_loss_streak"],
            }
        )
...
def load_minute_bars_for(sym, period, lookback, aggregate_from):
    ...
    try:
        ...
        try:
            raw, _src = ib.load_minute_bars(
                db, sym, int(period), int(lookback), int(aggregate_from or 0)
            )
        finally:
            db.close()
...
def attach_excursions(trades, *, period=30, lookback=8000, aggregate_from=0, verbose=False):
    ...
    for t in trades:
        ...
        bars = cache[sym]
        mfe, mae, used = range_excursion(
            t["direction"], t["entry_px"], bars, t["entry_dt"], t["exit_dt"]
        )
        t["mfe_bar"] = mfe
        t["mae_bar"] = mae
        t["xcov_n"] = used
    ...
    return {
        ...
        "period": period,
        "loaded_syms": sum(1 for b in cache.values() if b),
    }
...
def observations(trades, overall, exsum):
    ...
    if aw is not None and al is not None:
        ...
        obs.append(
            "  · 期望拆解：赢面贡献 %s/笔，亏面拖累 %s/笔，净期望 %s/笔（胜率%s×均盈%s / 败率%s×均亏%s）。"
            % (
                _money(e_win),
                _money(e_loss),
                _money(overall["expectancy"]),
                _pct(wr),
                _money(aw),
                _pct(1 - wr),
                _money(al),
    ...
    for title, kf in (
        ...
        for r in bucket_table(trades, kf):
            if r["n"] >= 10 and r["pf"] is not None and r["pf"] < 0.7:
                obs.append(
                    "  · 弱势桶提示：%s=%s 共%d笔 PF=%s 净%s，成本后期望为负，优先复盘该桶。"
    ...
    if gross_abs > 1e-9:
        obs.append(
            "  · 总费用 %s，占|毛盈亏| 的 %s（平今腿拉高成本，留意高频小盈被费用吃掉）。"
    ...
    for r in rg:
        if "强平" in r["key"]:
            obs.append(
                "  · %s %d笔（%s）净%s：日终不留仓是纪律成本，观察其是否系统性割在不利位置。"
                % (r["key"], r["n"], _pct(r["n"] / n), _money(r["net"]))
            )
    # 5) MFE/MAE 由盈转亏
    if exsum and exsum["n_loss"] > 0:
        obs.append(
            "  · 盘中MFE/MAE：亏损单%d笔中有%d笔（%s）盘中曾浮盈>0.1%%仍以亏损平仓（由盈转亏/回吐线索）；"
            "盈利单平均盘中浮亏 %s（能扛住的正常回撤）。"
            % (
                exsum["n_loss"],
                exsum["loss_once_green"],
...
def build_report(
    ...
    *,
    ...
    L.append(
        "输入：成交=%s（%d笔）；权益=%s（%d点）；盘中MFE/MAE=%s"
        % (
            trades_path or "默认",
            len(trades),
            equity_path or "无",
            len(equity or []),
            (
                "开启%dm重放 覆盖率%s" % (bar_period, _pct(exmeta["coverage"]))
                if exmeta
                else "关闭（加 --bars 开启）"
            ),
        )
    ...
    L.append(
        "口径声明：分钟回测成交为 bar 内规则假设（开盘成交/触及止损止盈/日终强平），非真实盘口队列；"
        "净盈亏已含开/平费与平今腿；纸面vs真实成交一致性（G30②，需G14盘口）本轮未做。"
    )
    L.append("=" * 96)
    if not trades:
        L.append(
            "无成交记录（文件缺失或0笔），安全降级：不出分桶。请先运行 portfolio.py 生成 portfolio_trades.csv。"
        )
        return "\n".join(L) + "\n"
    ...
    L.append(
        "  总笔数 %d（多 %d / 空 %d / 强平 %d）；胜率 %s；期望 %s/笔；盈亏比 %s；利润因子PF %s"
        % (
            overall["n"],
            sum(1 for t in trades if t["dir"] == "多"),
            sum(1 for t in trades if t["dir"] == "空"),
            sum(1 for t in trades if t["forced"]),
            _pct(overall["win_rate"]),
            _money(overall["expectancy"]),
            _f2(overall["payoff_ratio"]),
            _f2(overall["profit_factor"]),
        )
    )
    L.append(
        "  净盈亏合计 %s（毛 %s）；总费用 %s（占|毛| %s）；均持仓 %.1f bar；最长连胜 %d / 最长连亏 %d；最佳 %s / 最差 %s"
        % (
            _money(sum(t["net_yuan"] for t in trades)),
            _money(gross_all),
            _money(fees_all),
    ...
    if equity:
        ...
        if eqs:
            ...
            L.append(
                f"  权益曲线：期初 {eqs[0]:,.0f} → 期末 {eqs[-1]:,.0f}；区间峰值 {peak:,.0f}；最大回撤 {_pct(dd)}。"
            )
    L.append("=" * 96)
    ...
    L += _bucket_lines(
        "平仓原因组", bucket_table(trades, lambda t: reason_group(t["reason"])), key_w=18
    )
    L += _bucket_lines("平今/平昨", bucket_table(trades, lambda t: t["leg"] or "未知"), key_w=10)
    L += _bucket_lines(
        "信号强度(|入场分|,多空同档)",
        bucket_table(trades, lambda t: score_band(t["entry_score"])),
        key_w=22,
    )
    L += _bucket_lines(
        "持仓时长档(bar)", bucket_table(trades, lambda t: hold_band(t["hold_bars"])), key_w=14
    )
    L.append("=" * 96)
    ...
    if review in ("daily", "both"):
        ...
        L.append(
            "  %-12s %5s %6s %12s %12s %10s" % ("日", "笔数", "胜数", "净盈亏", "毛盈亏", "费用")
        )
        for r in rows:
            L.append(
                "  %-12s %5d %6d %12s %12s %10s"
                % (
                    r["key"],
                    r["n"],
                    r["win"],
        ...
        L.append(
            "  合计 %d 个交易日，盈利日 %d（%s），最佳日 %s / 最差日 %s。"
            % (
                len(rows),
                win_days,
                _pct(win_days / len(rows)) if rows else "—",
    ...
    if review in ("weekly", "both"):
        ...
        L.append(
            "  %-10s %5s %6s %12s %12s %10s" % ("周", "笔数", "胜数", "净盈亏", "毛盈亏", "费用")
        )
        for r in rows:
            L.append(
                "  %-10s %5d %6d %12s %12s %10s"
                % (
                    r["key"],
                    r["n"],
                    r["win"],
    ...
    if exsum:
        L.append(
            "五、盘中 MFE/MAE（h/l 重放，正小数；覆盖率 %d/%d=%s，含入场根全根极值为保守上界）"
            % (exsum["n"], len(trades), _pct(exsum["n"] / len(trades)))
        )
        L.append(
            "  全体：平均MFE %s / 平均MAE %s（比值%s，>1 说明持仓体验顺）。"
            % (
                _pct(exsum["avg_mfe"], 2),
                _pct(exsum["avg_mae"], 2),
                _f2(exsum["avg_mfe"] / exsum["avg_mae"], 2) if exsum["avg_mae"] else "—",
            )
        )
        L.append(
            "  盈利单：平均MFE %s / 平均MAE %s；亏损单：平均MFE %s / 平均MAE %s。"
            % (
                _pct(exsum["avg_mfe_win"], 2),
                _pct(exsum["avg_mae_win"], 2),
                _pct(exsum["avg_mfe_loss"], 2),
                _pct(exsum["avg_mae_loss"], 2),
            )
        )
        L.append("=" * 96)
...
def _trade_line(t):
    return "  %-4s %-2s %2d手 %s→%s 持%3dbar 净%10s [%s|入场分%s|%s]" % (
        ...
        _money(t["net_yuan"]),
        t["reason"],
        _f2(t["entry_score"], 2) if t["entry_score"] is not None else "—",
        t["leg"],
    )
...
def run(argv=None):
    ...
    ap.add_argument(
        "--equity", default=DEFAULT_EQUITY, help="portfolio_equity.csv 路径（可选，传空字符串关闭）"
    )
    ...
    if args.bars and trades:
        exmeta = attach_excursions(
            trades, period=args.period, lookback=args.lookback, aggregate_from=args.aggregate_from
        )
    ...
    try:
        ...
        j_metrics = {
            ...
            "payoff_ratio": ov.get("payoff_ratio"),
...
def _mk(
    ...
    *,
    ...
    return {
        ...
        "dir": d,
        "lots": 1,
        "entry_dt": parse_dt(entry),
        "exit_dt": parse_dt(exit_),
        "entry_px": entry_px,
        "exit_px": entry_px,
        "leg": leg,
        "hold_bars": hold,
        "gross_yuan": net + fees if gross is None else gross,
        "open_fee_yuan": fees / 2,
        "close_fee_yuan": fees / 2,
        "fee_yuan": fees,
        "net_yuan": net,
        "reason": reason,
        "forced": "强平" in reason,
...
def selftest():
    ...
    ts = [
        ...
        _mk(
            "RB", "黑色", "多", -50, reason="反向信号", score=2.5, hold=20, exit_="2026-01-06 15:00"
        ),
        _mk(
            "MA",
            "能源化工",
    ...
    # 手工挂 mfe 验证汇总
    ts2 = [
        _mk("A", "x", "多", 10, exit_="2026-01-02 15:00"),
