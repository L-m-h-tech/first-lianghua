    ...
    try:
        ...
        for table, col in DATE_COLUMN.items():
            ...
            if col not in cols:
                continue
            dst.execute(
                f"CREATE TABLE {table} AS SELECT * FROM src.{table} WHERE substr({col},1,4)=?",
                (year,),
            )
    ...
    try:
        ...
        for table, col in DATE_COLUMN.items():
            ...
            if col not in cols:
                continue
            rows = src.execute(
                f"SELECT * FROM {table} WHERE substr({col},1,4)=? ORDER BY 1", (year,)
            ).fetchall()
...
def _selftest():
    ...
    c.executemany(
        ...
        [
            ("2025-12-31 23:00:00", "RB0", 100),
            ("2026-01-02 10:00:00", "RB0", 101),
            ("2026-06-01 10:00:00", "MA0", 99),
            ("2027-01-01 10:00:00", "RB0", 102),
        ],
    )
    c.executemany(
    ...
    assert (
        cn["quotes"] == 2
        and cn["minute_bars"] == 1
        and os.path.exists(os.path.join(csvdir, "quotes_2026.csv"))
    ), cn
