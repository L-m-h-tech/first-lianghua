...
def build_pathology(trades, bars_by_sym=None, post_bars=8, period_min=30):
    ...
    r1_trades = [t for t in short if t.get("entry_score") is not None and abs(t["entry_score"]) < 3]
    r1_n = len(r1_trades)
    ...
    for t in short:
        legs[
            "平今"
            if "平今" in (t["leg"] or "")
            else ("平昨" if "平昨" in (t["leg"] or "") else "其他")
        ].append(t)
    leg_rows = {k: agg(v) for k, v in legs.items()}
    fee_ratio_short = (
        sum(t["fee_yuan"] for t in short) / abs(sum(t["gross_yuan"] for t in short))
        if short and abs(sum(t["gross_yuan"] for t in short)) > 0
        else None
    )
    fee_ratio_mid = (
        sum(t["fee_yuan"] for t in mid) / abs(sum(t["gross_yuan"] for t in mid))
        if mid and abs(sum(t["gross_yuan"] for t in mid)) > 0
        else None
    )
    ...
    return {
        ...
        "r1_score_rows": score_rows,
        "r1_weak_n": r1_n,
        "r1_weak_net": round(r1_net, 0),
        "r3_reverse": rev,
...
def render(res):
    ...
    L.append(
        "1-2bar 桶: %d 笔 / 净 %+.0f / 胜率 %.0f%% / PF %s ｜ 对照 3-6bar 桶: %d 笔 / 净 %+.0f / PF %s"
        % (
            so["n"],
            so["net"],
            (so["win_rate"] or 0) * 100,
            ("%.2f" % so["pf"])
            if so["pf"] and so["pf"] < 999
            else ("∞" if so["pf"] == 999.0 else "—"),
            mo["n"],
            mo["net"],
            ("%.2f" % mo["pf"]) if mo["pf"] else "—",
        )
    )
    L.append("")
    ...
    for k, a in res["reason_rows"].items():
        L.append(
            "   %-8s %5d 笔  净 %+10.0f  胜率 %5.1f%%"
            % (k, a["n"], a["net"], (a["win_rate"] or 0) * 100)
        )
    r2 = res["r2_stop"]
    L.append("")
    L.append(
        "② 止损单病理（R2 核心证据，n=%d，其中 %d 笔有分钟库覆盖）："
        % (r2["n_stops"], r2["n_with_bars"])
    )
    L.append(
        "   止损距离中位数 %.2f%% ｜ 出场后 4 根内有利回摆中位数 %.2f%%"
        % ((r2["stop_dist_median"] or 0) * 100, (r2["post_fav4_median"] or 0) * 100)
    )
    L.append(
        "   whipsaw 率（回摆≥止损距离）: 4根 %.0f%% ｜ 8根 %s"
        % (
            (r2["whipsaw_rate_4bar"] or 0) * 100,
            ("%.0f%%" % (r2["whipsaw_rate_8bar"] * 100))
            if r2.get("whipsaw_rate_8bar") is not None
            else "—",
        )
    )
    ...
    L.append("")
    L.append(
        "③ R1 预登记（入场门槛 2→3）：1-2bar 桶内 |分|<3 的笔数 %d / 净 %+0.f"
        % (res["r1_weak_n"], res["r1_weak_net"])
    )
    for k, a in res["r1_score_rows"].items():
        L.append(
            "   %-14s %5d 笔  净 %+10.0f  PF %s"
            % (
                k,
                a["n"],
                a["net"],
                ("%.2f" % a["pf"])
                if a["pf"] and a["pf"] < 999
                else ("∞" if a["pf"] == 999.0 else "—"),
            )
        )
    L.append("   验收标准（预登记）：双样本 PF≥1.05 且 笔数下降≥40%")
    L.append("")
    L.append(
        "④ R3 预登记（反向信号只平不反手）：%d 笔 / 净 %+0.f"
        % (res["r3_reverse"]["n"], res["r3_reverse"]["net"])
    )
    L.append("   验收标准（预登记）：反向桶亏损收窄≥50%，其余桶不变差")
    L.append("")
    L.append(
        "⑤ R4 预登记（平今税）：1-2bar 桶 费用/|毛利| = %s vs 3-6bar 桶 %s"
        % (
            ("%.1f%%" % (res["fee_ratio_short"] * 100))
            if res["fee_ratio_short"] is not None
            else "—",
    ...
    L.append("")
    L.append(
        "（只读切片：候选规则进 Phase2 双样本影子后才允许动 config；本报告即预登记文档，先定标准防事后挑数）"
    )
    return "\n".join(L)
...
def selftest():
    ...
    trades = [
        # 1-2bar 止损多单：入场 100 出 99（-1%），出场后 4 根回摆 +1.5%（whipsaw）
        {
            ...
            "entry_px": 100.0,
            "exit_px": 99.0,
            "entry_dt": __import__("datetime").datetime(2026, 9, 1, 9, 31),
            "exit_dt": __import__("datetime").datetime(2026, 9, 1, 10, 1),
            "leg": "平今",
            "hold_bars": 2,
            "gross_yuan": -100.0,
            "open_fee_yuan": 3,
        ...
        # 1-2bar 止损空单：无回摆（继续下行 2%）——正确止损
        {
            ...
            "entry_px": 2500.0,
            "exit_px": 2525.0,
            "entry_dt": __import__("datetime").datetime(2026, 9, 2, 9, 31),
            "exit_dt": __import__("datetime").datetime(2026, 9, 2, 10, 1),
            "leg": "平今",
            "hold_bars": 2,
            "gross_yuan": -250.0,
            "open_fee_yuan": 2.5,
        ...
        # 1-2bar 反向信号
        {
            ...
            "entry_px": 70000.0,
            "exit_px": 70050.0,
            "entry_dt": __import__("datetime").datetime(2026, 9, 3, 9, 31),
            "exit_dt": __import__("datetime").datetime(2026, 9, 3, 10, 1),
            "leg": "平今",
            "hold_bars": 1,
            "gross_yuan": 50.0,
            "open_fee_yuan": 12,
        ...
        # 3-6bar 对照（止盈）
        {
            ...
            "entry_px": 100.0,
            "exit_px": 105.0,
            "entry_dt": __import__("datetime").datetime(2026, 9, 5, 9, 31),
            "exit_dt": __import__("datetime").datetime(2026, 9, 7, 14, 31),
            "leg": "平昨",
            "hold_bars": 5,
            "gross_yuan": 500.0,
            "open_fee_yuan": 3,
    ...
    for i in range(4):  # RB 止损后 4 根：h=101.5（回摆 +1.5%）→ whipsaw
        ...
        bars["RB"].append(
            {"dt": t, "o": 99.0, "h": 101.5, "l": 98.5, "c": 100.0, "v": 1, "amount": 1}
        )
    t0 = datetime(2026, 9, 2, 10, 2)
    for i in range(4):  # MA 止损后继续下行（l 更低）→ 无回摆
        t = t0 + timedelta(minutes=30 * i)
        bars["MA"].append(
            {"dt": t, "o": 2525.0, "h": 2526.0, "l": 2510.0, "c": 2512.0, "v": 1, "amount": 1}
        )
    ...
    assert (
        res["r1_weak_n"] == 1 and abs(res["r1_weak_net"] + 106.0) < 1
    )  # |分|<3 仅 RB 一笔（CU 3.2 不在口径内）
...
def main(argv=None):
    ...
    ap.add_argument(
        "--no-bars", action="store_true", dest="no_bars", help="跳过分钟库回放（无 whipsaw 切片）"
    )
    ...
    res = build_pathology(
        trades, bars_by_sym=bars_by_sym, post_bars=args.post_bars, period_min=args.period
    )
    txt = render(res)
    os.makedirs(os.path.join(_ROOT, "reports"), exist_ok=True)
    with open(
        os.path.join(_ROOT, "reports", "short_trade_pathology.txt"), "w", encoding="utf-8"
    ) as f:
        f.write(txt + "\n")
    with open(
        os.path.join(_ROOT, "reports", "short_trade_pathology.json"), "w", encoding="utf-8"
    ) as f:
