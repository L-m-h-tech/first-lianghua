...
def _conn():
    ...
    con.execute(
        "CREATE TABLE IF NOT EXISTS tushare_cal("
...
def _upsert(con, table, cols, rows):
    ...
    if not rows:
        return 0
    ph = ",".join("?" * len(cols))
    con.executemany("INSERT OR REPLACE INTO %s(%s) VALUES(%s)" % (table, ",".join(cols), ph), rows)
    con.commit()
    return len(rows)
...
def harvest_cal(con):
    ...
    if not rows:
        return 0
    data = [
        (r.get("exchange"), r.get("cal_date"), int(r.get("is_open") or 0), r.get("pretrade_date"))
        for r in rows
        if r.get("cal_date") >= CAL_START
    ]
...
def run(verbose=True):
    con = _conn()
    counts = {"tushare_cal": harvest_cal(con), "fut_basic": harvest_basic(con)}
