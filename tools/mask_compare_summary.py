...
def _pf_line(pf):
    ...
    return "期数%d/净t%+.2f/净均收%+.3f%%/胜率%.0f%%/单调%.0f%%/Q5-Q1%+.3f" % (
        pf.get("n_periods", 0),
        pf.get("net_t", 0.0) if pf.get("net_t") is not None else 0.0,
        (pf.get("net_mean") or 0.0) * 100,
        (pf.get("win") or 0.0) * 100,
...
def _extract(path, key):
    ...
    try:
        ...
        raw_pf = {
            ...
            "net_t": (raw.get("pf") or {}).get("net_t"),
            "net_mean": (raw.get("pf") or {}).get("net_mean"),
            "win": (raw.get("pf") or {}).get("win"),
            "bands": raw.get("bands"),
        }
        mask_pf = {
            "n_periods": mask.get("n_periods"),
            "net_t": (mask.get("pf") or {}).get("net_t"),
...
def build_report():
    ...
    if carry:
        ...
        L.append(
            "  （缺 carry_eval.json 的 mask_compare，先跑 tools/carry_eval.py --mask-compare）"
        )
    L.append("")
    ...
    if xs:
        ...
        L.append(
            "  （缺 xsmom_eval.json 的 mask_compare_xs，先跑 tools/xsmom_eval.py --mask-compare）"
        )
    L.append("")
    L.append("【诚实结论】")
    L.append("  两工具一致：日线级疑似锁板几乎不出现（全样本仅个位数）；“临近交割”剔除（15天窗口）")
    L.append(
        "  会砍掉约一半样本、主窗（1023交易日上限）下掩码后期数减半、绩效波动大（净t/单调不稳定）——"
    )
    L.append(
        "  掩码价值在日线级在于防锁板，交割剔除主要作用是降样本量而非改善信号；研究侧如实记录，不进综合分。"
    )
    L.append("=" * 104)
    return "\n".join(L)
...
def _mk_fake(path, key):
    ...
    d = {
        key: {
            "raw": {
                "n_periods": 12,
                "pf": {"net_t": 1.36, "net_mean": 0.0235, "win": 0.67},
                "bands": {"mono": 0.75, "spread": 0.0241},
            },
            "mask": {
                "n_periods": 6,
                "pf": {"net_t": 0.95, "net_mean": 0.0330, "win": 0.67},
