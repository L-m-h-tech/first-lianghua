        ...
        with open(path, encoding="utf-8-sig", newline="") as fh:
            ...
            for row in rd:
                ...
                if not sym:
                    continue
                o = out.setdefault(
                    sym, {"name": row.get("name", ""), "margin_mult": None, "fee_mult": None}
                )
                try:
...
def render(dev, local, rows):
    ...
    lines = [
        "合约元数据校准（装置 ctp_instruments vs 量化本地配置表）",
        "=" * 60,
        "生成: %s | ctp_instruments %d 条 / 本地表 %d 条" % (now, len(dev), len(local)),
        "",
    ]
    ...
    lines.append(
        "可对照 %d 品种｜一致 %d｜不一致 %d｜仅装置 %d｜仅本地 %d"
    ...
    for r in rows:
        st = (
            ...
            if r["match"] is True
                ...
                "不一致"
                if r["match"] is False
                else ("仅装置" if r["device_mult"] is not None else "仅本地")
            )
        )
        lines.append(
            "  %-8s %-10s %10s %10s %s"
    ...
    lines.append("")
    lines.append(
        "说明：ctp_instruments 由界面操作收集装置从本地 CSV seed + Legend 采集补充，"
...
def selftest():
    ...
    conn.execute(
        "CREATE TABLE ctp_instruments(instrument_id TEXT, volume_multiple REAL,"
        " instrument_name TEXT, source TEXT)"
    )
    conn.executemany(
        "INSERT INTO ctp_instruments VALUES(?,?,?,?)",
        [
            ("RB", 10.0, "螺纹钢", "seed"),
            ("CU", 5.0, "铜", "seed"),
            ("AG", 15.0, "白银", "seed"),
    ...
    local = {
        "RB": {"margin_mult": 10.0, "fee_mult": 10.0, "name": "螺纹钢"},
...
def main(argv=None):
    ...
    print(
        "contract_calibrate: ctp_instruments %d / 可对照 %d / 不一致 %d → %s"
