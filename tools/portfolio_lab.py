...
def dense_matrix(
    return_map, analysis_days=ANALYSIS_DAYS, coverage_min=COVERAGE_MIN, fill_missing=False
):
...
def rolling_proxy(mat, methods=None, lookback=None, rebal=None, shrink=None, cap=None):
    ...
    while t < T:
        ...
        for m in methods:
            ...
            prev_w[m] = w
        for m in methods:
            out[m]["seg_bounds"].append(
                {"start": t, "length": hold_end - t, "entry_turnover": seg_tv[m]}
            )
...
DEFAULT_ONEWAY_COST = (
    1.5e-4  # 单边调仓成本率（费5bp+滑点10bp 的数量级，与 wf_cost_lab 基准同档，可参数化）
)
...
def gross_cost_grid(
    proxy,
    gross_list=DEFAULT_GROSS_GRID,
    one_way_cost=DEFAULT_ONEWAY_COST,
    methods=None,
            ...
            rows.append(
                {
                    ...
                    "ann_ret_net": st_net.get("ann_ret"),
                    "ann_vol_net": st_net.get("ann_vol"),
                    "sharpe_net": st_net.get("sharpe"),
                    "maxdd_net": st_net.get("maxdd"),
                    "ann_ret_gross": st_gross.get("ann_ret"),
                    "sharpe_gross": st_gross.get("sharpe"),
                    "maxdd_gross": st_gross.get("maxdd"),
                    "ann_cost_drag": ann_charge,
...
def drawdown_window(nav, idxs=None, dates=None):
    ...
    res = {"maxdd": 0.0, "peak_i": None, "trough_i": None, "peak_date": None, "trough_date": None}
    if not nav:
        return res
    peak = nav[0]
    peak_k = 0
...
def latest_snapshot(mat, syms, sectors, methods=None):
    ...
    for m in methods:
        ...
        top = [
            {"sym": syms[i], "sector": sectors.get(syms[i]), "w": r["weights"][i]}
            for i in order[:8]
            if r["weights"][i] > 1e-6
        ]
        # 目标波动缩放演示（不改核心权重）
        _, lev, ann_pre = pc.target_vol_scale(
            r["weights"],
            r["cov"],
            config.PC_TARGET_VOL_ANNUAL,
            config.PC_PERIODS_PER_YEAR,
            config.PC_MAX_GROSS,
        )
        snap[m] = {
            "ann_vol": r["ann_vol"],
            "eff_n": r["eff_n"],
            "div_ratio": r["div_ratio"],
            "max_rc": max(r["rc_frac"]),
            "gross": r["gross"],
            "top": top,
            "target_vol_leverage": lev,
            "ann_vol_before_scale": ann_pre,
        }
    return snap
...
def run(db_path=DEFAULT_DB, txt_path=None, json_path=None, verbose=True, mask=False):
        ...
        try:
            ...
            if os.path.exists(db_p):
                ...
                removed = sum(
                    ...
                    for sym, dm in return_map.items()
                    for dt in dm
                    if not (mask.get(sym) or {}).get(dt, {}).get("tradable", True)
                )
    ...
    for m in config.PC_METHODS:
        ...
        nav_summary[m] = {
            ...
            "maxdd_nav": dw["maxdd"],
            "dd_peak": dw["peak_date"],
            "dd_trough": dw["trough_date"],
        }
    for m in config.PC_METHODS:
        tv = proxy[m]["turnover"]
        en = proxy[m]["eff_n"]
        stats[m]["avg_turnover_rebal"] = sum(tv) / len(tv) if tv else None
        stats[m]["ann_turnover"] = (
            (sum(tv) / len(tv) * config.PC_PERIODS_PER_YEAR / config.PC_REBAL) if tv else None
        )
    ...
    L.append("=" * 104)
    L.append(
        "G26 组合构建实验台 portfolio_lab（纯离线读 G21 面板，风险型权重、不预测收益、不接 main 不改综合分/sizing）"
    )
    L.append(
        "固定宇宙=%d/%d 品种（最近%d日覆盖率≥%.0f%%），稠密区间 %s~%s 共%d日；协方差窗%d、再平衡每%d日、对角收缩%.2f、单票上限%.0f%%"
        % (
            len(syms),
            len(all_syms),
            ANALYSIS_DAYS,
            ...
            config.PC_REBAL,
            config.PC_SHRINK,
            config.PC_MAX_WEIGHT * 100,
        )
    )
    L.append("-" * 104)
    L.append(
        "【一】滚动样本外代理回测（每%d日用过去%d日定权持有，equal 等权为基线；无成本、多头、不使用未来）"
        % (config.PC_REBAL, config.PC_LOOKBACK)
    )
    head = "  %-8s %8s %8s %7s %8s %8s %9s %9s" % (
        "方法",
        "年化收益",
        "年化波动",
        "夏普",
    ...
    for m in config.PC_METHODS:
        s = stats[m]
        L.append(
            "  %-8s %+7.2f%% %7.2f%% %7.2f %7.2f%% %8.2f %9.2f %8.2f%s"
            % (
                name[m],
                s["ann_ret"] * 100,
                s["ann_vol"] * 100,
                ...
                "  ←基线"
                if m == "equal"
                else "  波动较等权%+.1f%%" % ((s["ann_vol"] / base_vol - 1) * 100),
            )
        )
    nav_line = "  期末净值(初始1.0)：" + "  ".join(
        "%s=%.4f" % (name[m], nav_summary[m]["end_nav"])
        for m in config.PC_METHODS
    ...
    L.append(dd_line)
    L.append(
        "  逐日组合历史净值曲线（四方法对齐）已落 reports/portfolio_nav.csv（date+各法日收益/净值）。"
    )
    L.append(
        "  读法：风险型分配的价值应体现在'波动/回撤更低、夏普不更差'，而非收益更高（它不预测涨跌）；换手越大调仓成本越高。"
    )
    L.append("-" * 104)
    L.append(
        "【二】最新快照（过去%d日协方差；目标年化波动%.0f%%所需杠杆，受总敞口%.1f倍上限）"
    ...
    for m in config.PC_METHODS:
        ...
        L.append(
            "  ● %s：年化波动%.2f%%、有效N%.1f、分散化度%.2f、最大单品种风险贡献%.1f%%、目标波动杠杆%.2f"
            % (
                name[m],
                s["ann_vol"] * 100,
                s["eff_n"],
    ...
    L.append(
        "【三】总敞口 gross 网格 × 换手成本影子（固定宇宙%d品种；同一权重轨迹按总敞口线性放大，"
        "再平衡段首日扣单边换手成本，单边费率%.1fbp；多头线性、不预测收益）"
        % (len(syms), DEFAULT_ONEWAY_COST * 1e4)
    )
    L.append(
        "  %-8s %5s %9s %9s %8s %9s %9s %9s %10s"
        % (
            "方法",
            "gross",
            "年化净",
    ...
    for m in config.PC_METHODS:
        for cell in gross_grid[m]:
            L.append(
                "  %-8s %4.1fx %+8.2f%% %+8.2f%% %7.2f%% %8.2f %8.2f %8.2f%% %9.2f%%"
                % (
                    name[m],
                    cell["gross"],
                    (cell["ann_ret_net"] or 0) * 100,
                    (cell["ann_ret_gross"] or 0) * 100,
                    (cell["ann_vol_net"] or 0) * 100,
                    cell["sharpe_net"] or 0,
                    cell["sharpe_gross"] or 0,
                    (cell["maxdd_net"] or 0) * 100,
                    cell["ann_cost_drag"] * 100,
                )
            )
    L.append(
        "  ─ 全%d品种口径（缺失日收益按0=当日无敞口，覆盖率稳健性对照）列同上、单边%.1fbp，三档 gross 全跑："
    ...
    L.append(
        "  逐日组合净值已含四方法 gross=1 口径（reports/portfolio_nav.csv）；本网格明细落 reports/portfolio_gross_grid.csv。"
    )
    L.append(
        "  读法：gross 等比放大收益与波动，夏普(毛)理论上不变、净夏普随换手成本上升而下降；回撤随 gross 线性放大，杠杆只改风险预算不产生 alpha。"
    )
    L.append("-" * 104)
    L.append(
        "诚实边界：日收益来自已比例复权主连面板、固定宇宙有幸存者偏差、代理未计手续费/滑点/保证金与换月、"
    ...
    with open(nav_path, "w", encoding="utf-8", newline="") as fp:
        ...
        wcsv.writerow(
            ["date"]
            + [m + "_ret" for m in config.PC_METHODS]
    ...
    payload = {
        ...
        "nav_csv": os.path.basename(nav_path),
        "snapshot": snap,
        "gross_grid": gross_grid,
        "gross_one_way_cost": DEFAULT_ONEWAY_COST,
        "gross_grid_csv": os.path.basename(gross_path),
        "all_universe": {
            "n": len(syms_all),
            "dates": [dates_all[0], dates_all[-1]],
    ...
    try:
        ...
        el.safe_record(
            "portfolio_lab",
            {
                ...
                "lookback": config.PC_LOOKBACK,
                "rebal": config.PC_REBAL,
                "shrink": config.PC_SHRINK,
                "max_weight": config.PC_MAX_WEIGHT,
                "target_vol_annual": config.PC_TARGET_VOL_ANNUAL,
                "max_gross": config.PC_MAX_GROSS,
                "methods": list(config.PC_METHODS),
                "panel_db": os.path.basename(db_path),
                "gross_grid": list(gross_list),
                "one_way_cost": DEFAULT_ONEWAY_COST,
                "all_universe_n": len(syms_all),
            },
            lab_metrics,
            inputs=[db_path],
            artifacts=[txt_path, json_path, nav_path, gross_path],
            conclusion="equal夏普%.2f / erc夏普%.2f / inv_vol夏普%.2f（固定宇宙%d品种 %s~%s）"
            % (
                stats["equal"]["sharpe"],
                stats["erc"]["sharpe"],
                stats["inv_vol"]["sharpe"],
                len(syms),
                dates[0],
                dates[-1],
            ),
...
def selftest():
    ...
    for m in proxy:
        assert (
            len(navs_m := nav_curve(proxy[m]["daily"]))
            == len(proxy[m]["daily"])
            == len(proxy[m]["idx"])
        )
        assert (
            proxy[m]["idx"][0] == 60 and proxy[m]["idx"][-1] == 259
        )  # 首个再平衡在 lookback 之后、无未来
    ...
    bounds = [
        {"start": 60, "length": 2, "entry_turnover": 0.5},
        {"start": 62, "length": 2, "entry_turnover": 0.2},
    ]
    n0, c0 = gross_net_daily(daily, bounds, gross=1.0, one_way_cost=0.0)
    assert all(abs(a - b) < 1e-15 for a, b in zip(n0, daily, strict=False)) and sum(c0) == 0
    ...
    mask = {
        "S0": {dt: {"tradable": dt != "2025-260"} for dt in rm["S0"]},
    ...
    print(
        "portfolio_lab selftest ALL PASS（稠密面板对齐/固定宇宙覆盖率筛选/滚动样本外无未来且GMV波动≤等权/"
...
def main(argv=None):
    ...
    ap.add_argument(
        "--mask",
        action="store_true",
        help="G22续：读 research_panel.db 算可交易性掩码并剔除不可交易日后重做组合实验",
    )
