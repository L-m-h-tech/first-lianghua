...
def analyze(
    db_path=os.path.join(_ROOT, "cache", "research_panel.db"),
    ...
    meta = {
        "n_universe": len(syms),
        "n_all": len(all_syms),
        "window": [dates[lo], dates[-1]],
        "n_days": len(rab[0]),
        "oil": OIL_SYM if oil_idx is not None else None,
        "lookback": lookback,
        "shrink": config.PC_SHRINK,
        "cap": config.PC_MAX_WEIGHT,
    }
...
def render(meta, per):
    ...
    L.append("=" * 108)
    L.append(
        "G5 组合风险实验台 portfolio_risk_lab（纯离线读 G21 面板；只读研究，不接 main、不改 risk_gate/综合分/sizing/持仓）"
    )
    L.append(
        "固定宇宙=%d/%d 品种，风险窗 %s~%s 共%d日；协方差对角收缩%.2f、单票上限%.0f%%、满仓多头权重和=1（未加杠杆）；VaR/ES 损失为正"
        % (
            meta["n_universe"],
            meta["n_all"],
            meta["window"][0],
    ...
    L.append(
        "  平均绝对相关 %.3f（系统性联动强度，越接近1越难分散）、平均带符号相关 %+.3f"
    ...
    L.append(
        "  最强相关对 Top8："
        + "、".join("%s-%s=%.2f" % (a, b, r) for a, b, r in eq["strongest_pairs"])
    )
    L.append(
        "  最弱相关对 Top8："
        + "、".join("%s-%s=%.2f" % (a, b, r) for a, b, r in eq["weakest_pairs"])
    )
    L.append("-" * 108)
    ...
    for m in METHODS:
        ...
        L.append(
            "  %-8s %8s %9s %9s %9s %9s %9s %9s %+7.0f%% %8.1f%s"
            % (
                METHOD_CN[m],
                _pct(s["param"]["ann_vol"]),
                _pct(h95["var"]),
                _pct(h95["es"]),
                _pct(h99["var"]),
                _pct(p95[1]),
    ...
    L.append(
        "  读法：历史VaR>参数VaR（肥尾溢价为正）说明真实左尾比正态更厚、参数法会低估风险；ES≥VaR 是条件尾部平均损失；"
    )
    L.append(
        "        风险型权重(inv_vol/erc/gmv)的价值应体现为 VaR/ES 与年化波动更低，而非收益更高。"
    )
    L.append(
        "  分散化收益（1−组合参数VaR/加权单体VaR）："
    ...
    # 【三】原油压力
    L.append(
        "【三】原油（%s）压力情景：各品种对原油日收益 OLS beta，冲击经 beta 线性一阶传导（忽略非线性/相关突变）"
        % meta["oil"]
    )
    if "oil_stress" not in eq:
        L.append(
            "  固定宇宙中未找到 %s，跳过原油压力（如需可改 OIL_SYM 为能源代理品种）。" % OIL_SYM
        )
    else:
        # beta 最强的品种
        betas = eq["oil_betas"]
        ...
        L.append(
            "  等权组合原油−5% 主要拖累："
            + "、".join(
                "%s(beta%.2f)=%+.2f%%" % (t["sym"], t["beta"], t["pnl"] * 100)
                for t in d5["top"][:6]
            )
        )
    L.append("-" * 108)
    L.append(
        "诚实边界：日收益来自比例复权主连面板、固定宇宙有幸存者偏差；历史法窗仅%d日、极端分位样本少；"
        "参数法假设正态且多日用√h(i.i.d.)；原油压力为线性一阶、beta 用历史窗不代表危机时联动；"
        "本结果只用于研究'组合层风险有多大、风险型权重能否降尾部风险'，不直接产生任何减仓/熔断动作。"
        % meta["n_days"]
    )
    return "\n".join(L)
...
def selftest():
    ...
    print(
        "portfolio_risk_lab selftest ALL PASS（窗口切片/超长窗取全/四方案渲染与压力段不崩 共3组）"
    )
    return 0
