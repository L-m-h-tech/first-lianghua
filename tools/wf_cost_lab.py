...
DEFAULT_CODES = (
    ...
    "I",
    "TA",
)  # 默认代表品种（流动性好、覆盖黑色/化工），--codes/--all 覆盖
...
def build_cost_surface(runner, fee_grid=DEFAULT_FEE_GRID, slip_grid=DEFAULT_SLIP_GRID):
    ...
    fee_grid = tuple(fee_grid)
    slip_grid = tuple(slip_grid)
    rows = []
    for fee in fee_grid:
        cells = []
    ...
    base = {
        "fee": BASE_FEE,
        "slip": BASE_SLIP,
        "fi": _nearest_idx(fee_grid, BASE_FEE),
        "si": _nearest_idx(slip_grid, BASE_SLIP),
    }
...
def breakeven_cost(surface):
    ...
    out["fee"] = _breakeven_axis(
        [surface["fee_grid"][i] for i in range(len(comp))],
...
def _breakeven_axis(grid, vals, base_val):
    first_neg = None
    for g, v in zip(grid, vals, strict=False):
...
def wf_stability(segments, names):
    ...
    chosen_names = [
        names[i] if (i is not None and 0 <= i < len(names)) else "?" for i in chosen_idx
    ]
    # 每个候选被选次数
    votes = {}
    ...
    switches = sum(1 for a, b in zip(chosen_idx, chosen_idx[1:], strict=False) if a != b)
    ...
    mean_is = _mean(is_list)
    mean_oos = _mean(oos_list)
    mean_best = _mean(best_list)
    ...
    return {
        ...
        "mean_oos_best": mean_best,
        "is_oos_decay": decay,
        "selection_regret": regret,
        "oos_positive_rate": (oos_pos / k) if k else None,
        "beat_median_rate": (len(beat) / k) if k else None,
        "grade": grade,
    }


# =========================== 换手与容量（数量级估算） ===========================
def estimate_turnover_capacity(
    bars,
    trades,
    multiplier,
    ...
    max_lots = (
        (max_notional / notional_per_lot / trades_per_day)
        if (notional_per_lot > 0 and trades_per_day > 0)
        else None
    )
    max_capital_notional = (max_lots * notional_per_lot) if max_lots is not None else None
    return {
        "n_days": n_days,
        "mkt_daily_notional": mkt_daily_notional,
        "n_trades": n,
        "trades_per_day": trades_per_day,
        "notional_per_lot": notional_per_lot,
        "strat_daily_notional_1lot": strat_daily_notional_1lot,
        "annual_turnover_lots_1lot": annual_turnover_lots,
        "participation_cap": participation_cap,
        "max_lots_per_trade": max_lots,
        "max_capital_notional": max_capital_notional,
...
def run_symbol(
    ...
    wf_train=20,
    wf_test=10,
    fee_grid=DEFAULT_FEE_GRID,
    slip_grid=DEFAULT_SLIP_GRID,
    participation_cap=DEFAULT_PARTICIPATION_CAP,
    verbose=False,
    purge=0,
    embargo=0,
    ...
    for tr, te in presets:
        wf_x = (
            bv.walk_forward(matrix, tr, te, purge=purge, embargo=embargo)
            if len(matrix) >= tr + embargo + te
            else {"segments": [], "n_segments": 0}
        )
    ...
    return {
        ...
        "period": period,
        "bars": grid["bars"],
        "n_days": len(grid["days"]),
        "n_combos": len(names),
        "wf_train": wf_train,
        "wf_test": wf_test,
        "purge": purge,
        "embargo": embargo,
        "wf_summary": wf_multi[0]["wf_summary"],
        "wf_multi": wf_multi,
        "stability": stab,
        "best_param": best_name,
        "surface": surface,
        "breakeven": be,
        "base_cell": base_cell,
        "capacity": capacity,
...
def render_symbol(res):
    ...
    base = res["base_cell"]
    L.append(
        "◆ %s（%s，%dm，%d根bar/%d交易日，%d组参数网格，WF=%d训/%d测）"
        % (
            res["sym"],
            res.get("name", ""),
            res["period"],
    ...
    L.append(
        "     最常被选：%s（%s 窗次，占比%s）；参数切换 %d 次（切换率%s）"
        % (
            st["top_param"],
            st["votes"].get(st["top_param"], 0),
            _pct(st["top_share"]),
            st["switches"],
            _pct(st["switch_rate"]),
        )
    ...
    L.append(
        "     样本内夏普均值 %s → 样本外 %s（衰减 %s）；OOS为正段占比%s、跑赢OOS中位数%s、选参遗憾%s"
        % (
            _r(st["mean_is_sharpe"], 3),
            _r(st["mean_oos_sharpe"], 3),
            _r(st["is_oos_decay"], 3),
            _pct(st["oos_positive_rate"]),
            _pct(st["beat_median_rate"]),
            _r(st["selection_regret"], 3),
        )
    )
    ...
    if len(multi) > 1 or iso:
        L.append(
            "     多周期/防前视对照（purge=%s、embargo=%s；隔离带越严OOS越可信，评级应不随窗口跳变才算稳）："
            % (res.get("purge", 0), res.get("embargo", 0))
        )
        for mm in multi:
            sx = mm["stability"]
            L.append(
                "       窗%2d训/%2d测：%d段 评级%-4s 最常选%-12s 锚定%s 切换%s OOS夏普%s（IS%s 衰减%s）"
                % (
                    mm["train"],
                    mm["test"],
                    mm["n_segments"],
                    ...
                    _pct(sx["top_share"]),
                    sx["switches"],
                    _r(sx["mean_oos_sharpe"], 3),
                    _r(sx["mean_is_sharpe"], 3),
                    _r(sx["is_oos_decay"], 3),
                )
            )
    # ③ 成本曲面（固定最优参数）
    L.append(
        "  ③ 成本敏感性曲面（固定全样本最优参数 %s；行=每腿费率，列=单边滑点；值=逐笔复利净收益）"
        % res["best_param"]
    )
    surf = res["surface"]
    ...
    L.append(
        "     基准格（*，fee%s/slip%s）：%d笔 胜率%s 笔夏普%s 复利%s；成本/|毛利|=%s"
        % (
            _bp(BASE_FEE),
            _bp(BASE_SLIP),
            base["n_trades"],
            _pct(base["win_rate"]),
            _r(base["per_trade_sharpe"], 3),
            _pct(base["total_compound"], 1),
            _pct(base["cost_to_gross"]),
        )
    )
    for axis, label in (("fee", "每腿费率"), ("slip", "单边滑点")):
        ...
        if b["first_negative"] is None:
            L.append(
                "     沿%s：扫描全程净收益未转负（扛住最高档 %s）" % (label, _bp(max(b["grid"])))
            )
        elif b["safety_x"] == 0:
            L.append("     沿%s：基准档已不盈利，首个转负档 %s" % (label, _bp(b["first_negative"])))
        else:
            L.append(
                "     沿%s：首个净收益转负档 %s ≈ 基准的 %.1f 倍（成本安全垫）"
                % (label, _bp(b["first_negative"]), b["safety_x"])
            )
    # 换手容量
    L.append(
        "     换手/容量（数量级估算，参与率上限%.0f%%）：日均%.2f笔、1手年换手%.0f笔；市场日均名义%s、单手名义%s；"
        % (
            cap["participation_cap"] * 100,
            cap["trades_per_day"],
            cap["annual_turnover_lots_1lot"],
            _money_wan(cap["mkt_daily_notional"]),
            _money_wan(cap["notional_per_lot"]),
        )
    )
    L.append(
        "             参与率上限下可承载每笔约 %.0f 手、对应名义资金约 %s（%s，库内bar覆盖口径）"
        % (
            (cap["max_lots_per_trade"] or 0.0),
            _money_wan(cap["max_capital_notional"]),
            "线性忽略冲击，精确待G14",
        )
    )
    L.append("")
...
def build_report(results, args_codes, period):
    ...
    if results:
        ...
        if r0.get("purge", 0) or r0.get("embargo", 0):
            iso_txt += " ｜ purge=%d embargo=%d（AFML防前视隔离带）" % (
                r0.get("purge", 0),
                r0.get("embargo", 0),
            )
    L = [
    ...
    for res in results:
        st = res["stability"]
        sx_fee = res["breakeven"]["fee"]["safety_x"]
        sx_sl = res["breakeven"]["slip"]["safety_x"]
        L.append(
            "  %-4s 评级%-4s 最优%-12s OOS夏普%s 基准复利%8s 安全垫 %s×%s 日均%.2f笔"
            % (
                res["sym"],
                st["grade"],
                res["best_param"],
                _r(st["mean_oos_sharpe"], 2),
    ...
    L.append(SEP)
    L.append(
        "口径：成本曲面用兜底比例费率模式（每腿 fee_rate、滑点按方向不利偏移），用于相对敏感度/安全垫，非绝对盈亏；"
    )
    L.append(
        "      WF 复用 backtest_validation 同一信号/撮合与滚动 IS选参-OOS验证，无未来函数；容量为线性数量级估算（无盘口深度，精确待 G14）。"
    )
    L.append(
        "      研究侧只读、不接 main、不改综合分/默认CSV；结论只提示风险，调参仍走双样本+影子+默认回退。"
    )
    L.append(SEP)
    return "\n".join(L)
...
def run(argv=None):
    ap = argparse.ArgumentParser(
        description="G27②③ WF参数稳定性+成本敏感性曲面/换手容量（研究侧只读）"
    )
    ap.add_argument(
        "--codes", default=",".join(DEFAULT_CODES), help="逗号分隔品种，默认 RB,MA,I,TA"
    )
    ...
    ap.add_argument(
        "--purge", type=int, default=0, help="IS 尾部 purge 行数（剔除标签跨入OOS的样本）"
    )
    ...
    codes = (
        _all_codes()
        if args.all
        else [c.strip().upper() for c in args.codes.split(",") if c.strip()]
    )
    ...
    presets = [
        (tr_list[min(k, len(tr_list) - 1)], te_list[min(k, len(te_list) - 1)]) for k in range(n_pre)
    ]
    ...
    for sym in codes:
        try:
            res = run_symbol(
                sym,
                period=args.period,
                wf_train=primary_train,
                wf_test=primary_test,
                participation_cap=args.participation,
                purge=args.purge,
        ...
        ov = [
            {
                ...
                "slip_safety_x": r["breakeven"]["slip"]["safety_x"],
            }
            for r in results
        ]
        el.safe_record(
            "wf_cost_lab",
            {
                ...
                "fee_grid": list(DEFAULT_FEE_GRID),
                "slip_grid": list(DEFAULT_SLIP_GRID),
            },
            ...
            % (
                len(results),
                "、".join("%s=%s" % (r["sym"], r["stability"]["grade"]) for r in results),
            ),
        )
    except Exception:
        pass
...
def selftest():
    ...
    bars = [
        {"dt": _DT("2026-09-0%d" % (1 if i < 10 else 2)), "c": 100.0, "v": 100.0} for i in range(20)
    ]
    ...
    pay = build_json_payload([fake_res])
