...
def reconcile_account(db_path, name=None):
    ...
    try:
        trades = [dict(r) for r in conn.execute("SELECT * FROM paper_trades ORDER BY ts, id")]
        equity = [dict(r) for r in conn.execute("SELECT * FROM paper_equity ORDER BY ts, id")]
        orders_pending = conn.execute(
            "SELECT COUNT(*) FROM paper_orders WHERE status IN ('pending','queued')"
        ).fetchone()[0]
    finally:
        conn.close()
    ...
    implied = [
        r["static_equity"] - (r["realized"] or 0)
        for r in equity
    ...
    return {
        ...
        "win_rate": round(sum(1 for p in net_list if p > 0) / len(net_list), 4)
        if net_list
        else None,
        ...
        "equity0_implied": round(implied[-1], 2) if implied else None,
        ...
        "closed_details": [
            {
                "pos_ref": legs[0]["pos_ref"],
                "sym": legs[0]["sym"],
                "direction": legs[0]["direction"],
                "entry_ts": legs[0].get("entry_ts"),
...
def match_signals(closed_details, outcome_rows, window_min=30):
    ...
    if not n:
        return [], {"n": 0}
    dir_ok = sum(1 for t, o in matched if (t["net_pnl"] > 0) == (float(o["ret"]) > 0))
    paper_ret = [
        (t["net_pnl"] / t["entry_notional"]) if t["entry_notional"] else None for t, _ in matched
    ]
...
def backtest_reference(reports_dir):
    ...
    if os.path.isfile(pe):
        try:
            with open(pe, encoding="utf-8-sig") as f:
                lines = [ln.strip() for ln in f if ln.strip()]
            if len(lines) >= 2:
                head = lines[0].split(",")
                last = lines[-1].split(",")
                row = dict(zip(head, last, strict=False))
...
def reconcile_all(accounts_dir=None, monitor_db=None, reports_dir=None, window_min=30):
    ...
    return {
        ...
        "total_closed_trips": total_closed,
        "total_net": total_net,
        ...
        if total_closed < 20
            ...
            if total_net > 0
            else "含真实成本后为负（汇总净 %+.0f 元）——按纪律回退阈值/控规模" % total_net
        ),
        "accounts": accounts,
...
def render(res):
    ...
    for a in sorted(res["accounts"], key=lambda x: x["net_total"], reverse=True):
        L.append(
            ...
            % (
                ...
                "%+.0f" % a["net_total"],
                ("%.2f" % a["pf"])
                if a["pf"] is not None and a["pf"] < 999
                ...
                a["slip_total"],
                "—"
                if a["identity_ok"] is None
                else ("ok" if a["identity_ok"] else "***差%.2f" % (a["equity0_spread"] or -1)),
            )
        )
    ...
    for a in sm:
        m = a["signal_match"]
        L.append(
            "  %-14s 匹配 %d 对｜方向一致率 %.0f%%｜信号均收 %+.3f%% vs 纸面净均收 %+.3f%%"
            "｜信号胜率 %.0f%% vs 纸面胜率 %.0f%%"
            % (
                a["name"][:14],
                m["n"],
                m["dir_consistency"] * 100,
    ...
    L.append(
        "  （口径：信号=纯方向价格收益不含成本；纸面=含费含滑点、持仓期≠信号回看期，只比方向与相对量级）"
    )
    L.append("")
    L.append(
        "③ 回测参照：%s"
        % (
            ...
            if res["backtest_reference"]
            else "（portfolio_equity.csv / backtest_report.txt 均未生成，诚实降级）"
        )
    )
    L.append("")
...
def selftest():
    ...
    try:
        ...
        for r in rows:
            conn.execute(
                "INSERT INTO paper_trades(ts,pos_ref,sym,side,direction,lots,price,"
        ...
        conn.execute(
            "INSERT INTO paper_equity(ts,static_equity,float_pnl,equity,realized,fees_paid,n_trades)"
...
def main(argv=None):
    ...
    if args.selftest:
        return selftest()
    res = reconcile_all(
        args.accounts_dir, args.monitor_db, args.reports_dir, window_min=args.match_window_min
    )
    txt = render(res)
