...
def _weighted_fwd(members, fkey, weight_kind):
    ...
    if weight_kind == "ivol":
        ...
        if s <= 1e-15:  # 波动率全部缺失/退化 -> 安全退回等权
            ...
            return sum(w * m[fkey] for m, w in zip(members, ws, strict=False)) / s
    return sum(m[fkey] for m in members) / len(members)


def cross_section_periods(
    dates,
    by_date,
    factor_key,
    horizon,
    ...
    for di in range(0, len(dates), step):
        ...
        for _sym, p in row.items():
            ...
            if not (math.isfinite(fv) and math.isfinite(yv)):
                continue
            members.append(
                {"sym": p["sym"], "sector": p["sector"], "fv": fv, fkey: yv, "vol": p.get(vkey)}
            )
        ...
        short_side = _weighted_fwd(
            bot, fkey, weight_kind
        )  # 最弱档的"绝对涨幅"；做空收益=-short_side
        ...
        periods.append(
            {
                ...
                "long": long_ret,
                "short_abs": short_side,
                "short_pnl": -short_side,
                "ls": ls,
                "mkt": mkt,
                "long_excess": long_ret - mkt,
                "long_syms": [m["sym"] for m in top],
                "short_syms": [m["sym"] for m in bot],
                "sec_long": dict(sec_long),
                "sec_short": dict(sec_short),
            }
        )
    return periods
...
def overlap_daily_ls(dates, by_date, factor_key, horizon, lookback, n_q, min_names):
    ...
    for d in dates:
        ...
        if len(bands) < n_q:
            continue
        out.append(_weighted_fwd(bands[-1], fkey, "equal") - _weighted_fwd(bands[0], fkey, "equal"))
    return out
...
def bands_profile(periods, n_q):
    ...
    inc = sum(1 for a, b in zip(means, means[1:], strict=False) if b >= a)
    mono = inc / (n_q - 1)
    rank_q = list(range(1, n_q + 1))
    col_ric = fe.spearman(rank_q, means) if len(means) >= 2 else 0.0
    return {"means": means, "mono": mono, "col_rank_ic": col_ric, "spread": means[-1] - means[0]}
...
def sector_breakdown(periods):
    ...
    exposure = {
        s: {
            "long": long_w.get(s, 0.0),
            "short": short_w.get(s, 0.0),
            "net": long_w.get(s, 0.0) - short_w.get(s, 0.0),
        }
        for s in sectors
    }
    loso = {}
    for drop in sectors:
        ...
        clean = [
            p["ls"] for p in periods if drop not in p["sec_long"] and drop not in p["sec_short"]
        ]
        if clean:
...
def sector_internal(dates, by_date, factor_key, horizon, lookback, n_q, min_sector, step=None):
    ...
    return {
        s: {"n": len(v), "mean": sum(v) / len(v), "win": sum(1 for x in v if x > 0) / len(v)}
...
def _candidate_perf(panel, factor_key, L, H, n_q, cond_min, cost_round, scope, leg):
    ...
    pers = cross_section_periods(
        dates, by_date, factor_key, H, L, n_q, cond_min, "equal", H, sector_scope=scope
    )
...
def conditional_scan(windows, factor_key, L, H, n_q, cond_min, cost_round, candidates):
    ...
    for name, scope, leg in candidates:
        ...
        for wname, panel in windows:
            pers, pf = _candidate_perf(
                panel, factor_key, L, H, n_q, cond_min, cost_round, scope, leg
            )
...
def robust_verdict(row, tmin, decay_tol, long_n_ratio=1.5):
    ...
    if lp["net_t"] < sp["net_t"] - decay_tol:
        why.append(
            "长窗(%s)t=%+.2f比短窗(%s)t=%+.2f衰减超过%.1f（regime偶然嫌疑）"
            % (ln, lp["net_t"], sn, sp["net_t"], decay_tol)
        )
    n_s, n_l = sp.get("n", 0), lp.get("n", 0)
    if n_l < n_s * long_n_ratio:
        why.append(
            "长窗n=%d未比短窗n=%d多%.0f%%（池内品种凑齐分档门槛的历史不足，两窗实为同源小样本，非独立长样本）"
...
# =========================== 裁决 ===========================
def gate_verdict(
    main_perf, oos_perf, bands, leg_long, leg_short_pnl, exposure, tmin, mono_gate, max_sector_drive
):
    ...
    if not (main_perf["net_t"] >= tmin):
        reasons.append(
            "净多空t=%+.2f未达门槛%.1f（非重叠期n=%d）" % (main_perf["net_t"], tmin, main_perf["n"])
        )
    ...
    if exposure:
        ...
        if drive > max_sector_drive:
            reasons.append(
                "板块偏置：%s净敞口占比%.0f%%>%.0f%%（动量可能只由单一板块驱动）"
...
def evaluate_grid(dates, by_date, lookbacks, horizons, n_q, min_names, cost_round, main_l, main_h):
    ...
    for L in lookbacks:
        for H in horizons:
            pers = cross_section_periods(
                dates, by_date, "z%d" % L, H, L, n_q, min_names, "equal", H
            )
...
def _xs_ls_summary(
    dates, by_date, main_l, main_h, n_q, min_names, cost_round, weight="equal", leg="ls"
):
    """对给定面板跑 XSMOM 主因子（z{main_l}）截面多空，返回精简绩效摘要（纯函数，供掩码前后对照复用）。"""
    pers = cross_section_periods(
        dates, by_date, "z%d" % main_l, main_h, main_l, n_q, min_names, weight, main_h
    )
    if not pers:
        return None
    key = LEG_KEY.get(leg, "ls")
...
def compare_mask_xs(
    ...
    raw_sum = _xs_ls_summary(
        raw_main_dates, raw_by2, main_l, main_h, n_q, min_names, cost_round, weight, leg
    )
    ...
    md_sum = _xs_ls_summary(
        md_main_dates, md_by2, main_l, main_h, n_q, min_names, cost_round, weight, leg
    )
    out.update(
        {
            "mask": md_sum,
            ...
            "removed": {
                ...
                "locked": fm["removed_locked"],
                "near": fm["removed_near"],
...
def render_mask_compare_xs(cmp, main_l, main_h, leg="ls"):
    ...
    L.append(
        "  %-18s %10s %10s %10s %8s %8s %8s"
        % ("口径", "期数", "净t", "净均收%", "胜率%", "单调%", "Q5-Q1%")
    )
    for label, key in (("原始(无掩码)", "raw"), ("掩码后(剔不可交易)", "mask")):
        ...
        pf, bp = s["pf"], s["bands"]
        L.append(
            "  %-18s %10d %10.2f %10.3f %8.0f %8.0f %8.3f"
            % (
                label,
                s["n_periods"],
                pf["net_t"],
...
def build_report(
    ...
    Lout.append("=" * 108)
    Lout.append(
        "样本：%d 个品种、%d 个(品种×交易日)点、%d 个共同调仓日；日K经主连换月跳空置0比例后复权；"
        % (n_sym, len(points), len(dates))
    )
    Lout.append(
        "      每个调仓日跨品种按波动调整动量z排序分%d档，多最强一档/空最弱一档，市场中性多空组合。"
        % n_q
    )
    if errors:
        Lout.append(
    ...
    Lout.append(
        "口径：主口径=非重叠(每H日调仓持有H日,可复利)；净=毛-两腿往返成本(单程往返%.2f%%、两腿%.2f%%/期)；"
        % (cost_round * 100, 2 * cost_round * 100)
    )
    Lout.append(
        "      t=净多空期均÷(期std/√期数)；分档单调=相邻档平均收益递增比例；纯标准库零新增依赖。"
    )
    Lout.append("")

    # 表1：L×H 参数网格
    Lout.append(
        "一、回看窗 L × 持有期 H 参数网格（因子=波动调整z、等权、非重叠；格式 净均收%/期 | t | 胜率 | 净夏普）"
    )
    head = " %-8s " % "L＼H"
    for H in horizons:
    ...
    Lout.append(" " + "-" * 104)
    grid = evaluate_grid(
        dates, by_date, lookbacks, horizons, n_q, min_names, cost_round, main_l, main_h
    )
    for L in lookbacks:
        line = " %-8d " % L
        for H in horizons:
    ...
    perf_is, perf_oos = (
        perf_stats(is_p, main_h, cost_round, leg_key),
        perf_stats(oos_p, main_h, cost_round, leg_key),
    )
    if pers:
    ...
    # 原始 ret 因子对照（不做波动调整）
    pers_ret = cross_section_periods(
        dates,
        by_date,
        "ret%d" % main_l,
        main_h,
    ...
    Lout.append(
        "二、主组合（L=%d 日动量排序、持有 %d 日、%s、%d 档、池=%s、腿=%s）逐档与两腿拆解"
        % (
            main_l,
            main_h,
            "反波动率加权" if weight_kind == "ivol" else "等权",
            n_q,
            scope_txt,
            LEG_LABEL.get(main_leg, main_leg),
        )
    )
    if not pers:
        Lout.append(
            " ⚠ 主组合无可用调仓期（当日可得品种数不足 --min-names=%d 或暖机不足），以下主组合明细为空。"
            % min_names
        )
    Lout.append(
        " 各档(Q1最弱→Q%d最强)平均未来%d日收益：%s"
        % (n_q, main_h, " | ".join("%+.2f%%" % (v * 100) for v in bp["means"]))
    )
    Lout.append(
        " 分档单调性 %.0f%%、档位列序Spearman=%+.3f、Q%d-Q1 毛多空价差 %+.2f%%/期"
        % (bp["mono"] * 100, bp["col_rank_ic"], n_q, bp["spread"] * 100)
    )
    Lout.append(
        " 腿拆解（毛，/期）：多头最强档 %+.2f%%｜做空最弱档 %+.2f%%（最弱档绝对涨幅%+.2f%%）｜"
        "全市场等权基准 %+.2f%%" % (leg_long * 100, leg_short * 100, leg_bot * 100, leg_mkt * 100)
    )
    Lout.append(
        " 多空与市场基准相关系数 %+.3f（接近0=确实市场中性、赚的是相对排序而非beta）"
        % fe.pearson([p["ls"] for p in pers], [p["mkt"] for p in pers])
    )
    _fmt = lambda p: (
        (
            "n=%d 毛均%+.2f%% 净均%+.2f%% 净t=%+.2f 胜率%.0f%% 净累计%+.1f%% 年化%+.1f%% "
            "净夏普%+.2f 最大回撤%.1f%%"
        )
        % (
            p["n"],
    ...
    Lout.append(
        " 稳健性③日频重叠调仓(期数%d、重叠使t偏乐观仅参考)：毛均%+.2f%%/期、t=%+.2f"
    ...
    Lout.append(
        " 板块内各自做截面多空（只在同板块品种间排序，要求≥%d个品种；毛均/期、胜率、期数）："
        % min_sector
    )
    if internal:
        pos = 0
    ...
    # 表4：裁决
    Lout.append(
        "四、『确定不更差』并入判据（净t≥%.1f、净均>0、OOS不转负、分档单调≥%.0f%%且价差>0、"
        "至少一条腿方向对、单一板块净敞口≤%.0f%%）" % (tmin, mono_gate * 100, max_drive * 100)
    )
    ok, reasons = gate_verdict(
        perf_n, perf_oos, bp, leg_long, leg_short, exposure, tmin, mono_gate, max_drive
    )
    verdict = {
        "ok": ok,
        "reasons": reasons,
        "main": {
            ...
            "spread": bp["spread"],
            "leg_long": leg_long,
            "leg_short_pnl": leg_short,
            "mkt_beta": leg_mkt,
        },
    }
    if ok:
        Lout.append(
            " ✅ 主组合通过全部判据：截面动量多空在本样本成立，可作为下一轮在 cross_section 挂"
    ...
    if robust_panel is not None and candidates:
        ...
        scan = conditional_scan(
            windows, fk_main, main_l, main_h, n_q, cond_min or min_names, cost_round, candidates
        )
        Lout.append(
            "五、条件化增强 × 双样本稳健对照（L=%d/H=%d；回答'板块池或只做多能否救回动量、且长样本不衰减'）"
            % (main_l, main_h)
        )
        Lout.append(
            " 候选（板块池/腿）            | %s 净均/t/夏普/n        | %s 净均/t/夏普/n       | 双样本稳健"
        ...
        Lout.append(
            " 双样本稳健判据：两窗净均>0 且净t≥%.1f、长窗t不短窗衰减超%.1f、且长窗非重叠期数≥短窗×%.1f"
            "（显著性须随样本量稳定、长样本须真有增量历史，防板块品种上市晚致两窗同源，第31轮教训）。"
            % (tmin, decay_tol or 0.5, long_n_ratio or 1.5)
        )
        if robust_names:
            Lout.append(
                " ✅ 通过双样本稳健的条件化候选：%s——可作为下一轮挂影子的优先对象（仍默认不改综合分）。"
                % "、".join(robust_names)
            )
        else:
            Lout.append(
                " ❌ 没有任何条件化候选（板块池/多头腿）通过双样本稳健检验：截面动量经条件化仍不达标，继续维持纯研究。"
            )
        Lout.append("")

    Lout.append(
        "诚实边界：①主连为比例后复权近似、样本为近约 %d 根日K（约%.1f年）单一行情 regime，品种上市早晚不一；"
        % (days, days / 252.0)
    )
    Lout.append(
        "②非重叠期数有限（持有%d日约%d期），t 统计对正态/独立假设敏感；③历史规律不代表未来，本报告只给研究证据，"
        % (main_h, len(pers))
    )
    Lout.append(
        "  绝不自动修改 analyzer/cross_section 任何权重；④即便通过，并入仍须『默认影子、缺省等价旧版、可一键回退』。"
    )
    sidecar = _sidecar(
        points,
        grid,
...
def _sidecar(
    ...
    return {
        ...
        "main_h": main_h,
        "n_points": len(points),
        "n_symbols": len({p["sym"] for p in points}),
        "grid": g,
        "verdict": verdict,
        "sector_exposure": exposure,
        "sector_internal": internal,
    }
...
def collect_points(items, lookbacks, horizons, days, workers, prefer_panel=False, panel_db=None):
    ...
    with ThreadPoolExecutor(max_workers=max(1, workers)) as pool:
        futs = [
            pool.submit(_fetch_one, it, lookbacks, horizons, days, prefer_panel, panel_db)
            for it in items
        ]
...
def run(argv=None):
    ...
    ap.add_argument(
        "--days",
        type=int,
        default=config.XSMOM_ROBUST_DAYS,
        help="拉取/长样本日K根数（默认2500≈9.9年，供双样本稳健对照）",
    )
    ap.add_argument(
        "--main-days",
        type=int,
        default=config.XSMOM_EVAL_DAYS,
    ...
    ap.add_argument(
        "--scope", default="", help="主组合板块池，逗号分隔（如 有色,农产品），缺省=全市场"
    )
    ap.add_argument(
        "--leg",
        choices=("ls", "lex", "long"),
    ...
    ap.add_argument(
        "--mask",
        action="store_true",
        help="G22续：读 research_panel.db 算可交易性掩码（锁板/交割）并剔除不可交易点后重做截面",
    )
    ap.add_argument(
        "--mask-compare",
        action="store_true",
        help="G22续（第67轮）：掩码前后截面多空绩效对照表（同一输入各跑无掩码/有掩码）",
    )
    ap.add_argument(
        "--panel",
        action="store_true",
    ...
    points, errors = collect_points(
        items,
        lookbacks,
        horizons,
        args.days,
        args.workers,
        getattr(args, "panel", False),
        ...
        try:
            ...
            if os.path.exists(db_p):
                ...
                for row in con.execute(
                    "SELECT sym,date,c,h,l FROM research_panel ORDER BY sym,date"
                ):
    ...
    text, sidecar, verdict = build_report(
        ...
        args.tmin,
        args.mono_gate,
        args.max_sector_drive,
        cost_round,
        args.days,
        args.weight,
        robust_panel=(long_dates, long_by),
        candidates=candidates,
        cond_min=args.cond_min_names,
        decay_tol=args.decay_tol,
        main_days=len(main_dates),
        main_scope=main_scope,
        main_leg=args.leg,
        long_n_ratio=args.long_n_ratio,
    )
    ...
    print(
        "品种时点 %d、覆盖品种 %d；主组合裁决 ok=%s；双样本稳健候选 %d 个%s；报告 -> %s；JSON -> %s"
        % (
            len(main_points),
            sidecar["n_symbols"],
            verdict["ok"],
...
def selftest():
    ...
    # 波动率全缺失安全退回等权
    assert (
        abs(_weighted_fwd([{"f": 1.0, "vol": None}, {"f": 3.0, "vol": None}], "f", "ivol") - 2.0)
        < 1e-12
    )
    ...
    ok, why = gate_verdict(
        good_perf, good_perf, good_bands, 0.01, 0.01, {"有色": {"net": 0.2}}, 1.5, 0.75, 0.6
    )
    ...
    # 单一板块偏置否决
    ok3, why3 = gate_verdict(
        good_perf, good_perf, good_bands, 0.01, 0.01, {"能化": {"net": 0.85}}, 1.5, 0.75, 0.6
    )
    ...
    assert (
        "五、条件化" not in text and "conditional" not in sidecar
    )  # 缺省不传 robust=旧行为、无第五章
    ...
    p_you = cross_section_periods(
        d11, by11, "z60", 20, 60, 3, 6, "equal", 20, sector_scope=("有色",)
    )
    ...
    scan = conditional_scan(
        [("短", (short_d, by_date)), ("长", (dates, by_date))], "z60", 60, 20, 5, 16, 0.0003, cands
    )
    ...
    ok_r, _ = robust_verdict(
        {"windows": {"短": _perf(2.0, 0.01, 40), "长": _perf(1.8, 0.01, 100)}}, 1.5, 0.5
    )
    assert ok_r
    ok_d, why_d = robust_verdict(
        {"windows": {"短": _perf(2.2, 0.01, 40), "长": _perf(1.0, 0.01, 100)}}, 1.5, 0.5
    )
    assert not ok_d and any("衰减" in w for w in why_d)
    # 长窗期数没比短窗多（板块品种上市晚、两窗同源）-> 即便 t 都高也不算双样本稳健
    ok_s, why_s = robust_verdict(
        {"windows": {"短": _perf(2.01, 0.027, 25), "长": _perf(2.01, 0.027, 25)}}, 1.5, 0.5
    )
    ...
    c0 = compare_mask_xs(
        pts, (20, 60), (5, 20), 20, 20, 5, 16, 0.0003, weight="equal", leg="ls", mask=None
    )
    ...
    c1 = compare_mask_xs(
        pts, (20, 60), (5, 20), 20, 20, 5, 16, 0.0003, weight="equal", leg="ls", mask=fake_mask
    )
    ...
    print(
        "xsmom_eval selftest ALL PASS（远期无泄漏/分档/加权/趋势多空/成本/IS-OOS/裁决门/报告/"
