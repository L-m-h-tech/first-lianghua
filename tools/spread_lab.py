...
def term_symbol_stat(term_series, win=Z_WIN, min_n=Z_MIN):
    ...
    return {
        ...
        "near": row.get("near"),
        "next": row.get("next"),
        "near_s": row.get("near_s"),
        "next_s": row.get("next_s"),
        "spread_pct": cur_sp,
        "carry_ann": row.get("carry_nn"),
        "slope": row.get("slope"),
        "curve": "back" if (cur_sp or 0) > 0 else "contango",
        "spread_z": zsp[ti],
        "spread_pctile": psp[ti],
        "carry_z": zcr[ci] if ci is not None else None,
        "oi_sum": row.get("oi_sum"),
        "n_live": row.get("n_live"),
        "n_days": len(term_series),
    }


def aligned_ratio(dates_a, close_a, dates_b, close_b):
    """按公共交易日对齐两条腿收盘价，返回 (公共日期升序, 比价A/B)；任一条腿非正/缺失跳过。"""
    mb = dict(zip(dates_b, close_b, strict=False))
    od, ratio = [], []
    for d, ca in zip(dates_a, close_a, strict=False):
        cb = mb.get(d)
...
def aligned_margin(close_map, weights, fee=0.0):
    ...
    for d in sorted(common):
        ...
        if all(_isnum(x) and x > 0 for x in px):
            v = sum(w * x for (_, w, _), x in zip(legs, px, strict=False)) - (fee or 0.0)
            od.append(d)
            vals.append(v)
    return od, vals
...
def margin_stat(dates, values, win=Z_WIN, min_n=Z_MIN, chg_win=CHG_WIN):
    ...
    t = len(values) - 1
    chg = (
        values[t] - values[t - chg_win]
        if t - chg_win >= 0 and _isnum(values[t - chg_win])
        else None
    )
    return {
        "date": dates[t],
...
def render(meta, term_stats, chain_stats, margin_stats, breadth):
    ...
    L.append("=" * 108)
    L.append(
        "G12 产业链/跨期价差监控 spread_lab（纯离线只读 term_history+research_panel，只监控不下单、不进综合分）"
    )
    L.append(
        "跨期样本 %s~%s；z=当前价差相对过去%d日尾窗（|z|≥1.5 标↑/↓）；全市场 backwardation 广度 %d/%d（%.0f%%）"
        % (
            meta["d0"],
            meta["d1"],
            Z_WIN,
    ...
    L.append("-" * 108)
    L.append(
        "【一】跨期价差 / 期限结构（按年化 carry 升序：越负=contango 远月升水越深、展期负carry越重）"
    )
    L.append(
        "  %-6s %-8s %-7s %9s %9s %8s %8s %8s %7s %10s"
        % (
            "品种",
            "近月",
            "次月",
    ...
    rows = sorted(
        term_stats,
        key=lambda r: (r["carry_ann"] is None, r["carry_ann"] if r["carry_ann"] is not None else 0),
    )
    for r in rows:
    ...
    L.append(
        "  读法：价差%>0/back=近高远低(现货升水、backwardation，多头展期有正carry)；<0/contango=远月升水；"
        "价差z极端=期限结构相对自身历史异常拉伸/收敛。"
    )
    L.append("-" * 108)
    L.append(
        "【二】产业链比价（无量纲 A/B；z=相对过去%d日偏离，近%d日变化看趋势；只监控相对强弱，不是利润额）"
        % (Z_WIN, CHG_WIN)
    )
    L.append(
        "  %-8s %-22s %-5s %9s %8s %7s %9s"
        % ("板块", "产业链比价", "腿", "最新比价", "z", "分位", "近60日")
    )
    for c in chain_stats:
        ...
        L.append(
            "  %-8s %-22s %s/%-4s %9.4f %8s %7s %9s"
            % (c["sector"], c["name"], c["a"], c["b"], st["ratio"], _zstr(st["z"]), pc, chg)
        )
    L.append("-" * 108)
    L.append(
        "【三】产业链盘面虚拟利润额（投料/出成系数+加工费，元/吨主产品；z=利润额相对过去%d日偏离，近%d日变化单位元/吨）"
        % (Z_WIN, CHG_WIN)
    )
    ...
    for mg in margin_stats:
        ...
        pc = ("%.2f" % st["pctile"]) if _isnum(st["pctile"]) else " -- "
        L.append(
            "  %-8s %-24s %10.0f %8s %7s %10s   %s"
            % (mg["sector"], mg["name"], st["value"], _zstr(st["z"]), pc, chg, mg["note"])
        )
    L.append("-" * 108)
    L.append(
        "【四】诚实边界：跨期=日频结算价、未计手续费/滑点/保证金/换月；【二】无量纲比价只看相对强弱（不同合约单位不可直接相减），"
    )
    L.append(
        "【三】盘面虚拟利润用行业经验投料/出成系数与固定加工费（随工艺/地区/时间浮动，非精确财务利润）、虚拟钢厂毛利未计其他制造成本、"
    )
    L.append(
        "DCE一号大豆为食用国产豆与进口压榨口径有偏差；固定面板有幸存者偏差；全程只监控不交易、不进综合分。"
    )
    return "\n".join(L)
...
def _load_panel_close(panel_db):
    ...
    rows = (
        ...
        else _load_all(store, sorted(store.symbols()))
    )
...
def run(term_db=TERM_DB, panel_db=PANEL_DB, txt_path=LAB_TXT, json_path=LAB_JSON, verbose=True):
    ...
    term_syms = sorted(
        r[0] for r in tstore.conn.execute("SELECT DISTINCT sym FROM ckline ORDER BY sym")
    )
    term_stats = []
    all_d0, all_d1 = [], []
    for s in term_syms:
        ...
        if ts:
            all_d0.append(ts[0]["date"])
            all_d1.append(ts[-1]["date"])
    ...
    meta = {
        "term_symbols": len(term_syms),
        "term_stats_n": len(term_stats),
        "d0": min(all_d0) if all_d0 else None,
        "d1": max(all_d1) if all_d1 else None,
        "panel_d0": pd0,
        "panel_d1": pd1,
        "z_win": Z_WIN,
        "chg_win": CHG_WIN,
    ...
    try:
        extreme = [
            r["sym"] for r in term_stats if _isnum(r["spread_z"]) and abs(r["spread_z"]) >= 1.5
        ]
        el.safe_record(
            "spread_lab",
            {
                "z_win": Z_WIN,
                "chg_win": CHG_WIN,
                "term_db": os.path.basename(term_db),
                "panel_db": os.path.basename(panel_db),
            },
            {
                "term_n": len(term_stats),
                "back_n": back,
                "z_extreme": extreme,
                "chain_n": sum(1 for c in chain_stats if c["stat"]),
                "margin_n": sum(1 for m in margin_stats if m["stat"]),
                "margin_latest": {
                    m["name"]: (
                        round(m["stat"]["value"], 1)
                        if m["stat"] and _isnum(m["stat"]["value"])
                        else None
                    )
                    for m in margin_stats
                    if m["stat"]
                },
            },
            inputs=[term_db, panel_db],
            artifacts=[txt_path, json_path],
            conclusion="G12跨期/产业链价差监控：%d品种backwardation %d只、|价差z|≥1.5异常 %s；产业链比价%d对、盘面虚拟利润%d条，只监控不下单"
            % (
                len(term_stats),
                back,
                ",".join(extreme[:12]) or "无",
...
def selftest():
    ...
    ts = [
        {"near_s": 101.0, "next_s": 100.0, "carry_nn": 0.05},
    ...
    od, r = aligned_ratio(["d1", "d2", "d3"], [2.0, 4.0, 6.0], ["d2", "d3", "d4"], [2.0, 2.0, 3.0])
    ...
    print(
        "spread_lab selftest ALL PASS（尾窗z/分位PIT、近-次价差形态、对齐比价、产业链z、盘面利润额系数、端到端 共10组）"
    )
    return 0
