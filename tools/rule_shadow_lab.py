...
def _stats(pf):
    ...
    fees = sum((t.get("open_fee_yuan") or 0.0) + (t.get("close_fee_yuan") or 0.0) for t in closed)
    exits = sorted(t["exit_dt"] for t in closed)
    return {
        "n": n,
        "net": round(net, 0),
        "win": round(wins / n, 3) if n else None,
        "pf": round(gp / gl, 2) if gl > 0 else (None if not gp else 999.0),
        "short_n": short_n,
        "stop_n": stop_n,
        "rev_n": len(rev),
        "rev_net": round(rev_net, 0),
        "fees": round(fees, 0),
...
def run_lab(codes=None, period=30, lookback=None, r1_entry=3.0, r4_min_hold=2, workers=6):
    ...
    args = pf_mod.parse_args(
        ["--codes", codes, "--period", str(period), "--lookback", str(lookback)]
    )
    ...
    with ThreadPoolExecutor(max_workers=max(1, workers)) as ex:
        futs = {
            ex.submit(pf_mod.load_minute_feed, it, args, fee_table, margin_table): it[0]
            for it in items
        }
    ...
    def _run(entry, min_hold, no_reverse):
        ...
        st = _stats(pf)
        st["perf"] = {
            "total_ret": round(pf.performance()["total_ret"], 4),
    ...
    for label, entry, mh, nr in variants:
        st = _run(entry, mh, nr)
        st.pop("closed")
        results.append({"label": label, "entry_th": entry, "min_hold": mh, "no_reverse": nr, **st})
    base = results[0]
    ...
    for label, entry, mh, nr in variants:
        ...
        halves.append({"label": label, "h1_net": round(sum(h1), 0), "h2_net": round(sum(h2), 0)})
    ...
    def _pf_of(r):
        return (
            r["pf"]
            if r["pf"] is not None and r["pf"] < 999
            else (10.0 if r["pf"] == 999.0 else 0.0)
        )
    ...
    if r1:
        ...
        verdicts["R1"] = (
            "通过"
            if (_pf_of(r1) >= 1.05 and drop >= 0.40)
    ...
    if r4:
        verdicts["R4"] = (
            "通过"
            if r4["net"] > base["net"]
            else "未通过（净 %+0.f vs 基线 %+0.f）" % (r4["net"], base["net"])
        )
    ...
    if r3:
        verdicts["R3"] = (
            "通过"
            if r3["net"] > base["net"]
            else "未通过（净 %+0.f vs 基线 %+0.f，代理口径）" % (r3["net"], base["net"])
        )
    ...
    return {
        ...
        "period": period,
        "lookback": lookback,
        "entry_baseline": args.entry,
        "r1_entry": r1_entry,
        "r4_min_hold": r4_min_hold,
        "n_syms": len(feeds),
...
def render(res):
    L = [
        "Phase2 规则影子实验室（第131轮——单变量影子回放，同批 feeds 确定性重放）",
        "=" * 66,
    ...
    L.append(
        "%-14s %5s %10s %6s %7s %6s %6s %9s %7s %7s"
        % ("配置", "笔数", "净盈亏", "胜率", "PF", "1-2bar", "止损", "最大回撤", "夏普", "费用")
    )
    for r in res["results"]:
        L.append(
            "%-14s %5d %10s %6s %7s %6d %6d %9s %7s %7.0f"
            % (
                r["label"],
                r["n"],
                format(r["net"], "+,.0f"),
                ...
                str(r["perf"]["sharpe"]) if r["perf"] else "—",
                r["fees"],
            )
        )
    L.append("")
...
def main(argv=None):
    ...
    res = run_lab(
        ...
        r1_entry=args.r1_entry,
        r4_min_hold=args.r4_min_hold,
