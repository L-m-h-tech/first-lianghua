...
def probe_servers(servers):
    ...
    with ThreadPoolExecutor(max_workers=min(len(servers), 16)) as pool:
        for srv, ok in zip(servers, pool.map(_probe, servers), strict=False):
            if ok:
                online.append(ok)
    return online
...
def collect(tasks, timeout=REQUEST_TIMEOUT, workers=None):
    ...
    if not _online:
        LOG("无在线服务器，请检查 SERVERS 列表与安全组/服务状态")
        return [], {}
    LOG(
        f"在线服务器 {len(_online)} 台（候选 {len(SERVERS)}，上限 {MAX_SERVERS}），任务 {len(tasks)} 个"
    )
    ...
    def _fetch_one(task):
        ...
        while len(tried) < len(_online) + 1:  # +1 容错（列表可能刚剔除）
            ...
            if srv is None or srv in tried:
                break
            tried.add(srv)
            data = _get(f"{srv}{url_tpl.format(c=code, p=period, l=lmt)}", timeout=timeout)
...
def main():
    ...
    LOG(
        f"模式={args.mode}  品种={len(SYMS)}  周期={periods}  总任务={len(tasks)}  "
        f"候选服务器={len(SERVERS)}  上限={MAX_SERVERS}"
    )
    if not SERVERS:
        LOG("请先配置 SERVERS 列表（服务器 IP:PORT）")
        return
    ...
    if not stats:
        return
    LOG(
        f"完成：成功={stats['success']}  失败={stats['fail']}  "
