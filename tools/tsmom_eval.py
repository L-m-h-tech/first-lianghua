...
def eval_factor_horizon(records, factor, horizon, n_q):
    ...
    for x, y, m in zip(xs, ys, months, strict=False):
        by_month[m].append((x, y))
    mic = fe.monthly_ic(by_month)
    buckets = fe.quantile_buckets(list(zip(xs, ys, strict=False)), n_q)
    mono, spread = fe.monotonic_score(buckets)
    hit = sum(1 for x, y in zip(xs, ys, strict=False) if (x > 0) == (y > 0) and x != 0) / n
    return {
        "n": n,
        "ic": ic,
        "rank_ic": ric,
...
def incremental_residual_ic(records, target, controls, horizon):
    ...
    for i, r in enumerate(records):
        ...
        if not (math.isfinite(tv) and math.isfinite(yv) and all(math.isfinite(v) for v in vals)):
            continue
        idx.append(i)
        for c, v in zip(controls, vals, strict=False):
...
def per_symbol_stats(records, factor, horizon, min_pts=20):
    ...
    ics = [
        fe.spearman([p[0] for p in pts], [p[1] for p in pts])
...
def ic_weighted_blend(records, zkeys, horizon, weights):
    ...
    for r in records:
        ...
        if yv is None or any(z is None for z in zs):
            continue
        x = sum(
            weights[k] * math.tanh(max(-3.0, min(3.0, z))) for k, z in zip(zkeys, zs, strict=False)
        )
        xs.append(x)
        ys.append(yv)
    return xs, ys
...
def _gate_ok(m, resid_ic, oos_ric, gate, psym=None):
    ...
    if psym is not None and psym["n"] >= 10:
        if psym["pos_ratio"] < 0.5 or psym["median"] < 0:
            reasons.append(
                "品种内不成立：仅%d/%d品种RankIC为正(%.0f%%)、中位%+.3f"
                % (
                    round(psym["pos_ratio"] * psym["n"]),
                    psym["n"],
                    psym["pos_ratio"] * 100,
                    psym["median"],
                )
...
def build_report(
    records, errors, lookbacks, horizons, main_h, n_q, min_sample, gate, oos_ratio, days, workers
):
    L = []
    L.append("=" * 104)
    L.append(
        " G7 多窗口时序动量 TSMOM(%s) 离线时序IC评估  生成于 %s"
        % ("/".join(map(str, lookbacks)), _now())
    )
    L.append("=" * 104)
    L.append(
        "样本：%d 个品种共 %d 个(品种×交易日) pooled 时点；日K经主连换月跳空置0的比例后复权；"
        "暖机≥%d根、持有到 %d 日，无未来函数。"
    ...
    L.append(
        "口径：RankIC=Spearman(因子, 未来H日收益)，主看它；IC=Pearson；ICIR=月度RankIC的mean/std；"
    )
    L.append(
        "      z=窗口收益÷(窗口日收益std×√%d)的波动调整趋势、跨品种可比；blend=等权tanh(z)；纯标准库零新增依赖。"
        % config.TSMOM_ANN
    )
    L.append("")
    ...
    L.append(
        "一、各因子对未来收益的预测力总表（看 5→20→60 日 的衰减/增强；格式 RankIC / ICIR / n）"
    )
    head = " %-16s " % "因子"
    for H in horizons:
        head += "| 未来%2d日                     " % H
    ...
    L.append(
        " %-16s %6s %8s %8s %7s %8s %7s %7s  各档平均未来收益(%%)/胜率(%%)"
    ...
    for fk, cn, _ in FACTORS:
        ...
        L.append(
            " %-16s %6d %+8.3f %+8.3f %+7.2f %+7.2f%% %6.0f%% %6.0f%%  %s"
            % (
                cn,
                m["n"],
                m["ic"],
    ...
    # 表4：边际增量（对短窗残差IC）
    L.append(
        "四、长窗相对现有短窗(ret5/ret20)的边际增量（剔除短窗线性信息后的残差 RankIC，主地平线%d日）"
        % main_h
    )
    resid = {}
    ...
    L.append(
        "五、等权合成 vs 样本内IC加权合成（IS=前%.0f%%估权重，OOS=后%.0f%%验证，主地平线%d日，防自欺）"
    ...
    for tag, subset in (("IS", is_rec), ("OOS", oos_rec)):
        ...
        L.append(
            " %-3s n=%-5d 等权blend RankIC=%+6.3f | IC加权合成 RankIC=%+6.3f"
            % (tag, len(xe), eqw, icw)
        )
    ...
    # 表6：裁决
    L.append(
        "六、是否满足『确定不更差』的并入判据（主H RankIC≥%.2f、ICIR>0、OOS不转负、对短窗残差增量≥0、"
    ...
    for fk, cn, is_long in FACTORS:
        ...
        verdict[fk] = {
            "ok": ok,
            "main_rank_ic": m["rank_ic"],
            "icir": m["icir"],
            "oos_rank_ic": oos_ric,
            "resid_rank_ic": resid.get(fk),
            "per_symbol_n": psym["n"],
            "per_symbol_pos_ratio": psym["pos_ratio"],
            "per_symbol_median": psym["median"],
            "reasons": reasons,
        }
        ...
        if m["n"] < min_sample:
            L.append(
                " %-16s 样本不足(n=%d<%d)，暂不下结论，继续影子积累。%s"
                % (cn, m["n"], min_sample, sym_txt)
            )
        elif ok:
            L.append(
                " %-16s ✅ 判据通过：主H RankIC=%+.3f、ICIR=%+.2f、OOS=%s、残差增量=%s；%s，可作为后续并入候选。"
                % (
                    cn,
                    m["rank_ic"],
                    m["icir"],
                    ("%+.3f" % oos_ric) if oos_ric is not None else "NA",
                    ("%+.3f" % resid[fk]) if resid.get(fk) is not None else "NA",
                    sym_txt,
                )
            )
        else:
    ...
    L.append("")
    L.append(
        "诚实边界：①主连为比例后复权近似、样本来自近约 %d 根日K（约4年），存在品种/时段与单一行情 regime 偏差；"
        % days
    )
    L.append(
        "②时序IC对非线性/拐点不敏感，历史规律不代表未来；③本报告只提供研究证据，绝不自动修改 analyzer 权重；"
    )
    L.append(
        "④即便判据通过，并入时仍须遵守『默认影子、缺省等价旧版、可一键回退』，且先只加信息展示再谈改分。"
    )
    sidecar = _sidecar(
        records, metrics, cmat, resid, weights, verdict, lookbacks, horizons, main_h, days
    )
    return "\n".join(L) + "\n", sidecar, metrics
...
def run(argv=None):
    ...
    ap.add_argument(
        "--panel", action="store_true", help="G21续：优先读已复权研究面板（缺省联网现拉）"
    )
    ap.add_argument(
        "--panel-db",
        default=None,
...
def selftest():
    ...
    for seg in range(12):  # 12 段交替的持续趋势段
        ...
        for _ in range(40):
            ...
            raw.append(
                {
                    "d": "202%d-%02d-%02d" % (seg // 4 + 2, seg % 12 + 1, len(raw) % 28 + 1),
                    "o": price,
                    "h": price * 1.001,
                    "l": price * 0.999,
                    "c": price,
    ...
    xe, ye = ic_weighted_blend(
        recs_up, ["z63", "z126", "z252"], 20, {"z63": 0.5, "z126": 0.5, "z252": 0.0}
    )
    ...
    assert build_report(recs_up[:8], [], (63, 126, 252), (5, 20), 20, 5, 120, 0.02, 0.3, 1023, 1)[
        0
    ]  # 少量样本仍能出报告文本

    # 9.5) _gate_ok：pooled 达标但分品种多数为负必须否决（防 pooled 截面伪相关）；反之过半为正才通过
    fakem = {"rank_ic": 0.03, "icir": 0.2}
    ok_bad, why = _gate_ok(fakem, 0.01, 0.02, 0.02, {"n": 64, "pos_ratio": 9 / 64, "median": -0.15})
    assert not ok_bad and any("品种内" in w for w in why), (ok_bad, why)
    ok_good, _ = _gate_ok(fakem, 0.01, 0.02, 0.02, {"n": 64, "pos_ratio": 0.6, "median": 0.03})
    assert ok_good

    # 10) build_report 全量跑通且裁决键齐全
    text, sidecar, _ = build_report(
        recs_up, [("BAD", "x")], (63, 126, 252), (5, 20, 60), 20, 5, 50, 0.02, 0.3, 1023, 1
    )
    assert "TSMOM" in text and len(sidecar["factors"]) == len(FACTORS)
    print(
        "tsmom_eval selftest ALL PASS（远期收益/OLS正交/趋势正IC·回复负IC/IS-OOS/增量/降级 共10组）"
    )
    return 0
