...
def selftest():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        try:
            ...
            ck(
                "manifest已登记",
                os.path.exists(pa.MANIFEST) and "test_src" in Path(pa.MANIFEST).read_text("utf-8"),
            )
...
def main(argv=None):
    ...
    if args.selftest:
        return selftest()
    html = (
        Path(args.html_file).read_text(encoding="utf-8", errors="ignore") if args.html_file else ""
    )
