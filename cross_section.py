...
def rank(fut_rows):
    ...
    try:
        ...
        out = []
        for r, s, c, zs, zc in zip(rows_in, scores, chgs, sz, cz, strict=False):
            if robust:
                xs = ws * zs + wc * zc
            else:
        ...
        for o in out:
            s = sectors.setdefault(
                o["cat"], {"n": 0, "up": 0, "down": 0, "xs": [], "score": [], "chg": []}
            )
            s["n"] += 1
            if o["chg"] > 1e-6:
                s["up"] += 1
...
def format_block(cs):
    ...
    L = []
    L.append(
        "【横截面强弱】(跨%d品种稳健z=0.6745(x-中位数)/MAD抗极端值; 综合强度=%.1f综合分z+%.1f涨跌幅z; "
        "只做横向比较、不改变原综合分/信号)"
    ...
    L.append(
        " 相对最强Top%d: " % config.XS_TOP_N
        + ("、".join(_brief(o) for o in cs["top_long"]) or "无")
    )
    L.append(
        " 相对最弱Top%d: " % config.XS_TOP_N
        + ("、".join(_brief(o) for o in cs["top_short"]) or "无")
    )
    b = cs["breadth"]
    L.append(
        " 全市场广度: 偏多%d家/中性%d家/偏空%d家，平均涨跌%+.2f%%%s"
        % (
            b["bull"],
            b["neutral"],
            b["bear"],
